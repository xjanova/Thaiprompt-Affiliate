    ...
    <div class="mb-6">
        <div class="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
            <div>
                ...
                <p class="text-gray-600 dark:text-gray-400 mt-1">กำหนดค่า Point Value และค่าคอมมิชชั่นสำหรับสินค้า</p>
            </div>
            <a href="{{ route('admin.mlm.product-pv.create') }}"
               class="bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white px-6 py-2.5 rounded-lg shadow-lg transition-all duration-200 flex items-center justify-center gap-2">
                            ...
                            <div class="flex items-center">
                                @if($productPv->product->images->count() > 0)
                                    <img src="{{ $productPv->product->images->first()->image_path }}"
                                         ...
                                         class="w-12 h-12 rounded-lg object-cover mr-3 shadow">
                                @else
                                    <div class="w-12 h-12 rounded-lg bg-gray-200 dark:bg-gray-700 mr-3 flex items-center justify-center">
                                ...
                                <div>
                                    ...
                                    <div class="text-sm text-gray-500 dark:text-gray-400">SKU: {{ $productPv->product->sku }}</div>
                                    <div class="text-xs text-gray-500 dark:text-gray-400">ราคา: ฿{{ number_format($productPv->product->price, 2) }}</div>
                                </div>
                            </div>
                        </td>
                    ...
                    <tr>
                        <td colspan="6" class="px-6 py-12 text-center text-gray-500 dark:text-gray-400">
                            <div class="flex flex-col items-center">
                                ...
                                <p class="text-sm text-gray-400 dark:text-gray-500 mt-1">เริ่มต้นด้วยการเพิ่มการกำหนด PV ให้กับสินค้า</p>
                                <a href="{{ route('admin.mlm.product-pv.create') }}"
                                   class="mt-4 bg-blue-600 hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-600 text-white px-6 py-2 rounded-lg inline-flex items-center gap-2 transition-all duration-200">
                                    <i class="fas fa-plus"></i>
                                    เพิ่มการกำหนด PV
                                </a>
                            </div>
    ...
    <div class="mt-6 bg-gradient-to-r from-purple-50 to-pink-50 dark:from-purple-900/20 dark:to-pink-900/20 rounded-xl shadow-lg p-6 border border-purple-100 dark:border-purple-800">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div class="bg-white dark:bg-gray-800 rounded-lg p-4 shadow border border-gray-200 dark:border-gray-700">
                <p class="text-sm text-gray-600 dark:text-gray-400">จำนวนสินค้าที่กำหนด PV</p>
                <p class="text-2xl font-bold text-purple-600 dark:text-purple-400 mt-1">{{ number_format($productPvs->total()) }}</p>
            </div>
            <div class="bg-white dark:bg-gray-800 rounded-lg p-4 shadow border border-gray-200 dark:border-gray-700">
                <p class="text-sm text-gray-600 dark:text-gray-400">PV เฉลี่ย</p>
                <p class="text-2xl font-bold text-blue-600 dark:text-blue-400 mt-1">
                    {{ number_format($productPvs->avg('pv_value'), 2) }} PV
                </p>
            </div>
            <div class="bg-white dark:bg-gray-800 rounded-lg p-4 shadow border border-gray-200 dark:border-gray-700">
                <p class="text-sm text-gray-600 dark:text-gray-400">ใช้ Global Rate</p>
                <p class="text-2xl font-bold text-green-600 dark:text-green-400 mt-1">
                    {{ number_format($productPvs->where('use_global_rate', true)->count()) }}
                </p>
            </div>
            <div class="bg-white dark:bg-gray-800 rounded-lg p-4 shadow border border-gray-200 dark:border-gray-700">
                <p class="text-sm text-gray-600 dark:text-gray-400">ใช้ Custom Rate</p>
