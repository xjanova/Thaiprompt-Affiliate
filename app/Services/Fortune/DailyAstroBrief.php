...
class DailyAstroBrief
{
     ...
     *
     * อยู่ที่นี่เพราะ **ทั้ง 3 เลนของดวงรายวันต้องใช้กฎชุดเดียวกัน**
     * (บทความเว็บ/DM · โพสเพจรายวัน · โพส 7 วันเกิด) — เคยเขียนแยกกันแล้วหลุด
     * ไปเลนละแบบ จนเลนโพส 7 วันเกิดไม่มีกฎเลยและสุ่มไพ่ทาโรต์แทนดาวอยู่ 5 เดือน
     *
     * ⚠️ บล็อกนี้จงใจ**ไม่ใส่อีโมจิ** — ปลายทางถูก FacebookContentPolicy กวาดอีโมจิทิ้ง
     *    ใส่ใน prompt เท่ากับสอนโมเดลว่าใช้ได้ (บทเรียน dignity_label 2026-08-02)
     *
     * @param  string  $dayName  ชื่อวันเกิดของกลุ่มเป้าหมาย เช่น "อังคาร"
     * @param  bool  $briefOk  false = คำนวณดาวไม่สำเร็จ ต้องห้ามอ้างดาวทั้งหมด
     */
    public static function promptRules(string $dayName, bool $briefOk): string
    {
        $factRule = $briefOk
