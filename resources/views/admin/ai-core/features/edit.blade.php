...
@section('title', 'แก้ไข AI Feature: ' . $feature->feature_name)

@section('content')
<div class="container-fluid px-4 py-6" x-data="featureEdit()">
    {{-- Header --}}
    <div class="mb-6">
        ...
        <p class="text-gray-600 dark:text-gray-400 ml-13">
            แก้ไขข้อมูล Feature: <strong>{{ $feature->feature_name }}</strong>
        </p>
    </div>

    <form method="POST" action="{{ route('admin.ai-core.features.update', $feature) }}" class="space-y-6">
        @csrf
        @method('PUT')
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                        ⚠️ ไม่สามารถแก้ไข Feature Key ได้ (ป้องกันความเสียหายต่อระบบ)
                    </p>
                </div>
                ...
                <div>
                    ...
                    <select name="category"
                        ...
                        <option value="marketplace" {{ old('category', $feature->category) == 'marketplace' ? 'selected' : '' }}>Marketplace</option>
                        <option value="generation" {{ old('category', $feature->category) == 'generation' ? 'selected' : '' }}>Generation</option>
                        <option value="integration" {{ old('category', $feature->category) == 'integration' ? 'selected' : '' }}>Integration</option>
                        <option value="automation" {{ old('category', $feature->category) == 'automation' ? 'selected' : '' }}>Automation</option>
                        <option value="analytics" {{ old('category', $feature->category) == 'analytics' ? 'selected' : '' }}>Analytics</option>
                        <option value="trading" {{ old('category', $feature->category) == 'trading' ? 'selected' : '' }}>Trading</option>
                        <option value="platform" {{ old('category', $feature->category) == 'platform' ? 'selected' : '' }}>Platform</option>
                    </select>
                    @error('category')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="quota_type"
                        ...
                        <option value="none" {{ old('quota_type', $feature->quota_type) == 'none' ? 'selected' : '' }}>ไม่จำกัด (None)</option>
                        <option value="count" {{ old('quota_type', $feature->quota_type) == 'count' ? 'selected' : '' }}>จำนวนครั้ง (Count)</option>
                        <option value="usage" {{ old('quota_type', $feature->quota_type) == 'usage' ? 'selected' : '' }}>การใช้งาน (Usage)</option>
                        <option value="duration" {{ old('quota_type', $feature->quota_type) == 'duration' ? 'selected' : '' }}>ระยะเวลา (Duration)</option>
                        <option value="token" {{ old('quota_type', $feature->quota_type) == 'token' ? 'selected' : '' }}>โทเค็น (Token)</option>
                    </select>
                    @error('quota_type')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                ...
                <div x-show="quotaType !== 'none'">
                    ...
                    <input type="number"
                           ...
                           value="{{ old('default_quota_limit', $feature->default_quota_limit) }}"
                           min="0"
                ...
                <div x-show="quotaType !== 'none'">
                    ...
                    <input type="number"
                           ...
                           value="{{ old('max_quota_allowed', $feature->max_quota_allowed) }}"
                           min="0"
                ...
                <div x-show="quotaType !== 'none'" class="flex items-center gap-3">
                    ...
                    <input type="checkbox"
                           name="allow_overage"
                           value="1"
                           {{ old('allow_overage', $feature->allow_overage) ? 'checked' : '' }}
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    <select name="pricing_tier"
                        ...
                        <option value="free" {{ old('pricing_tier', $feature->pricing_tier) == 'free' ? 'selected' : '' }}>ฟรี (Free)</option>
                        <option value="basic" {{ old('pricing_tier', $feature->pricing_tier) == 'basic' ? 'selected' : '' }}>พื้นฐาน (Basic)</option>
                        <option value="pro" {{ old('pricing_tier', $feature->pricing_tier) == 'pro' ? 'selected' : '' }}>โปร (Pro)</option>
                        <option value="enterprise" {{ old('pricing_tier', $feature->pricing_tier) == 'enterprise' ? 'selected' : '' }}>องค์กร (Enterprise)</option>
                    </select>
                    @error('pricing_tier')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                ...
                <div x-show="pricingTier !== 'free'">
                    ...
                    <input type="number"
                           name="base_price"
                           value="{{ old('base_price', $feature->base_price) }}"
                           min="0"
                           step="0.01"
                ...
                <div x-show="pricingTier !== 'free'">
                    ...
                    <input type="number"
                           name="usage_price"
                           value="{{ old('usage_price', $feature->usage_price) }}"
                           min="0"
                           step="0.01"
            ...
            <div class="space-y-4">
                ...
                <div class="flex items-center gap-3 p-4 bg-gray-50 dark:bg-gray-700/50 rounded-xl">
                    ...
                    <input type="checkbox"
                           name="is_enabled"
                           value="1"
                           {{ old('is_enabled', $feature->is_enabled) ? 'checked' : '' }}
                    ...
                    <div>
                        ...
                        <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                            Feature พร้อมใช้งานสำหรับ Tenants
                        </p>
                    </div>
                </div>
                ...
                <div class="flex items-center gap-3 p-4 bg-gray-50 dark:bg-gray-700/50 rounded-xl">
                    ...
                    <input type="checkbox"
                           name="is_visible"
                           value="1"
                           {{ old('is_visible', $feature->is_visible) ? 'checked' : '' }}
                           class="w-5 h-5 text-blue-600 bg-white dark:bg-gray-700 border-gray-300 dark:border-gray-600 rounded focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400">
                    <div>
                        <label class="text-sm font-semibold text-gray-700 dark:text-gray-300">
                ...
                <div class="flex items-center gap-3 p-4 bg-gray-50 dark:bg-gray-700/50 rounded-xl">
                    ...
                    <input type="checkbox"
                           ...
                           value="1"
                           {{ old('requires_approval', $feature->requires_approval) ? 'checked' : '' }}
                           class="w-5 h-5 text-blue-600 bg-white dark:bg-gray-700 border-gray-300 dark:border-gray-600 rounded focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400">
                    <div>
                        <label class="text-sm font-semibold text-gray-700 dark:text-gray-300">
...
function featureEdit() {
    return {
        quotaType: '{{ old('quota_type', $feature->quota_type) }}',
        pricingTier: '{{ old('pricing_tier', $feature->pricing_tier) }}'
    }
}
</script>
@endpush
