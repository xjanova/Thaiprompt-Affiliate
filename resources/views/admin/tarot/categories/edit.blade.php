...
<div class="container mx-auto px-4 py-8 max-w-4xl">
    ...
    <form action="{{ route('admin.tarot.categories.update', $category->id) }}" method="POST" class="bg-white rounded-lg shadow p-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-2 gap-6 mb-6">
            <div>
                ...
                <input type="text" name="name_th" value="{{ $category->name_th }}" required class="w-full rounded-lg border-gray-300">
            </div>
            <div>
                <label class="block text-sm font-medium mb-2">ชื่อภาษาอังกฤษ *</label>
                <input type="text" name="name_en" value="{{ $category->name_en }}" required class="w-full rounded-lg border-gray-300">
            </div>
        </div>
        ...
        <div class="grid grid-cols-3 gap-6 mb-6">
            <div>
                ...
                <input type="text" name="icon" value="{{ $category->icon }}" class="w-full rounded-lg border-gray-300">
            </div>
            <div>
                <label class="block text-sm font-medium mb-2">สี</label>
                <input type="color" name="color" value="{{ $category->color }}" class="w-full h-10 rounded-lg border-gray-300">
            </div>
            <div>
                <label class="block text-sm font-medium mb-2">ราคา</label>
                <input type="number" name="price" step="0.01" min="0" value="{{ $category->price }}" required class="w-full rounded-lg border-gray-300">
            </div>
        </div>
        ...
        <div class="flex gap-4">
            ...
            <a href="{{ route('admin.tarot.categories.index') }}" class="bg-gray-300 hover:bg-gray-400 px-6 py-2 rounded-lg">ยกเลิก</a>
        </div>
    </form>
</div>
@endsection
