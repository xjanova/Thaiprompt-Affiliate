...
<div class="container mx-auto px-4 py-8 max-w-7xl">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            ...
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <div class="flex items-center">
                        <p class="text-3xl font-bold text-gray-800 dark:text-white mr-2">{{ number_format($stats['average_rating'] ?? 0, 1) }}</p>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 border-l-4 border-cyan-500 hover:shadow-xl transition-all duration-200 hover:-translate-y-1">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-gray-500 dark:text-gray-400 text-sm font-medium mb-1">Total Ratings</p>
                    <p class="text-3xl font-bold text-gray-800 dark:text-white">{{ number_format($stats['total_ratings'] ?? 0) }}</p>
                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Total feedback received</p>
                </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 border-l-4 border-green-500 hover:shadow-xl transition-all duration-200 hover:-translate-y-1">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-gray-500 dark:text-gray-400 text-sm font-medium mb-1">Response Speed</p>
                    <p class="text-3xl font-bold text-gray-800 dark:text-white">{{ number_format($stats['average_response_speed'] ?? 0, 1) }}</p>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 border-l-4 border-blue-500 hover:shadow-xl transition-all duration-200 hover:-translate-y-1">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-gray-500 dark:text-gray-400 text-sm font-medium mb-1">Solution Quality</p>
                    <p class="text-3xl font-bold text-gray-800 dark:text-white">{{ number_format($stats['average_solution_quality'] ?? 0, 1) }}</p>
                    <div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2 mt-2">
                    ...
                    @forelse($ratings as $rating)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors duration-150">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    <div class="text-yellow-500 text-lg mr-2">
                                        @for($i = 1; $i <= 5; $i++)
                                            <i class="fas fa-star{{ $i <= $rating->rating ? '' : ' text-gray-300 dark:text-gray-600' }}"></i>
                                        @endfor
                                    </div>
                                    <span class="text-sm font-semibold text-gray-700 dark:text-gray-300">{{ $rating->rating }}/5</span>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <td class="px-6 py-4">
                                ...
                                @else
                                    <span class="text-sm text-gray-400 dark:text-gray-500 italic">No feedback</span>
                                @endif
                            </td>
