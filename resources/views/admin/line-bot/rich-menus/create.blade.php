...
<div class="container-fluid px-4 py-6">
    <div class="mb-6">
        <div class="flex items-center gap-3">
            <a href="{{ route('admin.line-bot.rich-menu.index') }}" class="text-gray-600 hover:text-gray-900">
            ...
            <div>
                ...
                <p class="text-sm text-gray-600">Design interactive menu for LINE chat</p>
            </div>
        </div>
    </div>
    ...
    <form method="POST" action="{{ isset($richMenu) ? route('admin.line-bot.rich-menu.update', $richMenu->id) : route('admin.line-bot.rich-menu.store') }}" enctype="multipart/form-data">
            ...
            <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="text" name="name" value="{{ old('name', $richMenu->name ?? '') }}" required
                                class="w-full px-4 py-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-purple-500">
                        </div>

                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-2">Chat Bar Text *</label>
                            <input type="text" name="chat_bar_text" value="{{ old('chat_bar_text', $richMenu->chat_bar_text ?? 'Menu') }}" required maxlength="14"
                                class="w-full px-4 py-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-purple-500">
                            <p class="text-xs text-gray-500 mt-1">Text shown on chat bar (max 14 characters)</p>
                        </div>

                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-2">Menu Size</label>
                            <select name="size" class="w-full px-4 py-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-purple-500">
                        ...
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-2">Menu Image *</label>
                            <input type="file" name="menu_image" accept="image/*"
                                class="w-full px-4 py-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-purple-500"
                                {{ isset($richMenu) ? '' : 'required' }}>
                            <p class="text-xs text-gray-500 mt-1">Upload menu template image (2500x1686px or 2500x843px)</p>
                            @isset($richMenu)
                                @if($richMenu->menu_image_url)
                                    <img src="{{ $richMenu->menu_image_url }}" alt="Current menu" class="mt-2 max-w-xs rounded-lg border">
                                @endif
                            @endisset
                        </div>
                    </div>
                ...
                <div class="bg-white rounded-2xl shadow-lg p-6">
                    ...
                    <textarea name="menu_data" rows="15"
                        class="w-full px-4 py-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-purple-500 font-mono text-sm"
            ...
            <div class="lg:col-span-1">
                <div class="bg-gradient-to-br from-purple-50 to-pink-50 rounded-2xl border border-purple-200 p-6 sticky top-6">
                    ...
                    <a href="https://developers.line.biz/en/docs/messaging-api/using-rich-menus/"
                       target="_blank"
                       class="mt-4 block w-full px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition text-center text-sm">
        ...
        <div class="mt-8 bg-white rounded-2xl shadow-lg p-6">
            <div class="flex items-center justify-between">
                <a href="{{ route('admin.line-bot.rich-menu.index') }}" class="text-gray-600 hover:text-gray-900 text-sm">
