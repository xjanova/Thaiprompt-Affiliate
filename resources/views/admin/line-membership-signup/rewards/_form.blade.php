...
<div class="space-y-6">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                    เงื่อนไขผู้แนะนำ <span class="text-red-500">*</span>
                </label>
                <select name="referrer_requirement" id="referrer_requirement"
                        class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-blue-500"
                        required>
                    <option value="any" {{ old('referrer_requirement', $reward->referrer_requirement ?? 'any') == 'any' ? 'selected' : '' }}>
                        ไม่กำหนด (ทุกคนได้รับ)
                    </option>
                    <option value="required" {{ old('referrer_requirement', $reward->referrer_requirement ?? '') == 'required' ? 'selected' : '' }}>
                        ต้องมีผู้แนะนำ
                    </option>
                    <option value="none" {{ old('referrer_requirement', $reward->referrer_requirement ?? '') == 'none' ? 'selected' : '' }}>
                        ไม่มีผู้แนะนำ (สมัครเอง)
                    </option>
                </select>
                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                    กำหนดว่าผู้สมัครต้องมีผู้แนะนำหรือไม่จึงจะได้รับรางวัลนี้
                </p>
                @error('referrer_requirement')
                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                @enderror
            </div>
