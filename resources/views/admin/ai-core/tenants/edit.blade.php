...
@section('title', 'แก้ไข Tenant: ' . $tenant->tenant_name)
...
<div class="container-fluid px-4 py-6">
    ...
    <div class="mb-6">
        ...
        <p class="text-gray-600 dark:text-gray-400 ml-13">
            แก้ไขข้อมูล Tenant: <strong>{{ $tenant->tenant_name }}</strong>
        </p>
    </div>

    <form method="POST" action="{{ route('admin.ai-core.tenants.update', $tenant) }}" class="space-y-6">
        @csrf
        @method('PUT')
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text"
                           ...
                           disabled
                           class="w-full px-4 py-3 bg-gray-100 dark:bg-gray-900 border border-gray-300 dark:border-gray-600 rounded-xl text-gray-600 dark:text-gray-400 cursor-not-allowed">
                    <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                        ⚠️ ไม่สามารถแก้ไข Tenant Key ได้
                    </p>
                </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    <select name="subscription_tier"
                        ...
                        <option value="free" {{ old('subscription_tier', $tenant->subscription_tier) == 'free' ? 'selected' : '' }}>Free</option>
                        <option value="basic" {{ old('subscription_tier', $tenant->subscription_tier) == 'basic' ? 'selected' : '' }}>Basic</option>
                        <option value="pro" {{ old('subscription_tier', $tenant->subscription_tier) == 'pro' ? 'selected' : '' }}>Pro</option>
                        <option value="enterprise" {{ old('subscription_tier', $tenant->subscription_tier) == 'enterprise' ? 'selected' : '' }}>Enterprise</option>
                    </select>
                ...
                <div>
                    ...
                    <input type="datetime-local"
                           ...
                           value="{{ old('subscription_starts_at', $tenant->subscription_starts_at ? $tenant->subscription_starts_at->format('Y-m-d\TH:i') : '') }}"
            ...
            <div class="space-y-4">
                ...
                <div class="flex items-center gap-3 p-4 bg-gray-50 dark:bg-gray-700/50 rounded-xl">
                    ...
                    <input type="checkbox"
                           name="is_enabled"
                           value="1"
                           {{ old('is_enabled', $tenant->is_enabled) ? 'checked' : '' }}
                ...
                <div class="flex items-center gap-3 p-4 bg-gray-50 dark:bg-gray-700/50 rounded-xl">
                    ...
                    <input type="checkbox"
                           name="allow_trial"
                           value="1"
                           {{ old('allow_trial', $tenant->allow_trial) ? 'checked' : '' }}
                           class="w-5 h-5 text-green-600 bg-white dark:bg-gray-700 border-gray-300 dark:border-gray-600 rounded focus:ring-2 focus:ring-green-500 dark:focus:ring-green-400">
                    <div>
                        <label class="text-sm font-semibold text-gray-700 dark:text-gray-300">
                            🎁 อนุญาต Trial Period
