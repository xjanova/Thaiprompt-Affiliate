...
trait QaSettleTrait
{
    ...
    /**
     * 🔢 (2026-09-26) ลูกค้า "กดปุ่มคำถามแนะนำ" = ตั้งใจถามข้อใหม่ ไม่ใช่กำลังเล่ายาว → ล้างสตรีค
     *
     * เคสจริง reading 13746 (FTU-260926-S8307 · Celtic 99฿) — owner: "บอทตอบทำนายช้ามาก"
     *   18:25:24 กดปุ่ม "2" ห่างจากคำตอบก่อนหน้า 57 วิ — คำตอบยาว 1,093 ตัว ระบบคิดว่าต้องอ่าน 120 วิ
     *   ⇒ นับว่า "อ่านไม่ทัน = ยังเล่าไม่จบ" → สตรีค +1 → รอ 50 วิก่อน AI เริ่มคิด ทุกข้อที่เหลือ
     *   กดปุ่มอีกครั้ง → สตรีคถึง 2 → สั่ง AI ตอบสั้นแบบรับฟัง ทั้งที่ลูกค้าถามคำถามจริง
     *   (log 7 วันย้อนหลัง: ข้อความถามตอบ 99 โดนหน้าต่าง 50 วิเกินครึ่ง)
     *
     * ปุ่มคำถามแนะนำคือคำถามครบในตัวที่ระบบเสนอเอง — กดแล้วแปลว่า "อยากได้คำตอบข้อนี้" ไม่ใช่เล่าต่อ
     * ⇒ ห้ามนับเป็นสตรีค และล้างสตรีคเดิมทิ้ง → ใช้หน้าต่างรอพื้นฐาน + ได้คำทำนายเต็ม
     * ถ้าลูกค้ากลับไปพิมพ์รัวอีก qaTrackRamble() จะนับใหม่จาก 0 ตามปกติ
     */
    public function qaResetRamble(FortuneReading $reading): void
    {
        try {
            if ((int) ($reading->getConversationState('qa_ramble_streak', 0) ?: 0) > 0) {
                $reading->setConversationState('qa_ramble_streak', 0);
            }
        } catch (\Throwable $e) {
            // non-blocking — สถิติเพื่อจับจังหวะเท่านั้น ห้ามทำให้ flow ตอบคำถามล้ม
        }
