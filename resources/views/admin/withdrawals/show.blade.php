...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-gray-600 dark:text-gray-400 mt-1">รหัสคำขอ: {{ $withdrawal->request_id }}</p>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="glass-fusion rounded-xl shadow-lg p-6" hover:scale-105 transition-transform border border-white/20 dark:border-white/10>
                ...
                <div class="space-y-3">
                    <div class="flex justify-between items-center py-2">
                        <span class="text-gray-600 dark:text-gray-400">จำนวนที่ขอถอน</span>
                    ...
                    <div class="flex justify-between items-center py-2 border-t">
                        <span class="text-gray-600 dark:text-gray-400">ภาษี ({{ $withdrawal->tax_percentage }}%)</span>
                        <span class="text-lg font-semibold text-red-600">- {{ number_format($withdrawal->tax, 2) }} THB</span>
                    </div>
                    <div class="flex justify-between items-center py-3 border-t-2 border-gray-300 dark:border-gray-600 bg-green-50 -mx-6 px-6">
            ...
            <div class="glass-fusion rounded-xl shadow-lg p-6" hover:scale-105 transition-transform border border-white/20 dark:border-white/10>
                ...
                <div class="space-y-3">
                    <div>
                        <span class="text-sm text-gray-500 dark:text-gray-400">ชื่อ</span>
                    ...
                    <div>
                        <span class="text-sm text-gray-500 dark:text-gray-400">กระเป๋าเงิน</span>
            ...
            <div class="glass-fusion rounded-xl shadow-lg p-6" hover:scale-105 transition-transform border border-white/20 dark:border-white/10>
                ...
                <div class="bg-gradient-to-r from-indigo-50 to-purple-50 rounded-xl p-4 space-y-2">
                    <div>
            ...
            <div class="glass-fusion rounded-xl shadow-lg p-6" hover:scale-105 transition-transform border border-white/20 dark:border-white/10>
                ...
                <div class="bg-red-50 border-l-4 border-red-500 p-4">
                    ...
                    <p class="text-sm text-gray-600 dark:text-gray-400">ปฏิเสธโดย: {{ $withdrawal->rejectedBy->name }}</p>
                    <p class="text-xs text-gray-500 dark:text-gray-400">{{ $withdrawal->rejected_at->format('d/m/Y H:i') }}</p>
                    @endif
                </div>
            </div>
            @endif
            ...
            <div class="glass-fusion rounded-xl shadow-lg p-6" hover:scale-105 transition-transform border border-white/20 dark:border-white/10>
                ...
                <div class="mb-4">
                    <span class="text-sm text-gray-600 dark:text-gray-400 block mb-2">สลิปการโอน</span>
                    <img src="{{ asset($withdrawal->transfer_slip) }}" alt="Transfer Slip" class="max-w-md rounded-xl shadow-md">
                </div>
                @endif
                @if($withdrawal->transfer_note)
                <div class="bg-green-50 border-l-4 border-green-500 p-4">
                    <p class="text-sm text-gray-600 dark:text-gray-400 mb-1">หมายเหตุจากแอดมิน</p>
        ...
        <div class="space-y-6">
                ...
                <div class="space-y-4">
                    <div class="flex items-start space-x-3">
                        ...
                        <div class="flex-1">
                            ...
                            <p class="text-xs text-gray-500 dark:text-gray-400">{{ $withdrawal->created_at->format('d/m/Y H:i') }}</p>
                        </div>
                    </div>
                    ...
                    <div class="flex items-start space-x-3">
                        ...
                        <div class="flex-1">
                            ...
                            <p class="text-xs text-gray-500 dark:text-gray-400">{{ $withdrawal->approved_at->format('d/m/Y H:i') }}</p>
                            @if($withdrawal->approvedBy)
                            <p class="text-xs text-gray-600 dark:text-gray-400">โดย: {{ $withdrawal->approvedBy->name }}</p>
                            @endif
                        </div>
                    </div>
                    @endif
                    ...
                    <div class="flex items-start space-x-3">
                        ...
                        <div class="flex-1">
                            ...
                            <p class="text-xs text-gray-500 dark:text-gray-400">{{ $withdrawal->transfer_completed_at->format('d/m/Y H:i') }}</p>
                        </div>
                    </div>
                    @endif
                    ...
                    <div class="flex items-start space-x-3">
                        ...
                        <div class="flex-1">
                            ...
                            <p class="text-xs text-gray-500 dark:text-gray-400">{{ $withdrawal->rejected_at->format('d/m/Y H:i') }}</p>
                            @if($withdrawal->rejectedBy)
                            <p class="text-xs text-gray-600 dark:text-gray-400">โดย: {{ $withdrawal->rejectedBy->name }}</p>
                            @endif
                        </div>
                    </div>
                    @endif
            ...
            <div class="glass-fusion rounded-xl shadow-lg p-6 space-y-3" hover:scale-105 transition-transform border border-white/20 dark:border-white/10>
                ...
                <form action="{{ route('admin.withdrawals.approve', $withdrawal->id) }}" method="POST">
                    @csrf
                    <button type="submit" class="w-full bg-green-600 hover:bg-green-700 text-white font-semibold py-3 px-4 rounded-xl transition">
                        ✅ อนุมัติคำขอ
                    </button>
                </form>

                <button x-on:click="$dispatch('show-reject-modal')" class="w-full bg-red-600 hover:bg-red-700 text-white font-semibold py-3 px-4 rounded-xl transition">
                    ❌ ปฏิเสธคำขอ
                </button>
            </div>
            @endif
            ...
            <div class="glass-fusion rounded-xl shadow-lg p-6" hover:scale-105 transition-transform border border-white/20 dark:border-white/10>
                ...
                <form action="{{ route('admin.withdrawals.complete', $withdrawal->id) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                    @csrf
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">อัปโหลดสลิปการโอน <span class="text-red-500">*</span></label>
                        <input type="file" name="transfer_slip" accept="image/*" required class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-indigo-500">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">หมายเหตุ</label>
                        <textarea name="transfer_note" rows="3" class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-indigo-500" placeholder="หมายเหตุเพิ่มเติม (ถ้ามี)"></textarea>
                    </div>
                    <button type="submit" class="w-full bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 px-4 rounded-xl transition">
                        ✨ ยืนยันการโอนเงิน
                    </button>
                </form>
            </div>
...
<div x-data="{ showModal: false }"
    ...
    <div class="flex items-center justify-center min-h-screen px-4">
        ...
        <div class="relative glass-fusion rounded-xl shadow-2xl max-w-md w-full p-6" hover:scale-105 transition-transform border border-white/20 dark:border-white/10>
            <h3 class="text-xl font-bold text-gray-900 mb-4">ปฏิเสธคำขอถอนเงิน</h3>
            <p class="text-sm text-gray-600 dark:text-gray-400 mb-4">รหัสคำขอ: {{ $withdrawal->request_id }}</p>
            ...
            <form action="{{ route('admin.withdrawals.reject', $withdrawal->id) }}" method="POST">
                ...
                <div class="flex space-x-3">
                    <button type="button" x-on:click="showModal = false" class="flex-1 bg-gray-100/50 dark:bg-gray-800/50 hover:bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 font-semibold py-2 px-4 rounded-xl transition">
                        ยกเลิก
                    </button>
