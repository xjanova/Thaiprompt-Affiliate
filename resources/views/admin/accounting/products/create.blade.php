...
<div class="p-6">
    ...
    <div class="mb-6">
        <div class="flex items-center mb-4">
            ...
            <h1 class="text-3xl font-bold text-gray-900 dark:text-white">เพิ่มสินค้าและบริการ</h1>
        </div>
        <p class="text-gray-600 dark:text-gray-400">กรอกข้อมูลสินค้าหรือบริการใหม่</p>
    </div>

    <!-- Form -->
    <form action="{{ route('admin.accounting.products.store') }}" method="POST" class="max-w-4xl">
        @csrf
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6 space-y-6">
            ...
            <div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <label class="flex items-center p-4 border-2 rounded-lg cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700 {{ old('type', 'goods') === 'goods' ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20' : 'border-gray-300 dark:border-gray-600' }}">
                        <input type="radio" name="type" value="goods" {{ old('type', 'goods') === 'goods' ? 'checked' : '' }} class="mr-3">
                        <div>
                            <i class="fas fa-box text-blue-600 mr-2"></i>
                            <span class="font-medium text-gray-900 dark:text-white">สินค้า</span>
                        </div>
                    </label>
                    <label class="flex items-center p-4 border-2 rounded-lg cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700 {{ old('type') === 'service' ? 'border-purple-500 bg-purple-50 dark:bg-purple-900/20' : 'border-gray-300 dark:border-gray-600' }}">
                        <input type="radio" name="type" value="service" {{ old('type') === 'service' ? 'checked' : '' }} class="mr-3">
                        <div>
