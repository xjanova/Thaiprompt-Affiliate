...
@section('title', $pageTitle ?? 'แก้ไขเทมเพลต')
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl p-6 border border-gray-200 dark:border-gray-700">
                    ...
                    <div class="grid grid-cols-3 gap-4 text-center">
                        <div class="p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                            <p class="text-2xl font-bold text-purple-600 dark:text-purple-400">{{ number_format($template->usage_count) }}</p>
                            <p class="text-sm text-gray-500 dark:text-gray-400">ครั้งที่ใช้งาน</p>
                        </div>
                        <div class="p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                            <p class="text-2xl font-bold text-blue-600 dark:text-blue-400">{{ $template->projects_count ?? 0 }}</p>
                            <p class="text-sm text-gray-500 dark:text-gray-400">โปรเจกต์</p>
                        </div>
                        <div class="p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                            <p class="text-2xl font-bold text-green-600 dark:text-green-400">{{ $template->created_at->diffForHumans() }}</p>
            ...
            <div class="space-y-6">
                ...
                <div class="space-y-3">
                    ...
                    <button type="button" @click="deleteTemplate"
                        ...
                        <i class="fas fa-trash mr-2"></i> ลบเทมเพลต
                    </button>
                </div>
            </div>
        </div>
...
function templateForm() {
    return {
        form: {
            ...
            system_prompt: @json($template->system_prompt ?? ''),
            user_prompt_template: @json($template->user_prompt_template ?? ''),
            ...
            is_featured: {{ $template->is_featured ? 'true' : 'false' }},
        },
        models: @json($models),
        saving: false,
