...
<div class="min-h-screen bg-gray-50 dark:bg-slate-900 py-6 px-4 sm:px-6 lg:px-8" x-data="{
    ...
    <div class="mb-8">
        <div class="flex items-center gap-4 mb-4">
            <a href="{{ route('admin.line-bot.avatars.index') }}"
               class="flex items-center justify-center w-12 h-12 rounded-xl bg-white dark:bg-slate-800 shadow-lg hover:shadow-xl transition-all duration-300 group">
                <svg class="w-6 h-6 text-gray-600 dark:text-gray-300 group-hover:text-[#00B900] transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <!-- Toolbar -->
    <div class="mb-6 bg-white dark:bg-slate-800 rounded-2xl shadow-xl border border-gray-100 dark:border-slate-700 p-4">
        <div class="flex flex-col lg:flex-row items-center gap-4">
            ...
            <div class="flex-1 w-full">
                <div class="relative">
                    ...
                    <input type="text" x-model="searchQuery" placeholder="ค้นหา Avatar..."
                           class="w-full pl-10 pr-4 py-3 border-2 border-gray-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-gray-900 dark:text-white rounded-xl focus:ring-4 focus:ring-green-500/20 focus:border-[#00B900] transition-all duration-300">
                </div>
            </div>

            <!-- View Mode Toggle -->
            <div class="flex items-center gap-2 bg-gray-100 dark:bg-slate-700 rounded-xl p-1">
                <button @click="viewMode = 'grid'"
                        :class="viewMode === 'grid' ? 'bg-white dark:bg-slate-600 shadow-md' : 'text-gray-500 dark:text-gray-400'"
                ...
                <button @click="viewMode = 'list'"
                        :class="viewMode === 'list' ? 'bg-white dark:bg-slate-600 shadow-md' : 'text-gray-500 dark:text-gray-400'"
                        class="px-4 py-2 rounded-lg transition-all duration-300">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <!-- Select All -->
            <label class="flex items-center gap-3 cursor-pointer bg-gray-100 dark:bg-slate-700 px-4 py-3 rounded-xl hover:bg-gray-200 dark:hover:bg-slate-600 transition-all duration-300">
                <input type="checkbox" :checked="allSelected" @change="toggleAll()"
                       class="w-5 h-5 text-[#00B900] bg-gray-100 border-gray-300 rounded focus:ring-green-500">
            ...
            <div class="flex items-center justify-between">
                ...
                <div class="flex items-center gap-2">
                    <button @click="selectedAvatars = []"
                            class="px-4 py-2 bg-white dark:bg-slate-700 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-100 dark:hover:bg-slate-600 transition-all duration-300 font-bold flex items-center gap-2">
                    ...
                    <button onclick="if(confirm('คุณแน่ใจหรือไม่ที่จะลบ Avatar ที่เลือก?')) { alert('Bulk delete feature coming soon!') }"
                            class="px-4 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600 transition-all duration-300 font-bold flex items-center gap-2">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div x-show="viewMode === 'grid'" class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-4">
        @foreach($avatars ?? [] as $avatar)
            <div class="group bg-white dark:bg-slate-800 rounded-2xl shadow-lg border border-gray-100 dark:border-slate-700 overflow-hidden hover:shadow-2xl hover:scale-105 transition-all duration-300 relative">
                <!-- Selection Checkbox -->
                <div class="absolute top-2 left-2 z-10">
                    <label class="flex items-center justify-center w-8 h-8 rounded-full cursor-pointer"
                           :class="isSelected({{ $avatar->id }}) ? 'bg-gradient-to-r from-[#00B900] to-[#00E600]' : 'bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm'">
                        ...
                        <svg x-show="!isSelected({{ $avatar->id }})" class="w-5 h-5 text-gray-400 dark:text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <!-- List View -->
    <div x-show="viewMode === 'list'" x-cloak class="bg-white dark:bg-slate-800 rounded-2xl shadow-xl border border-gray-100 dark:border-slate-700 overflow-hidden">
                ...
                <thead class="bg-gradient-to-r from-[#00B900] to-[#00E600]">
                    <tr>
                        <th class="px-6 py-4 text-left">
                            <input type="checkbox" :checked="allSelected" @change="toggleAll()"
                                   class="w-5 h-5 text-white bg-white/20 border-white/30 rounded focus:ring-green-500">
                        </th>
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-slate-700">
                    @foreach($avatars ?? [] as $avatar)
                        <tr class="hover:bg-gray-50 dark:hover:bg-slate-700/50 transition-colors duration-200"
                            ...
                            <td class="px-6 py-4">
                                <div class="w-16 h-16 rounded-lg overflow-hidden bg-gray-100 dark:bg-slate-700 flex items-center justify-center">
                                    @if($avatar->type === 'image' || $avatar->type === 'gif')
                                        <img src="{{ $avatar->file_url ?? Storage::url($avatar->file_path) }}"
                                             alt="{{ $avatar->name }}"
                                             class="w-full h-full object-cover">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-2">
                                    ...
                                    @else
                                        <span class="px-3 py-1 bg-gray-100 dark:bg-gray-900 text-gray-600 dark:text-gray-400 rounded-full text-xs font-bold">
                                            ปิดใช้งาน
                                        </span>
                                    @endif
                                    @if($avatar->is_default)
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-center gap-2">
                                    <a href="{{ route('admin.line-bot.avatars.edit', $avatar->id) }}"
                                       class="px-3 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-all duration-300">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
