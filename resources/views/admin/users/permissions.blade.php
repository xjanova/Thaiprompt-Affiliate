...
<div class="space-y-6">
    ...
    <div class="bg-gradient-to-r from-indigo-500 to-purple-600 rounded-lg shadow-lg p-6 text-white">
        <div class="flex items-center">
            <div class="h-16 w-16 bg-white rounded-full flex items-center justify-center text-indigo-600 text-2xl font-bold">
    ...
    <div class="bg-white rounded-lg shadow-md p-6">
        <div class="mb-6">
            ...
            <p class="text-gray-600 mt-1">กำหนดสิทธิ์การเข้าถึงฟีเจอร์ต่างๆ ของระบบ</p>
        </div>
        ...
        <form method="POST" action="{{ route('admin.users.permissions.update', $user) }}">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                @foreach($permissionGroups as $groupName => $permissions)
                    <div class="col-span-1">
                        <h4 class="text-lg font-semibold text-gray-800 mb-4 pb-2 border-b-2 border-indigo-200">
                            {{ $groupName }}
                        </h4>
                        <div class="space-y-4">
                            @foreach($permissions as $permission => $info)
                                <div class="bg-gray-50 rounded-lg p-4 hover:bg-gray-100 transition {{ $isDisabled ? 'opacity-75' : '' }}">
                                    <div class="flex items-start">
                                        <div class="flex items-center h-5">
                                            <input type="checkbox"
                                                   ...
                                                   {{ $isDisabled ? 'disabled' : '' }}
                                                   class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500 h-5 w-5 {{ $isDisabled ? 'cursor-not-allowed' : 'cursor-pointer' }}">
                                        </div>
            ...
            @if(!$user->is_super_admin)
                <div class="mt-8 flex gap-3">
                    <button type="submit" class="px-6 py-3 bg-indigo-600 text-white font-semibold rounded-lg hover:bg-indigo-700 transition shadow-md hover:shadow-lg">
                        💾 บันทึกการตั้งค่า
                    </button>
                    <a href="{{ route('admin.users.index') }}" class="px-6 py-3 bg-gray-200 text-gray-700 font-semibold rounded-lg hover:bg-gray-300 transition shadow-md hover:shadow-lg">
                        ❌ ยกเลิก
                    </a>
                </div>
            @else
                <div class="mt-8">
                    <a href="{{ route('admin.users.index') }}" class="px-6 py-3 bg-gray-200 text-gray-700 font-semibold rounded-lg hover:bg-gray-300 transition shadow-md hover:shadow-lg inline-block">
    ...
    <div class="bg-white rounded-lg shadow-md p-6">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div class="bg-gray-50 border border-gray-200 rounded-lg p-4 text-center">
                <div class="text-3xl font-bold text-gray-600">{{ $totalPermissions }}</div>
                <div class="text-sm text-gray-800 mt-1">สิทธิ์ทั้งหมด</div>
            </div>
