...
<div class="container-fluid px-4 py-6 max-w-4xl mx-auto">
    ...
    <div class="mb-8">
        ...
        <h1 class="text-3xl font-bold text-gray-900 dark:text-white">สร้าง Keyword ใหม่</h1>
        <p class="text-gray-600 dark:text-gray-400 mt-2">เพิ่ม Keyword ใหม่ให้กับระบบ Hybrid Bot</p>
    </div>

    {{-- Form --}}
    <form method="POST" action="{{ route('admin.line-bot.keywords.store') }}" class="space-y-8">
        @csrf
        ...
        <div class="glass-fusion rounded-2xl shadow-lg border border-white/20 dark:border-white/10 p-8">
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <select name="category" class="w-full px-4 py-3 rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:outline-none focus:ring-2 focus:ring-purple-500 @error('category') border-red-500 @enderror" required>
                ...
                <div>
                    ...
                    <div class="flex items-center gap-4">
                        <input type="range" name="priority" min="1" max="100" value="{{ old('priority', 50) }}"
                        ...
                        <span class="text-lg font-bold text-gray-900 dark:text-white min-w-[50px] text-right" id="priorityValue">{{ old('priority', 50) }}</span>
                    </div>
        ...
        <div class="glass-fusion rounded-2xl shadow-lg border border-white/20 dark:border-white/10 p-8">
            ...
            <textarea name="trigger_words" placeholder="refund, คืนเงิน, return, การคืนเงิน"
                class="w-full px-4 py-3 rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:outline-none focus:ring-2 focus:ring-purple-500 @error('trigger_words') border-red-500 @enderror"
                rows="4" required>{{ old('trigger_words') }}</textarea>
            @error('trigger_words')
                <p class="text-red-600 dark:text-red-400 text-sm mt-2"><i class="fas fa-exclamation-circle mr-1"></i>{{ $message }}</p>
            @enderror
            <p class="text-gray-500 dark:text-gray-400 text-sm mt-2">🔍 ใช้ทั้งภาษาอังกฤษและไทย เพื่อให้มีการจับคู่ที่ดีขึ้น</p>
        </div>
        ...
        <div class="glass-fusion rounded-2xl shadow-lg border border-white/20 dark:border-white/10 p-8">
            ...
            <div class="space-y-4 mb-6">
                <label class="flex items-center p-4 border-2 border-gray-300 dark:border-gray-600 rounded-lg cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700/50 transition @if(old('response_type') === 'text' || !old('response_type')) border-purple-600 bg-purple-50 dark:bg-purple-500/10 @endif">
                    <input type="radio" name="response_type" value="text" class="w-4 h-4" onchange="switchResponseType()" @checked(!old('response_type') || old('response_type') === 'text')>
                    <span class="ml-3">
                ...
                <label class="flex items-center p-4 border-2 border-gray-300 dark:border-gray-600 rounded-lg cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700/50 transition @if(old('response_type') === 'quick_reply') border-purple-600 bg-purple-50 dark:bg-purple-500/10 @endif">
                    <input type="radio" name="response_type" value="quick_reply" class="w-4 h-4" onchange="switchResponseType()">
                    <span class="ml-3">
                ...
                <label class="flex items-center p-4 border-2 border-gray-300 dark:border-gray-600 rounded-lg cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700/50 transition @if(old('response_type') === 'flex_message') border-purple-600 bg-purple-50 dark:bg-purple-500/10 @endif">
                    <input type="radio" name="response_type" value="flex_message" class="w-4 h-4" onchange="switchResponseType()">
                    <span class="ml-3">
        ...
        <div id="responseContent" class="glass-fusion rounded-2xl shadow-lg border border-white/20 dark:border-white/10 p-8">
            ...
            <div id="textResponseSection" class="space-y-6">
                ...
                <textarea name="response_text" placeholder="ตอบได้อย่างนี้..."
                    class="w-full px-4 py-3 rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:outline-none focus:ring-2 focus:ring-purple-500"
                    rows="5">{{ old('response_text') }}</textarea>
            ...
            <div id="flexResponseSection" class="hidden space-y-6">
                ...
                <textarea name="response_flex_json" placeholder='{"type":"bubble","body":{"type":"box","layout":"vertical","contents":[]}}'
                    ...
                    rows="10">{{ old('response_flex_json') }}</textarea>
                <p class="text-gray-500 dark:text-gray-400 text-sm">📖 <a href="https://developers.line.biz/en/docs/messaging-api/using-flex-messages/" target="_blank" class="text-purple-600 hover:underline">ดู Flex Message Documentation</a></p>
            </div>
        </div>
