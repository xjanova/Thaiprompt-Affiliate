...
class CelticCrossService
{
    ...
    protected function buildCardFirstMandate(bool $lean = false, bool $withAstro = false): string
    {
        ...
        return "━━━━━━━━━━━━━━━━━\n"
            ...
            .($withAstro
                ? "ไพ่ 10 ใบ × ตำแหน่ง *และ* ผังดวงจริงที่แนบมา (ลัคนา/ภพ/ดาวเจ้าชนะ/ดาวจร) = แหล่งความจริง *ร่วมกัน* น้ำหนัก *100%* ของทุกประโยค\n"
                    ."(ทั้งสองอย่างเป็นของจริงที่คำนวณมาแล้ว — ห้ามเลือกใช้อย่างเดียวแล้วทิ้งอีกอย่าง ดูวิธีอ่านร่วมในบล็อก 🧬 ด้านล่าง)\n\n"
            ...
            .($withAstro
                ? "🧭 *ลำดับความสำคัญ*: (1) อ่านไพ่ + ตำแหน่ง *ร่วมกับ* ผังดวง = แกน 100% → (2) คำแนะนำ/ทางออก/มู/ธรรมะ\n"
                    ."   เป็น \"ผลที่งอกจากไพ่+ดวง\" เท่านั้น — จะแนะนำอะไร ต้องชี้ได้ว่าไพ่ใบไหน/ดาวดวงไหนทำให้แนะแบบนั้น\n"
    ...
    protected function buildFusionDoctrine(bool $lean = false): string
    {
        if ($lean) {
            return "━━━━━━━━━━━━━━━━━\n"
                ."🧬 อ่านร่วม 3 ชั้น — ไพ่ × ผังดวง × ดาวจร (บทสรุปนี้ต้องเป็นเนื้อเดียวกัน)\n"
                ."━━━━━━━━━━━━━━━━━\n"
                ."• 🃏 ไพ่ = *อะไรกำลังเกิด* · 🌀 ผังดวง (ลัคนา/ภพ/ดาวเจ้าชนะ) = *ทำไมถึงเป็นแบบนี้* · 🔭 ดาวจร/ดาวเสวยอายุ = *เมื่อไหร่*\n"
                ."• ทุกคำฟันธงต้องยืนบน *อย่างน้อย 2 ชั้น* — และเหตุผลต้องส่งต่อกัน (พื้นดวงเป็นแบบนี้ ⇒ จึงเจอเรื่องที่ไพ่ชี้ ⇒ ดาวจรบอกว่าสุกงอมช่วงไหน)\n"
                ."• ❌ ห้ามแยกเป็น \"ย่อหน้าดวง\" แล้ว \"ย่อหน้าไพ่\" · ❌ ห้ามฟันธงช่วงเวลาโดยไม่มีดาวจร/ดาวเสวยอายุรองรับ\n"
                ."• ⚖️ ไพ่กับดวงขัดกัน = เรื่องปกติ ห้ามเงียบ → บอกว่าเป็น \"แรงดึงสองทาง\" ชั้นไหนแรงกว่าเพราะอะไร แล้วฟันธง *คำตอบเดียว*\n"
                ."━━━━━━━━━━━━━━━━━\n\n";
        }

        return "━━━━━━━━━━━━━━━━━\n"
            ."🧬 อ่านร่วม 3 ชั้น — ไพ่ × ผังดวง × ดาวจร ต้องออกมาเป็น \"เนื้อเดียวกัน\"\n"
            ."━━━━━━━━━━━━━━━━━\n"
            ...
            ."📐 *หน้าที่ของแต่ละชั้น* (ห้ามสลับ ห้ามพูดซ้ำเรื่องเดียวกันคนละที่):\n"
            ."• 🃏 ไพ่ 10 ใบ = *อะไรกำลังเกิด* — เหตุการณ์ ตัวละคร อารมณ์ สภาพตอนนี้ และผลลัพธ์วิ่งไปทางไหน\n"
            ."• 🌀 ผังดวง (ลัคนา/ภพ/ดาวเจ้าชนะ/ทักษา) = *ทำไมถึงเป็นแบบนี้* — โครงสร้างพื้นดวง นิสัย จุดแข็ง-จุดอ่อน สนามชีวิตที่หนักที่สุด\n"
            ."• 🔭 ดาวจร + ดาวเสวยอายุ/ดวงปีนี้ = *เมื่อไหร่* — นาฬิกาของเรื่อง ใช้ผูกช่วงเวลาแทนการเดาเอง\n\n"

            ."✅ *วิธีเขียนให้เป็นเนื้อเดียว (บังคับ)*:\n"
            ."• ทุกคำฟันธงต้องยืนบน *อย่างน้อย 2 ชั้น* — ในใจต้องตอบได้ว่า \"ไพ่ใบไหน + ดาว/ภพไหน + ทำไมสองอย่างนี้ชี้เรื่องเดียวกัน\"\n"
            ."• เหตุผลต้อง *ส่งต่อกันเป็นลูกโซ่*: พื้นดวงบอกว่าลูกเป็นคนแบบไหน ⇒ จึงมักเจอเหตุการณ์แบบที่ไพ่ชี้ ⇒ ดาวจรบอกว่าเรื่องจะสุกงอมช่วงไหน\n"
            ."  ตัวอย่างโทน: \"ลัคนาลูกทำให้ยอมคนง่ายในเรื่องคู่ (ภพปัตนิหนัก) — ไพ่ตำแหน่งอุปสรรคที่กลับหัวก็ชี้ตรงกันว่ารอบนี้ลูกเป็นฝ่ายกลืนไว้ฝ่ายเดียว\n"
            ."   และดาวจรกำลังทับภพนั้นอยู่ ⇒ เรื่องจะแตกให้เห็นภายใน 1-2 เดือนนี้ ไม่ลากยาวกว่านั้น\"\n"
            ."• ทุกด้าน/ทุกหัวข้อที่เขียน ต้องมีทั้งไพ่และดวงอยู่ในย่อหน้าเดียวกัน — ไม่ใช่ไพ่ล้วนบางหัวข้อ ดวงล้วนบางหัวข้อ\n\n"
            ...
            ."• ❌ ห้ามฟันธง \"ช่วงเวลา\" โดยไม่มีดาวจร/ดาวเสวยอายุ/ไพ่ตำแหน่งอนาคตรองรับ\n"
            ."• ❌ ห้ามอ้างดาว/ภพที่ไม่มีในผังด้านบน (ผังคำนวณมาแล้ว — อ่านจากของจริงเท่านั้น)\n\n"

            ."⚖️ *ถ้าไพ่กับดวงขัดกัน* (เกิดได้ปกติ — ห้ามเงียบ ห้ามเลือกข้างมั่ว):\n"
            ."• ไพ่ชนะเรื่อง \"สถานการณ์ตอนนี้ / คนรอบตัว / ผลระยะใกล้\" — ผังดวงชนะเรื่อง \"โครงสร้าง นิสัย แนวโน้มระยะยาว\"\n"
            ."• พูดออกมาตรงๆ ว่าเป็น *แรงดึงสองทาง* + บอกว่าชั้นไหนแรงกว่า *เพราะอะไร* แล้วฟันธงออกมาเป็น *คำตอบเดียว*\n"
            ."  เช่น \"พื้นดวงลูกมีดาวหนุนงานยาว แต่ไพ่ตำแหน่งอุปสรรคกลับหัวบอกว่า 3 เดือนนี้ยังติด ⇒ ฟันธงว่าไปได้ แต่ช้ากว่าที่หวัง ต้องข้ามด่านคนกลางก่อน\"\n"
    ...
    protected function buildCardTalkPolicy(?string $userQuestion = null): string
    {
        ...
        return "━━━━━━━━━━━━━━━━━\n"
            ...
            ."   ✅ \"มีคนใกล้ตัวกำลังคิดไม่ซื่อกับลูกอยู่ — คนที่พูดดีต่อหน้านั่นแหละ\"\n\n"
