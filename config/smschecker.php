...
return [
    ...
    */
    ...
    /*
    |--------------------------------------------------------------------------
    | ด่านถอด QR ในสลิปเอง ก่อนยิง SlipOK (Local QR pre-check)
    |--------------------------------------------------------------------------
    |
    | เลขอ้างอิงรายการ (transRef) ฝังอยู่ในตัว QR ของสลิปอยู่แล้ว → ถอดเองได้ฟรี
    | ถ้าเลขนั้นเคยถูกใช้ไปแล้ว (มีใน slip_verifications) ก็ตอบ "สลิปซ้ำ" ได้
    | โดยไม่ต้องเสียเครดิต SlipOK
    |
    | ใช้เฉพาะผลเชิงบวก (ถอดได้ + เลขตรง) — ถอดไม่ได้ = ปล่อยผ่านไปยิงตามปกติเสมอ
    | ตั้ง SLIPOK_LOCAL_QR_PRECHECK=false ใน .env เพื่อปิดฉุกเฉิน (สวิตช์ดับ)
    |
    | ⚡ ต้นทุน: เพิ่มเวลา ~0.1-0.45 วิ ต่อสลิป 1 ใบ (วัดจริงบนสกรีนช็อต 1080×2400
    |    หลังย่อภาพเหลือด้านยาว 1000px — ก่อนย่อใช้ 1.7-3.1 วิ)
    |
    */
    'local_qr_precheck' => env('SLIPOK_LOCAL_QR_PRECHECK', true),
