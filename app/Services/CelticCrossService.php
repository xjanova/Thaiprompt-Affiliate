...
class CelticCrossService
{
    ...
    public function askQuestion(FortuneReading $reading, string $userQuestion, bool $isAutoBaseChart = false): array
    {
        ...
        try {
            ...
            // 🧹 (2026-09-04 owner) ตัดอักษรจีน/ญี่ปุ่น/เกาหลี/รัสเซียที่โมเดลหลุดมา
            //   เลนนี้ไม่เคยผ่าน sanitizer ตัวไหนเลย ⇒ ของหลุดวิ่งถึงลูกค้าตรงๆ
            //   (เคส rd12214 seq2 จบประโยคด้วย "...หรือสุขภาพ在线a")
            $response = ThaiOutputSanitizer::stripForeignScript((string) $response, [
                'lane' => 'celtic_qa',
                'reading_id' => $reading->id,
                'sequence' => $sequence,
            ...
            $reading->refresh();
            // 💰 (2026-09-04) พื้นดวงเปิดตัวรอบที่ 2+ → ตอบได้ แต่ *ห้ามตัดโควตา*
            //   เคส FTU-260904-S1420: รอบแรกยิงตอนยังไม่มีวันเกิด (ไพ่ล้วน) → ลูกค้าพิมพ์วันเกิด
            //   ตามมาทีหลัง → เส้น "late birthdate" ยิงพื้นดวงใหม่พร้อมดวงดาว = ถูกต้องแล้ว
            //   แต่ลูกค้าถูกตัดโควตา 2 ข้อสำหรับ "พื้นดวงเปิดตัว" อย่างเดียว (used=2 ก่อนถามอะไรเลย)
            $reading->markCelticAnswered(
                $sequence,
                ! $isAutoBaseChart,
                ! ($isAutoBaseChart && $this->baseChartAlreadyDelivered($reading, $sequence))
            );
    ...
    /**
     * ⭐ (2026-09-13) สเปก "บล็อกคะแนนดวง" ท้ายส่วน 5 ด้านของพื้นดวงเปิดตัว (Celtic 99 ข้อแรก)
     *
     * เจ้าของสั่ง "ทำเลย" กับกล่องสรุปคะแนนดาว (แบบ ข ในตัวอย่างหน้าตา Messenger)
     * โมเดลเขียนเป็นตัวเลขล้วน "ความรัก: 4/5" — ฝั่ง FB ตัวแต่ง FortuneMessengerFormatter::renderScores()
     * แปลงเป็น "★★★★☆  ความรัก" · LINE/บันทึกหลังบ้านเห็นตัวเลขตรง ๆ (อ่านรู้เรื่องอยู่แล้ว)
     *
     * ⚠️ ห้ามใส่อีโมจิของหัวข้ออื่น (💞/💼/💰/🍀/🌿 …) ในสเปกนี้ — ด่าน must เช็ค str_contains ทั้งก้อน
     *    โมเดลลอกอีโมจิติดมา = ด่านผ่านทั้งที่เซคชั่นนั้นหายจริง (rule_must_gate_no_foreign_emoji_in_spec)
     *    ⇒ อ้างชื่อด้านเป็น "คำ" เท่านั้น · ⭐ ไม่อยู่ในรายการ must ของกลุ่มไหน
     * ⚠️ ชื่อด้าน 5 คำต้องตรงกับ FortuneMessengerFormatter::SCORE_AREAS (ตัวแปลงจับด้วยชื่อ)
     */
    public static function scoreSummarySpec(): string
    {
        return "⭐ สรุปคะแนนดวง — ต่อท้ายทุกด้านข้างบน (ก่อนบรรทัดชวนถาม) เขียนบล็อกนี้ตามรูปแบบนี้เป๊ะ บรรทัดละ 1 ด้าน:\n"
            ."⭐ สรุปคะแนนดวง\n"
            ."ความรัก: N/5\n"
            ."การงาน: N/5\n"
            ."การเงิน: N/5\n"
            ."โชคลาภ: N/5\n"
            ."สุขภาพ: N/5\n"
            ."   • N = เลขเต็ม 1-5 ที่ตรงกับที่ฟันธงไว้ในด้านนั้นข้างบน (แย่ = 1-2 · กลาง ๆ = 3 · ดี = 4 · ดีมาก = 5)\n"
            ."   • ❌ ห้ามให้คะแนนสูงทุกด้านเพื่อเอาใจ — ด้านที่ทำนายว่ามีเคราะห์ต้องได้คะแนนต่ำจริง\n"
            ."   • ❌ ห้ามเขียนคำอธิบายต่อท้ายคะแนน ❌ ห้ามเปลี่ยนชื่อด้าน/ลำดับ ❌ ห้ามใส่อีโมจิในบรรทัดคะแนน\n";
    }
    ...
    protected function generateBaseChartSectioned(FortuneReading $reading, array $cards, string $userQuestion): ?array
    {
        ...
        // 🃏 (2026-09-06 FTU-260906-K2263) เจ้าชะตาข้ามวันเกิด → ไม่มีผังดาวให้ผสาน
        //   ต้องเปลี่ยนคำสั่งทั้งชุด ไม่งั้นพรอมต์สั่งให้ "ผสานดวงดาว" ที่ไม่มีอยู่จริง
        //   ⇒ โมเดลมั่วราศี/ลัคนา หรือออกตัวว่าอ่านไม่ได้ทั้งที่ลูกค้าจ่าย 99 แล้ว
        $cardsOnly = ($birthAstroBlock === '');

        // 🕛 (2026-09-09) ผังใบนี้นับภพจากอะไร — ลัคนาจริง / จันทร์ลัคน์ / ไม่มีเลย
        //   ต้องรู้ *ก่อน* ประกอบพรอมต์ เพราะ must gate ต้องตัดสินว่าจะบังคับ 🕛 ไหม
        //   บังคับทั้งที่ผังไม่มีลัคนา = ด่านตกทุกครั้ง → repair → เผาโทเคน → สุดท้ายโมเดลมั่วให้
        //   (เหตุผลเดียวกับ $cardsOnly ที่ตัด 🕛 ออกตอนไม่มีวันเกิด)
        $lagnaBasis = 'none';
        if (! $cardsOnly) {
            try {
                $lagnaBasis = (new \App\Services\Fortune\ThaiAstrologyService)->resolveLagnaBasis(
                    $reading->birth_date?->format('Y-m-d'),
                    $reading->birthHourFloat()
                );
            } catch (\Throwable $e) {
                $lagnaBasis = 'none';
            }
        ...
        $shared = ($personaBlock !== '' ? $personaBlock."\n\n" : '')
            .($cardsOnly
                ? "คุณคือ \"{$brandName}\" — แม่หมอเซียนไพ่เซลติก กำลังอ่าน \"พื้นดวงเปิดตัว\" ครั้งแรกให้เจ้าชะตา\n"
                    ."🃏 เจ้าชะตา *ไม่สะดวกบอกวันเดือนปีเกิด* → รอบนี้อ่านจาก \"หน้าไพ่ทั้ง 10 ใบ + ตำแหน่ง Celtic\" ล้วน\n"
                    ."❌ ห้ามพูดถึง/เดา ราศี ปีนักษัตร ธาตุ ดาวเจ้าชนะ ดาวเสวยอายุ ลัคนา ภพ/เรือนชะตา — ไม่มีข้อมูล การเดาคือการโกหกลูกค้า\n"
                    ."❌ ห้ามออกตัว/บ่นว่าไม่มีวันเกิดเลยอ่านได้ไม่เต็มที่ — ไพ่ 10 ใบพอสำหรับพื้นดวง ฟันธงให้เต็มที่เหมือนปกติ\n\n"
            ...
            .$this->buildBaseChartRerunNotice($reading)
            .$this->buildCardFirstMandate(withAstro: $birthAstroBlock !== '')
            .$this->buildCurrentDateContext()
            .$this->buildCardTalkPolicy(null)
            .$this->buildCardNamingDirective()
            ."━━━━━━━━━━━━━━━━━\n🃏 ไพ่ 10 ใบ (Celtic Cross) ของเจ้าชะตา:\n━━━━━━━━━━━━━━━━━\n".$cardsText."\n\n"
            // 🧬 (2026-09-03) ผังดวง + หลักอ่านร่วม 3 ชั้น วางติดกันเสมอ — ให้ข้อมูลกับกติกาการใช้อยู่ที่เดียวกัน
            .($birthAstroBlock !== ''
                ? "━━━━━━━━━━━━━━━━━\n🌟 ดวงดาวจากวันเกิด (หลักเจ้าชนะ)\n━━━━━━━━━━━━━━━━━\n".$birthAstroBlock."\n\n"
                    .$this->buildFusionDoctrine()
                : '');
        ...
        if ($bmForced) {
            $groups = [
                ...
                [
                    ...
                    'spec' => "เจาะรายละเอียด \"ตกลงโดนอะไร ใครทำ เพราะอะไร\" (อ่านตามหน้าไพ่ × ตำแหน่ง Celtic เท่านั้น — ไม่ทำพาดหัวซ้ำ):\n"
                        ...
                        ."👤 ผู้น่าสงสัย — บอก \"ลักษณะ\" เท่านั้น (เพศ/ความสัมพันธ์ คนใกล้ตัว-คู่แข่ง-อดีตคนรัก-เพื่อนงาน / ช่วงวัย / ทิศที่มา) ❌ ห้ามมั่วชื่อจริง → ชวนลูกยืนยัน \"คนลักษณะนี้คิดออกไหมว่าใคร\"\n"
                        ...
                        ."⚖️ กรรมย้อนผู้ทำไหม — ตอบตามไพ่จริง ❌ ห้ามปั้นแช่งให้สะใจ ชี้ให้ลูกโฟกัส \"ป้องกัน+แก้ที่ตัวเอง\"\n"
                ...
                [
                    ...
                    'spec' => "สรุป \"ทางแก้ที่ทำได้จริง ไม่เปลืองเงิน ไม่งมงาย\" (เน้นทำเองก่อน • อ้างหลักครูบาอาจารย์/พระเกจิที่มีจริง):\n"
                        // 🪬 (2026-09-05 owner) บอก *ชื่อบท* ได้ — ❌ ห้ามพิมพ์ *ตัวบท* (โมเดลท่องขาด+ผิด)
                        ."🙏 ทำเองที่บ้านก่อน — สวดมนต์ (บอกได้แค่ *ชื่อบท* เช่น อิติปิโส / พาหุงมหากา แล้วให้ไปเปิดจากหนังสือสวดมนต์หรือขอจากพระ ❌ ห้ามพิมพ์ตัวบทให้) / แผ่เมตตา / ทำบุญอุทิศ / รักษาศีล / สมาธิ / น้ำมนต์\n"
                        ...
                        .'ปิดท้าย 1 บรรทัดสั้น: ชวนลูกถามเจาะเรื่องของ/การแก้ต่อได้เลย (❌ ไม่ใส่ รัก/งาน/เงิน/สุขภาพ ทั่วไป ❌ ไม่ใส่ ฤกษ์/สีมงคล/เลขมงคลทั่วไป)',
                ],
            ];
        } else {
            ...
            // 🕉️ (2026-09-08 owner) *"คนหนึ่งอาจเหมาะกับการบูชาหลายองค์เทพได้ แต่ต้องตามตำรา
            //    ไม่ใช่ตามไพ่ เดิมมันตามไพ่อย่างเดียว ถ้าไม่มีวันเกิดก็ใช้ไพ่แทน"*
            //
            //    เดิมองค์เทพมาจากคลังรายไพ่ล้วน ⇒ 4 องค์ (ลักษมี/พิฆเนศ/กวนอิม/ครุฑ) ครอบ 78%
            //    ของไพ่ทั้งสำรับ ลูกค้าจึงเห็นวนอยู่ไม่กี่องค์ · ตอนนี้ตำราวันเกิดเป็นตัวตัดสิน
            //    ไพ่เหลือหน้าที่บอกแค่ "รอบนี้ท่านหนุนด้านไหน"
            //    null = ไม่รู้วันเกิด → ตกไปใช้ไพ่ล้วนเหมือนเดิม (ห้ามเดาวันเกิด)
            $deityService = app(\App\Services\Fortune\PatronDeityDoctrine::class);
            $deityDoctrine = $deityEnabled ? $deityService->resolveForReading($reading) : null;
            // 🍀 (2026-09-03, owner) 'wealth' กลับเข้า g2 — เซคชั่น 🍀 โชคลาภ ต้องมีข้อเท็จจริงรายไพ่รองรับ
            //   เดิมตัดออกกัน g2 บวม แต่ตอนนั้น g2 ยังไม่มีหัวข้อที่ถามเรื่องลาภตรงๆ
            //   ไม่ฉีดคลัง = AI มโนเกณฑ์ลาภจากอากาศ (บทเรียนเดียวกับ [[rule_daily_horoscope_real_ephemeris]])
            //   ต้นทุนจริง ~10 บรรทัด (รายไพ่ 10 ใบ) = เท่าคลังความรักที่ฉีดอยู่แล้ว
            $g2Signal = $deityEnabled ? ['health', 'love', 'wealth', 'patron_deity'] : ['health', 'love', 'wealth'];
            $g2DeitySpec = $deityEnabled
                ? "🕉️ องค์เทพ/สิ่งศักดิ์สิทธิ์คุ้มครอง-บูชาประจำตัว (บล็อกสุดท้าย):\n"
                    .($deityDoctrine !== null
                        // ── มีวันเกิด → ตำราเป็นตัวตัดสิน (ไพ่เหลือหน้าที่บอกน้ำหนักของรอบนี้)
                        ? $deityService->knowledgeBlock($deityDoctrine)
                            ."• ยึด *องค์ประธาน* ด้านบนเป็นองค์ประจำตัวของลูก แล้วต่อด้วย *องค์เสริม* ที่ระบุไว้ —\n"
                            ."  บูชาได้หลายองค์พร้อมกัน ไม่ต้องบังคับเลือกองค์เดียว (บอกให้ชัดว่าองค์ไหนเป็นหลัก องค์ไหนเสริมด้านใด)\n"
                            ."• อธิบายที่มาสั้น ๆ *ตามตำรา* — เกิดวันไหน ดาวประจำวันอะไร พระประจำวันเกิดปางอะไร ธาตุอะไร\n"
                            ."  ❌ ห้ามบอกว่าเลือกองค์จากไพ่ ❌ ห้ามเปลี่ยนตัวองค์ตามหน้าไพ่\n"
                            ."• ไพ่ในสำรับใช้บอกได้แค่ *รอบนี้ท่านหนุนด้านไหนเป็นพิเศษ* (จริตเฉพาะรอบ) เท่านั้น\n"
                        // ── ไม่รู้วันเกิด → ไพ่ล้วนเหมือนเดิม + ชวนบอกวันเกิดเพื่อความแม่น
                        : "• เจ้าชะตายังไม่ได้บอกวันเกิด ⇒ รอบนี้อ่านองค์จาก \"คลังองค์เทพรายไพ่\" ด้านบน:\n"
                            ."  สรุปองค์ที่เด่นสุด 1 องค์เป็นองค์อุปถัมภ์ (เสริมได้อีกไม่เกิน 1 องค์)\n"
                            ."• ปิดท้ายบล็อกนี้ 1 บรรทัด: ถ้าบอก ว/ด/ป เกิด (ยิ่งมีเวลาเกิดยิ่งดี) แม่หมอจะชี้องค์ประจำตัว\n"
                            ."  *ตามตำราวันเกิด* ให้แม่นกว่านี้ได้\n")
                    ."• ทุกคนมีองค์อุปถัมภ์เสมอ ❌ ห้ามตอบ \"ไม่มี/แล้วแต่ศรัทธา\"\n"
                    ."• บอกระดับองค์ (มหาเทพ / เทพนพเคราะห์ / เทพชั้นรอง / ครูบา-สิ่งศักดิ์สิทธิ์สายไทย) + คุ้มครอง-หนุนดวงด้านไหน\n"
                    // 🪬 (2026-09-05 owner) เดิมสั่งให้คาย "คาถา/บทสวดเฉพาะองค์" → โมเดลท่องขาด+ผิด
                    //   เปลี่ยนเป็น: บอกชื่อบท + ที่ไปหาของจริง ❌ ห้ามพิมพ์ตัวบท
                    ."• บูชาเปิดดวงอย่างไร — ของบูชา + วันไหว้ + วิธีตั้งจิต *แยกรายองค์ที่แนะนำ* (ดึงจากข้อมูลด้านบน) ❌ ห้ามมั่วชื่อองค์ที่ไม่มีจริง ไม่แน่ใจ→แนะแนวทางกลางที่ถูกต้อง\n"
                    ."  🚫 *ตัวบทคาถา/บทสวดขององค์นั้น ห้ามพิมพ์ให้เด็ดขาด* — บอกได้แค่ชื่อบท แล้วให้ไปขอจากสำนัก/พระ/หนังสือของจริง (ท่องขาดคำ = ไม่ศักดิ์สิทธิ์)\n"
            ...
            $groups = [
                ...
                [
                    'label' => 'g1-foundation',
                    // 🕛 (2026-09-03, owner) +1 ส่วน (ลัคนา&เรือนชะตา) → ขยายเพดานความยาวตาม
                    // 🃏 (2026-09-06) เส้นไม่มีวันเกิด: ตัด 🕛 ออก → เหลือ 2 ส่วน ย่อเพดานตาม
                    'len' => ($cardsOnly || $lagnaBasis === 'none') ? '550-850' : '700-1050',
                    // ⚠️ '🕛' อยู่ใน must ด้วยโดยตั้งใจ — must คือด่านตรวจผลของโหมดคอลเดียว
                    //   ถ้าโมเดลข้ามเซคชั่นใหม่ ต้องตกไปโหมดแบ่งบล็อก (เปลืองโทเคนแต่ได้ครบ)
                    //   ไม่ใส่ = เซคชั่นใหม่กลายเป็น "ใส่ก็ได้ไม่ใส่ก็ได้" = ของที่สร้างแล้วไม่มีใครบังคับ
                    // 🃏 (2026-09-06) แต่ถ้าไม่มีวันเกิด **ห้ามใส่ 🕛 ใน must** — ลัคนา/เรือนชะตา
                    //   คำนวณไม่ได้เลย ⇒ ด่านจะตกทุกครั้ง → repair → แบ่งบล็อก → ยังตก = เผาโทเคน
                    //   แล้วบังคับให้โมเดล "เขียนลัคนาให้ได้" = มั่วราศีใส่ลูกค้า
                    // 🕛 (2026-09-09) ไม่มีทั้งลัคนาและจันทร์ลัคน์ (ไม่รู้เวลาเกิด + จันทร์ย้ายราศีวันนั้น)
                    //   ⇒ ตัด 🕛 ออกจาก must ด้วย — เหตุผลเดียวกับ $cardsOnly ทุกประการ
                    'must' => ($cardsOnly || $lagnaBasis === 'none') ? ['🌟', '🔮'] : ['🌟', '🔮', '🕛'],
                    'spec' => $cardsOnly
                        ? "เขียน 2 ส่วนนี้ (ขึ้นหัวข้อด้วย emoji ตามนี้เป๊ะ เว้นบรรทัดให้อ่านง่าย • พาดหัวเรื่องเด่นทำไปแล้วในส่วนแรก ไม่ต้องทำซ้ำ):\n"
                            ."🌟 พื้นฐานนิสัย&พลังประจำตัว — อ่านจาก *ไพ่บุคคล/ไพ่ที่สื่อบุคลิก* ในสำรับ (Court cards, ไพ่ตำแหน่ง 1 ตัวตน, ตำแหน่ง 7 มุมมองต่อตัวเอง)\n"
                            ."   นิสัยพื้นฐาน จุดแข็ง-จุดอ่อน สิ่งที่คนอื่นเห็น vs ตัวจริง — ระบุชื่อไพ่ที่ใช้อ่านทุกข้อ\n"
                            ."   ❌ ห้ามพูดถึง ราศี/ปีนักษัตร/ธาตุ/ดาวเจ้าชนะ — รอบนี้ไม่มีวันเกิด (พูดถึง = โกหกลูกค้า)\n"
                            ."🔮 ภาพรวมชีวิตช่วงนี้ — อ่านจากแกนเวลาของ Celtic: ตำแหน่ง 3/4 (อดีต) → 1/2/5 (ปัจจุบัน) → 6/9/10 (อนาคต)\n"
                            .'   เล่าเป็นเส้นเรื่องเดียวกัน อดีต→ปัจจุบัน→อนาคต ฟันธงว่ากำลังอยู่ช่วงไหนของวงจร'
                        // 🕛 (2026-09-09) จำนวนส่วนต้องตรงกับที่ต่อท้ายจริง — ไม่มีลัคนา/จันทร์ลัคน์
                        //   ⇒ lagnaSectionDirective() คืนค่าว่าง ⇒ เหลือ 2 ส่วน ถ้ายังเขียน "3 ส่วน"
                        //   โมเดลจะไปเค้นหาส่วนที่ 3 มาเติมเอง = มโนลัคนา (กับดักเดิมย้ายที่)
                        : 'เขียน '.($lagnaBasis === 'none' ? '2' : '3')." ส่วนนี้ (ขึ้นหัวข้อด้วย emoji ตามนี้เป๊ะ เว้นบรรทัดให้อ่านง่าย • พาดหัวเรื่องเด่นทำไปแล้วในส่วนแรก ไม่ต้องทำซ้ำ):\n"
                            ...
                            .\App\Services\Fortune\ThaiAstrologyService::lagnaSectionDirective($lagnaBasis),
                ],
                [
                    'label' => 'g2-areas',
                    // 🍀 (2026-09-03, owner) +1 ด้าน (โชคลาภ) → ขยายเพดานความยาวตาม
                    'len' => $deityEnabled ? '900-1250' : '750-1050',
                    // ⚠️ '🍀' อยู่ใน must ด้วยโดยตั้งใจ — must คือด่านตรวจผลของโหมดคอลเดียว
                    //   ถ้าโมเดลข้ามเซคชั่นใหม่ ต้องตกไปโหมดแบ่งบล็อก (เปลืองโทเคนแต่ได้ครบ)
                    //   ไม่ใส่ = เซคชั่นใหม่กลายเป็น "ใส่ก็ได้ไม่ใส่ก็ได้" (บทเรียนเดียวกับ 🕛 ลัคนา)
                    'must' => ['💞', '💼', '💰', '🍀', '🌿'], // องค์เทพไม่ใส่ — เป็น conditional (ผูก $deityEnabled)
                    'signal' => $g2Signal, // สุขภาพ(อวัยวะ)+ความรัก+การเงิน/โชคลาภ รายไพ่ (+องค์เทพ ถ้า gate เปิด)
                    ...
                    'spec' => 'เขียน 5 ด้านนี้'.($deityEnabled ? ' + ปิดท้ายด้วยองค์เทพประจำตัว' : '')
                        .' (ขึ้นหัวข้อด้วย emoji ตามนี้เป๊ะ • '
                        // 🃏 (2026-09-06) ไม่มีวันเกิด = ไม่มีภพให้ผูก → สั่งให้ผูกไพ่+ตำแหน่งแทน
                        .($cardsOnly
                            ? 'แต่ละด้านผูก *ไพ่+ตำแหน่ง Celtic* ที่เกี่ยวข้อง (❌ ห้ามอ้างดาว/ภพ — ไม่มีวันเกิด)'
                            : 'แต่ละด้านผูกดาว(ภพ)+ไพ่ที่เกี่ยวข้อง')
                        ." • ฟันธงชัด • ไม่ต้องทำพาดหัวเรื่องเด่นซ้ำ):\n"
                        ."💞 ความรัก\n"
                        ."💼 การงาน\n"
                        ."💰 การเงิน — เงินที่ \"หามาเอง\": รายรับ-รายจ่าย/หนี้สิน/เก็บออม-ลงทุน (ลาภลอยไม่ต้องพูดที่นี่ เก็บไว้หัวข้อโชคลาภ)\n"
                        // 🍀 (2026-09-03, owner) เซคชั่นใหม่ — Q1 เดิมมี 💰 การเงิน แต่ไม่เคยมี "โชคลาภ"
                        //   ทั้งที่ g0 ยกโชคลาภเป็นตัวอย่างเรื่องเด่น + บทสรุป VIP ก็มี 💰 การเงิน/โชคลาภ
                        //   ⇒ ลูกค้าจ่าย 99 แล้วพื้นดวงเปิดตัวไม่เคยฟันธงเรื่องลาภให้เลย
                        // ⚠️ อ้างถึงหัวข้ออื่นด้วย "คำ" เท่านั้น — ห้ามใส่อีโมจิของหัวข้ออื่น (💰/🌿/...) ในบรรทัดนี้:
                        //   ด่าน must เช็ค str_contains บนคำตอบทั้งก้อน ⇒ โมเดลลอกอีโมจินั้นติดมาเป็นข้อความ
                        //   = ด่านผ่านทั้งที่เซคชั่นนั้นหายไปจริง (ลูกค้าจ่ายแล้วหายเงียบ ไม่มีใครจับได้)
                        ."🍀 โชคลาภ — *คนละเรื่องกับหัวข้อการเงิน*: ลาภลอย/เงินก้อนนอกแผน/รางวัล/ของขวัญ/มรดก/เงินคืน/คนอุปถัมภ์ยื่นให้\n"
                        .($cardsOnly
                            ? "   • อ่านจาก *ไพ่ที่ชี้เรื่องลาภ* ในสำรับ + ตำแหน่ง 9 (ความหวัง/สิ่งที่กำลังมา) และ 10 (ผลลัพธ์) (+คลังการเงิน/โชคลาภ รายไพ่ ถ้ามีด้านบน)\n"
                            : "   • อ่านจาก *ภพศุภะ(9) + ภพลาภะ(11) + ดาวพฤหัสบดี* ในผังด้านบน ผสานไพ่ที่ชี้เรื่องลาภ (+คลังการเงิน/โชคลาภ รายไพ่ ถ้ามีด้านบน)\n")
                        ."   • ฟันธงให้ชัด: *มีเกณฑ์ลาภไหม • มาทางไหน (คน/งาน/มรดก/รางวัล/เงินคืน) • ช่วงไหน • ก้อนใหญ่หรือเล็ก*\n"
                        ."     — ไม่มีเกณฑ์ก็บอกตรงๆ ว่ารอบนี้ไม่มี แล้วชี้ทางหาเพิ่มด้วยมือตัวเอง ❌ ห้ามปั้นลาภปลอบใจ\n"
                        ."   • ⚠️ จรรยาบรรณ: ❌ ห้ามฟันเลขหวย/เลขเด็ด/หุ้น-คริปโตเป๊ะ ❌ ห้ามการันตีถูกรางวัล ❌ ห้ามยุให้กู้-ลงทุนเสี่ยงเพราะดวงดี\n"
                        ."     โชคลาภ = \"แนวโน้ม\" ที่ต้องลงมือจริงควบคู่ (วินัยการเงินเป็นหลัก)\n"
                        ."🌿 สุขภาพ\n"
                        // ⭐ (2026-09-13) ยกเว้นบล็อกคะแนนดวงไว้ในบรรทัดห้ามนี้ด้วย — ไม่งั้น "ห้ามบทสรุปรวม" ชนกับคำสั่งเขียนคะแนน
                        //   แล้วโมเดลทิ้งอันใดอันหนึ่ง (rule_conflicting_directives_model_drops_one)
                        ."❌ 5 ด้านบนนี้ ห้ามแปะ สีมงคล/เลขมงคล/ฤกษ์/วันมงคล/ทิศ/บทสรุปรวมแบบย่อหน้า/ธรรมะทิ้งท้าย — เก็บไว้บทสรุป VIP ตอนจบเท่านั้น (รอบนี้เพิ่งเปิดดวง)\n"
                        ."   (ยกเว้นบล็อก ⭐ คะแนนดวงข้างล่าง — เป็นตัวเลขล้วน ไม่ใช่บทสรุปรวม)\n"
                        .$g2DeitySpec
                        .self::scoreSummarySpec()
                        .'ปิดท้ายด้วย 1 บรรทัดสั้นเท่านั้น: ชวนลูกพิมพ์ถามเจาะลึกเรื่องที่อยากรู้ได้เลย (❌ ไม่สรุปยาว ❌ ไม่ใส่ธรรมะ/มงคลทั่วไป)',
                ],
            ];
     ...
     */
    protected function buildCardFirstMandate(bool $lean = false, bool $withAstro = false): string
    {
        // 🧬 (2026-09-03 owner) รอบที่มี "ผังดวงจริง" แนบมาด้วย → กฎเหล็กต้องพูดว่า
        //   "ไพ่ + ผังดวง" ไม่ใช่ "ไพ่เท่านั้น"
        //
        //   ⚠️ ทำไม: เดิมกฎนี้เขียนไว้ว่า "ไพ่ = แหล่งความจริงเดียว น้ำหนัก 100% ·
        //     ประโยคที่ตอบไม่ได้ว่ามาจากไพ่ใบไหน = ห้ามพูด" แล้ววางทับ $baseChartDirective
        //     ที่สั่งว่า "ใช้ดวงดาวเป็นแกนหลัก ผสานไพ่" ⇒ พรอมต์เดียวมี 2 หลักที่ขัดกันเอง
        //     โมเดลเลยเลือกทางที่ปลอดภัยกว่า = ทิ้งผังดวง อ่านไพ่ล้วน
        //     ⇒ ลูกค้าอ่านแล้วรู้สึกว่า "พื้นดวงกับไพ่ไม่เกี่ยวกัน" (owner ทักตรงจุดนี้)
        //   กฎกันคำลอยๆ ("ทุกอย่างอยู่ที่ใจเรา") ยังอยู่ครบ — แค่ขยายที่มาที่ถูกยอมรับ
        //   จาก "ไพ่" เป็น "ไพ่ หรือ ดาว/ภพ/ดาวจร ในผังที่แนบมา" (ยังห้ามมโนดาวที่ไม่มีในผัง)
        $originLine = $withAstro
            ? "• ในใจต้องตอบได้เสมอว่า \"ประโยคนี้มาจากไพ่ใบไหน/ตำแหน่งอะไร — หรือดาว/ภพ/ดาวจรดวงไหนในผัง\" — ตอบไม่ได้ = ห้ามพูด\n"
            : "• ในใจต้องตอบได้เสมอว่า \"ประโยคนี้มาจากไพ่ใบไหน ตำแหน่งอะไร\" — ตอบไม่ได้ = ห้ามพูด\n";

        if ($lean) {
            return "━━━━━━━━━━━━━━━━━\n"
                .($withAstro
                    ? "🃏 กฎเหล็ก — ทุกประโยคต้องงอกจาก \"ไพ่ 10 ใบ + ผังดวงจริง\" 100%\n"
        ...
        return "━━━━━━━━━━━━━━━━━\n"
            .($withAstro
                ? "🃏🌀 กฎเหล็กสูงสุด — ทำนายจาก \"หน้าไพ่ + ตำแหน่ง + ผังดวงจริง\" 100% (อ่านก่อนทุกกฎด้านล่าง)\n"
            ...
            .($withAstro
                // 🃏 (2026-09-04 owner) ไพ่ยังเป็น *แกน* — ผังดวงเป็นตัวประกอบที่อ้างอิงได้
                //   (รอบก่อนเขียนว่า "แหล่งความจริงร่วม" ⇒ ดวงกลบไพ่จนเป็นบทโหราศาสตร์ล้วน)
                ? "ไพ่ 10 ใบที่เปิด × ตำแหน่งของแต่ละใบ = *แกนของคำทำนาย* น้ำหนักหลักของทุกประโยค\n"
                    ."ผังดวงจริงที่แนบมา (ลัคนา/ภพ/ดาวเจ้าชนะ/ดาวจร) = *ตัวประกอบ* ใช้เน้นน้ำหนัก เพิ่มเหตุการณ์ ให้เหตุผล และชี้ช่วงเวลา\n"
                    ."(ทั้งคู่เป็นของจริงที่คำนวณมาแล้ว — ห้ามทิ้งอย่างใดอย่างหนึ่ง ดูสัดส่วนในบล็อก 🧬 ด้านล่าง)\n\n"
            ...
            ."✅ *ต้องทำ*:\n"
            .($withAstro
                ? "• ทุกข้อสรุป/คำทำนาย/คำแนะนำ ต้อง \"งอกจากไพ่ใบใดใบหนึ่งที่ตำแหน่งใดตำแหน่งหนึ่ง *หรือ* จากดาว/ภพ/ดาวจรที่ระบุไว้ในผัง\" เสมอ\n"
                    ."  ในใจต้องตอบได้ว่า \"ประโยคนี้มาจากไพ่ใบไหน ตำแหน่งอะไร / ดาวดวงไหน ภพไหน\" — ถ้าตอบไม่ได้ = ห้ามพูด\n"
            ...
            ."• 🔎 *โหมดนักสืบ — ชวนลูกมีส่วนร่วมให้ \"โดน\"*: พอทำนายเจอเรื่องเฉพาะ (เช่น \"มีคนคิดไม่ดีกับลูก\") → ชวนยืนยัน/เดา\n"
            ."   เช่น \"คิดออกไหมว่าเป็นใคร? คนใกล้ตัวหรือที่ทำงานหรือเปล่า?\" — ถ้าลูกรับว่าใช่/เริ่มเล่า → ซักให้เล่ารายละเอียด แล้วผูกไพ่ทำนายให้ลึก-ตรงขึ้น\n"
            ...
            ."      (ระบบคำนวณให้เองทันทีที่ลูกพิมพ์วันเกิด เช่น เทียบธาตุ/ดาวมิตร-ศัตรู ของคู่รัก, จับจังหวะเวลาจากดาวเสริมไพ่ตำแหน่งอนาคต)\n"
            ."      • *การ \"ขอ\" วันเกิด* = เก็บข้อมูล `[TYPE:D]` ไม่นับ / พอ *ได้* วันเกิดแล้ว → ทำนายเสริมดวงทันที `[TYPE:A]` นับ (ห้ามขอแล้วเงียบ)\n"
            .($withAstro
                // 🧬 (2026-09-03 owner) รอบที่มีผังดวงจริงแล้ว — ห้ามลดดวงเป็น "ของประดับ"
                //   owner: "ทุกอย่างต้องนำมาพยากรณ์ร่วมกันทั้งหมด ให้เป็นเนื้อเดียวกัน เป็นเหตุผลซึ่งกันและกัน"
                ? "      • 🧬 *ไพ่กับดวงต้องอ่านร่วมกัน อย่าออกทะเล*: ไพ่บอก \"อะไรกำลังเกิด\" · ผังดวงบอก \"ทำไมถึงเป็นแบบนี้\" · ดาวจรบอก \"เมื่อไหร่\" — ทุกอย่างวนกลับมาที่คำถาม (ดูบล็อก 🧬 ด้านล่าง)\n"
            ...
            .($withAstro
                ? "🧭 *ลำดับความสำคัญ*: (1) อ่านไพ่ + ตำแหน่ง = แกน → (2) ผังดวง/ลัคนา หนุนให้คมและมีเหตุผลขึ้น\n"
                    ."   → (3) คำแนะนำ/ทางออก/มู/ธรรมะ เป็น \"ผลที่งอกจาก (1)+(2)\" เท่านั้น\n"
                    ."   จะแนะนำอะไร ต้องชี้ได้ว่าไพ่ใบไหนทำให้แนะแบบนั้น และดวงส่วนไหนมาหนุน\n"
            ...
            // 🪬 (2026-09-05) กฎ "ห้ามพิมพ์ตัวบทสวด/คาถา" **ไม่ได้อยู่ตรงนี้** —
            //   ฉีดที่ system role ผ่าน FortuneAIService::getReadingSystemMessage() ซึ่งคลุม
            //   ทั้ง Deep 39 + Celtic 99 ทุก provider อยู่แล้ว. เคยฉีดซ้ำตรงนี้ = พรอมต์บวม
            //   ~750 ตัว × ทุก call ของ Q1/Q2+/บทสรุป โดยไม่ได้อะไรเพิ่ม
            ."━━━━━━━━━━━━━━━━━\n\n";
    }

    /**
     * 🧬 (2026-09-03 owner) หลักอ่านร่วม 3 ชั้น — ไพ่ × ผังดวง × ดาวจร ต้องเป็น "เนื้อเดียวกัน"
     *
     * owner: "พื้นฐานดวงกับดวงของไพ่ที่เปิด มันยังไม่สอดคล้องกัน ... จริงๆ ทุกอย่างต้องนำมา
     *         พยากรณ์ร่วมกันทั้งหมด ให้เป็นเนื้อเดียวกัน แม่นยำที่สุด เป็นเหตุผลซึ่งกันและกัน"
     *
     * ⚠️ ปัญหาเดิม: ข้อมูลครบทั้ง 4 จุดของเลน 99฿ (พื้นดวงเปิดตัว / Q1 / Q2+ / บทสรุป VIP)
     *   แต่ *ไม่เคยมีกฎว่าให้เอามาเถียงกันยังไง* — พรอมต์บอกแค่ "ผสาน" ลอยๆ
     *   ⇒ โมเดลเขียนออกมาเป็น 2 บทพูดขนานกัน (ย่อหน้าดวง / ย่อหน้าไพ่) แล้วลูกค้าอ่านแล้ว
     *     รู้สึกว่าไม่เกี่ยวกัน · และเวลาไพ่กับดวงชี้คนละทาง โมเดลเลือกข้างเงียบๆ หรือฟันธง
     *     ขัดกันเองในบทเดียว
     *
     * บล็อกนี้ให้ 3 อย่างที่ "ผสาน" ลอยๆ ให้ไม่ได้:
     *   1. หน้าที่ของแต่ละชั้น (อะไร / ทำไม / เมื่อไหร่) — กันการพูดซ้ำเรื่องเดียวกัน 2 รอบ
     *   2. เกณฑ์ยืนอย่างน้อย 2 ชั้นต่อ 1 คำฟันธง — บังคับให้เหตุผลส่งต่อกันจริง
     *   3. กติกาเวลาขัดกัน — ต้องพูดออกมาแล้วคลี่ให้จบ ห้ามเงียบ ห้ามฟันธง 2 ทางในบทเดียว
     *
     * ⚠️ ใช้เฉพาะรอบที่ *มีผังดวงจริงแนบมาในพรอมต์แล้ว* — ไม่มีวันเกิด = ไม่ต้องฉีด
     *    (คู่กับ buildCardFirstMandate(withAstro: true) เสมอ ไม่งั้นกฎ 2 ตัวจะขัดกันอีกรอบ)
     *
     * @param  bool  $lean  true = ฉบับย่อสำหรับบทสรุป VIP (พรอมต์นั้นถูกบีบเรื่องความยาวอยู่)
     */
    protected function buildFusionDoctrine(bool $lean = false): string
    {
        if ($lean) {
            return "━━━━━━━━━━━━━━━━━\n"
                ."🧬 ไพ่นำ · ดวงประกอบ — บทสรุปนี้ต้องเป็นเนื้อเดียวกัน\n"
                ."━━━━━━━━━━━━━━━━━\n"
                ."• 🃏 *ไพ่ = แกนของคำตอบ* — ความหมาย+บริบทของไพ่คือเนื้อหลักของทุกย่อหน้า (อ้างว่า \"ไพ่บอกว่า...\" ได้ ห้ามเอ่ยชื่อไพ่)\n"
                ."• 🌀 *ดวง/ลัคนา = ตัวประกอบ* — ใช้ **เน้นน้ำหนัก · เพิ่มรายละเอียดเหตุการณ์ · อธิบายว่าทำไมถึงเป็นแบบนั้น · ชี้ช่วงเวลา** ให้คำตอบหนักแน่นขึ้น\n"
                // 🌀 (2026-09-04) วัดบทสรุปจริง rd12274: ลัคนา 0 · ดาวเจ้าเรือน 0 · ภพ 0 ครั้ง ทั้งบท
                //   "ตัวประกอบ" ที่ไม่มีเกณฑ์ขั้นต่ำ = โมเดลตัดทิ้งได้ทั้งหมด ⇒ กลับไปเป็นบทไพ่ล้วนเหมือนเดิม
                ."• 📐 *ขั้นต่ำ*: ต้องหยิบพื้นดวงมาใช้ *อย่างน้อย 1-2 จุด* ในบท (ลัคนา / ดาวเจ้าเรือน / ดาวเสวยอายุ / ภพที่เกี่ยว)\n"
                ."   ผูกกับเรื่องที่ลูกถาม — ไม่ใช่ท่องชื่อดาวลอยๆ (❌ ห้ามบรรยายองศา/ศัพท์เทคนิค)\n"
                ."• ❌ ห้ามแยกเป็น \"ย่อหน้าดวง\" แล้ว \"ย่อหน้าไพ่\" · ❌ ห้ามให้ดวงกลบไพ่จนอ่านเหมือนบทโหราศาสตร์\n"
                ."• ⚖️ ไพ่กับดวงขัดกัน = ห้ามเงียบ → อธิบายด้วย *อิทธิพล* ที่ทำให้ผลเปลี่ยน (บุญ/องค์เทพคุ้มครอง/ดาวหนุน/ดาวกด) แล้วฟันธง *คำตอบเดียว*\n"
                ."━━━━━━━━━━━━━━━━━\n\n";
        }

        return "━━━━━━━━━━━━━━━━━\n"
            ."🧬 ไพ่นำ · ดวงประกอบ — ต้องออกมาเป็น \"เนื้อเดียวกัน\"\n"
            ."━━━━━━━━━━━━━━━━━\n"
            ."รอบนี้เจ้าชะตาให้วันเกิดแล้ว ⇒ มี *ผังดวงจริง* (ลัคนา/12 ภพ/ดาว 9 ดวง/ทักษา/ดาวเสวยอายุ + ดาวจรวันนี้)\n"
            ."แนบมาพร้อมไพ่ 10 ใบ — ทั้งสองอย่างเป็นของคนคนเดียวกัน จึงต้องอธิบายชีวิตเรื่องเดียวกัน ไม่ใช่คนละเรื่อง\n\n"

            // 🃏 (2026-09-04 owner) สัดส่วนที่เจ้าของสั่งเอง:
            //   *"เน้นบริบทคำทำนายไพ่เป็นหลักสำหรับแบบ 99 ทั้งคุณไสยและแบบปกติ แล้วจึงนำดวง ลัคนา
            //   มาวิเคราะห์ประกอบ เพื่อเน้นหรือเพิ่มเหตุการณ์ ให้คำทำนายในเรื่องที่ถาม สมเหตุผล
            //   มีทางออก มั่นใจในการตอบ ได้มากขึ้น"*
            //   ⚠️ ห้ามเขียนบล็อกนี้ให้ดวงมีน้ำหนักเท่าไพ่ — รอบก่อนเขียนว่า "แหล่งความจริงร่วม"
            //     ผลจริงจากบิล FTU-260904-H3377 คือคำทำนายกลายเป็น *บทโหราศาสตร์ล้วน* ไพ่หายเกลี้ยง
            ."📐 *สัดส่วน — ห้ามสลับ*:\n"
            ."• 🃏 *ไพ่ = แกนหลักของคำทำนาย* (พูดถึงมากที่สุด)\n"
            ."   ความหมาย + บริบทของไพ่ที่เปิด × ตำแหน่งของมัน = เนื้อคำตอบ: เกิดอะไรขึ้น · ใครเกี่ยว · จบยังไง\n"
            ."   ⇒ เจ้าชะตาต้องรู้สึกว่านี่คือ \"คำตอบจากไพ่ของตัวเอง\" (ใช้สำนวน \"ไพ่บอกว่า...\" ❌ ห้ามเอ่ยชื่อไพ่)\n"
            ."• 🌀 *ผังดวง + ลัคนา = ตัวประกอบ* (เสริมให้ไพ่คมขึ้น ไม่ใช่มาเล่าดวงแยกอีกเรื่อง) — ใช้ทำ 4 อย่างนี้เท่านั้น:\n"
            ."   1) *เน้นน้ำหนัก* — ดวงชี้ตรงกับไพ่ = ฟันธงแรงขึ้นได้ (\"ไพ่เตือนเรื่องนี้ และภพ...ในดวงลูกก็หนักเรื่องเดียวกันพอดี\")\n"
            ."   2) *เพิ่มเหตุการณ์/รายละเอียด* — ระบุคน ที่มา ลักษณะ ให้เจาะจงขึ้นจากภพ/ดาวเจ้าเรือน\n"
            ."   3) *ทำให้สมเหตุผล + มีทางออก* — อธิบายว่าทำไมชีวิตถึงเดินมาถึงจุดที่ไพ่ชี้ แล้วทางออกควรเป็นทางไหนตามพื้นดวง\n"
            ."   4) *ชี้ช่วงเวลา* — ดาวจร/ดาวเสวยอายุ = นาฬิกาของเรื่อง ใช้ผูกเดือน/ช่วง แทนการเดาเอง\n\n"

            ."✅ *วิธีเขียนให้เป็นเนื้อเดียว (บังคับ)*:\n"
            ."• เริ่มจากไพ่เสมอ → แล้วดึงดวงเข้ามาหนุนในย่อหน้าเดียวกัน (ไม่ใช่ย่อหน้าใหม่)\n"
            ."  ตัวอย่างโทน: \"ไพ่บอกชัดว่ารอบนี้ลูกเป็นฝ่ายกลืนไว้ฝ่ายเดียวในเรื่องคู่ —\n"
            ."   และพื้นดวงลูกก็หนักเรื่องคู่อยู่แล้ว (ลัคนาทำให้ยอมคนง่าย เพื่อรักษาความสัมพันธ์)\n"
            ."   สองอย่างนี้ชี้ตรงกัน แม่หมอจึงฟันธงว่าเรื่องจะแตกให้เห็นภายใน 1-2 เดือนนี้ ตามจังหวะดาวที่กำลังทับอยู่\"\n"
            ."• ทุกด้าน/ทุกหัวข้อ ต้องมีไพ่เป็นตัวตั้ง แล้วมีดวงเสริมอย่างน้อย 1 จุด — ไม่ใช่ไพ่ล้วนบางหัวข้อ ดวงล้วนบางหัวข้อ\n"
            ."• ในใจต้องตอบได้เสมอว่า \"ประโยคนี้มาจากไพ่ใบไหน+ตำแหน่งอะไร และดวงส่วนไหนมาหนุน\"\n\n"

            ."❌ *ห้าม*:\n"
            ."• ❌ ห้ามเขียนแยกเป็น \"ย่อหน้าดวง\" แล้วต่อด้วย \"ย่อหน้าไพ่\" (สองบทพูดขนานกัน = ลูกอ่านแล้วรู้สึกไม่เกี่ยวกัน)\n"
            ."• ❌ ห้ามให้ดวงกลบไพ่จนทั้งบทอ่านเหมือนคำทำนายโหราศาสตร์ — ลูกจ่ายค่าเปิดไพ่ 10 ใบ ต้องได้ยินเสียงไพ่\n"
            ."• ❌ ห้ามพูดถึงดวงลอยๆ ที่ไม่ผูกกับคำถาม/ไพ่ (เช่น ท่องนิสัยตามราศีแล้วจบ)\n"
            ."• ❌ ห้ามฟันธง \"ช่วงเวลา\" โดยไม่มีดาวจร/ดาวเสวยอายุ/ไพ่ฝั่งอนาคตรองรับ\n"
            ."• ❌ ห้ามอ้างดาว/ภพที่ไม่มีในผังด้านบน (ผังคำนวณมาแล้ว — อ่านจากของจริงเท่านั้น)\n\n"

            // ⚖️ (2026-09-04 owner) *"หากมีความขัดแย้งที่ไม่สมเหตุผล ก็ต้องให้เหตุผลได้ จากอิทธิพลของ
            //   ดวงหรือไพ่ เช่นเจ้าชะตามีบุญ หรือเทวดาองเทพช่วยผ่านเคราะห์นี้ได้"*
            //   ⇒ ทางออกเวลาชนกันไม่ใช่ "เลือกข้าง" แต่คือ "อธิบายด้วยอิทธิพลที่ทำให้ผลเปลี่ยน"
            ."⚖️ *ถ้าไพ่กับดวงชี้คนละทาง* (เกิดได้ปกติ — ห้ามเงียบ ห้ามเลือกข้างมั่ว):\n"
            ."• ห้ามปล่อยให้ขัดกันลอยๆ — ต้อง *อธิบายให้สมเหตุผล* ว่าทำไมสองอย่างถึงไม่ตรงกัน แล้วจบด้วยคำฟันธง *คำตอบเดียว*\n"
            ."• เครื่องมืออธิบาย (ใช้เท่าที่หน้าไพ่/ผังรองรับจริง ❌ ห้ามยกมาลอยๆ เพื่อปลอบ):\n"
            ."   – *บุญ/กรรมดี-ทานที่เคยทำ* ผ่อนเคราะห์ที่ไพ่ชี้ให้เบาลง\n"
            ."   – *องค์เทพ/สิ่งศักดิ์สิทธิ์ที่คุ้มครองเจ้าชะตา* ช่วยพยุงให้ผ่านด่านนี้ไปได้\n"
            ."   – *ดาวหนุน* (ดาวมิตร/ดาวอุจ/ภพดี) ค้ำไว้ ⇒ เรื่องร้ายเกิดแต่ไม่ถึงที่สุด\n"
            ."   – *ดาวกด* (ดาวศัตรู/นิจ/ดาวเสวยอายุที่หนัก) ⇒ เรื่องดีที่ไพ่ชี้มาช้ากว่าที่ควร\n"
            ."  ตัวอย่างโทน: \"ไพ่ชี้เกณฑ์เสียเงินก้อนชัดมาก แต่ดวงลูกมีดาวหนุนค้ำอยู่ และองค์ที่คุ้มครองลูกก็แรง\n"
            ."   ⇒ แม่หมอฟันธงว่าเสียจริงแต่ไม่หมดตัว จะมีคนหรือทางออกโผล่มาช่วยไว้ทัน ถ้าลูกไม่ฝืนเซ็นอะไรเพิ่ม\"\n"
            ."• ❌ ห้ามฟันธงสองอย่างที่ขัดกันในบทเดียวโดยไม่คลี่ให้จบ (ดวงดี-ไพ่ร้าย แล้วปล่อยลอย = ลูกจับได้ทันทีว่าไม่สอดคล้อง)\n"
            ."━━━━━━━━━━━━━━━━━\n\n";
    }

    /**
     * 🌟 (2026-09-04) บิลนี้เคยส่ง "พื้นดวงเปิดตัว" ให้ลูกค้าไปแล้วหรือยัง
     *
     * ⚠️ ทำไมพื้นดวงยิงได้ 2 รอบ (เคสจริง FTU-260904-S1420):
     *   1. เปิดไพ่ครบ 10 ใบ ตอนที่ยังไม่มีวันเกิด (ด่านวันเกิดถูกข้าม) → ยิงพื้นดวงจาก *ไพ่ล้วน*
     *   2. ลูกค้าพิมพ์วันเกิดตามมาทีหลัง → เส้น "late birthdate captured หลัง gate ปิด"
     *      สร้างพื้นดวงใหม่ให้ *พร้อมดวงดาว* — เจตนาถูกแล้ว (รอบแรกไม่มีดวงเลย)
     *   ผลข้างเคียงที่ไม่ตั้งใจ: ลูกค้าได้ข้อความ 5,600 ตัว 2 รอบซ้อน + **ถูกตัดโควตา 2 ข้อ**
     *   ทั้งที่ยังไม่ได้ถามอะไรเลยสักคำถาม (used=2 ก่อนเริ่มถามจริง)
     *
     * ใช้ 2 ที่: (ก) ไม่ตัดโควตารอบซ้ำ (ข) บอกโมเดลให้เขียนแบบ "อ่านใหม่เพราะเพิ่งได้วันเกิด"
     *
     * @param  int  $currentSequence  แถวของรอบปัจจุบัน — ไม่นับตัวเอง
     */
    protected function baseChartAlreadyDelivered(FortuneReading $reading, int $currentSequence): bool
    {
        $memoKey = $reading->id.'|'.$currentSequence;
        if (isset($this->baseChartDeliveredMemo[$memoKey])) {
            return $this->baseChartDeliveredMemo[$memoKey];
        }

        try {
            $delivered = $reading->celticQuestions()
                ->whereNotNull('answered_at')
                ->where('sequence', '<', $currentSequence)
                ->where(function ($q) {
                    $q->where('question', 'like', '%พื้นดวงรวม%')
                        ->orWhere('question', 'like', '%พื้นดวงเรื่อง%');
                })
                ->exists();
        } catch (\Throwable $e) {
            $delivered = false; // query ไม่ได้ → ถือว่ายังไม่เคยส่ง (พฤติกรรมเดิม = ตัดโควตา)
        }

        return $this->baseChartDeliveredMemo[$memoKey] = $delivered;
    }

    /** 🗄️ memo ของ baseChartAlreadyDelivered() — ต่อ request */
    protected array $baseChartDeliveredMemo = [];

    /**
     * 🔁 (2026-09-04) คำสั่งสำหรับ "พื้นดวงรอบที่ 2" — เกิดเมื่อลูกค้าให้วันเกิดตามมาทีหลัง
     *
     * เคสจริง FTU-260904-S1420: ลูกค้าได้ข้อความ 5,657 ตัว ตอน 21:13 (ไพ่ล้วน ไม่มีวันเกิด)
     *   แล้วได้อีก 5,585 ตัว ตอน 21:25 (หลังให้วันเกิด) — **พาดหัวเรื่องเดียวกันเป๊ะ**
     *   ("การตัดสินใจครั้งใหญ่ที่ค้างมานาน") ⇒ อ่านแล้วเหมือนระบบส่งซ้ำ ไม่ใช่ของใหม่
     *
     * รอบ 2 ต้องรู้ตัวว่าเป็น "การอ่านซ้ำที่ลึกขึ้น" — เปิดด้วยการบอกเหตุผล
     * แล้วเทน้ำหนักไปที่ *ของใหม่* (ดวงดาว/ลัคนา/เรือนชะตา/จังหวะเวลา) ไม่ใช่เล่าไพ่เดิมซ้ำ
     *
     * @return string ว่าง = เป็นพื้นดวงรอบแรกจริง (ไม่ต้องใส่อะไร)
     */
    protected function buildBaseChartRerunNotice(FortuneReading $reading): string
    {
        if (! $this->baseChartAlreadyDelivered($reading, PHP_INT_MAX)) {
            return '';
        }

        return "━━━━━━━━━━━━━━━━━\n"
            ."🔁 *นี่คือการอ่านพื้นดวง \"รอบที่สอง\" ของบิลนี้ — ห้ามเขียนเหมือนรอบแรก*\n"
            ."━━━━━━━━━━━━━━━━━\n"
            ."รอบแรกแม่หมออ่านให้ไปแล้วตอนที่ *ยังไม่รู้วันเกิด* (อ่านจากไพ่ล้วน)\n"
            ."ตอนนี้ได้วันเกิด (และเวลาเกิด ถ้าลูกบอก) แล้ว ⇒ รอบนี้คือการ *อ่านซ้ำให้ลึกขึ้นด้วยดวงดาว*\n\n"
            ."✅ ต้องทำ:\n"
            ."• เปิดด้วย 1-2 ประโยคบอกเหตุผลตรงๆ ว่าทำไมอ่านให้อีกรอบ — ให้ลูกไม่งงว่าทำไมได้ข้อความยาวซ้ำ\n"
            ."  เช่น \"พอได้วันเกิดของลูกแล้ว แม่หมอขออ่านใหม่ให้ลึกกว่าเดิมนะคะ รอบนี้มีดวงดาวมาช่วยยืนยันไพ่\"\n"
            ."• เทน้ำหนักไปที่ *ของใหม่ที่รอบแรกไม่มี*: ลัคนา · เรือนชะตา · ดาวเจ้าเรือน · ดาวเสวยอายุ · จังหวะเวลา\n"
            ."• เรื่องที่รอบแรกพูดไปแล้ว → พูดสั้นลง แล้ว *ต่อยอด* ว่าดวงดาวยืนยันหรือขัดกับไพ่ตรงไหน\n"
            ."❌ ห้ามเปิดด้วยพาดหัวเรื่องเด่นประโยคเดิมของรอบแรก ❌ ห้ามเล่าไพ่ 10 ใบซ้ำแบบเดิมทั้งชุด\n"
            ."━━━━━━━━━━━━━━━━━\n\n";
    }

    /**
     * 🔭 คำถามนี้ "ถามถึงข้างหน้า" ไหม — ด่านเปิดโหมดซีรี่ส์
     *
     * owner (2026-09-04): "ไอ้เหตุการณ์ในอนาคต ให้เส้นนี้เข้าเมื่อมีการถามถึงอนาคต และไพ่ขึ้นบอก
     *   ลักษณะที่เราสร้างเป็นเรื่องราวเพื่อบอกเล่าได้จริงนะ"
     *   ⇒ 2 ด่าน: (1) คำถามพูดถึงอนาคต (ตัวนี้) + (2) คลังมีสัญญาณจริง (storySignalLines คืนไม่ว่าง)
     *
     * ⚠️ สแกน "คำถามของลูกค้า" เท่านั้น — ห้ามสแกน previousContext (คำตอบเก่าของ AI)
     *    บทเรียนจาก buildBlackMagicDirective: สแกนคำตอบ AI แล้วเจอ "มีเสน่ห์" ⇒ ยิงตำราคุณไสย 7k ทุกเทิร์น
     */
    protected function isFutureOrientedQuestion(string $userQuestion): bool
    {
        $q = mb_strtolower(trim($userQuestion));
        if ($q === '' || mb_strpos($q, '__PREDICT_ALL__') !== false || mb_strpos($q, 'พื้นดวงรวม') !== false) {
            return false;
        }

        foreach ([
            'เมื่อไหร่', 'เมื่อไร', 'ตอนไหน', 'ช่วงไหน', 'กี่เดือน', 'กี่ปี', 'กี่วัน',
            // 🐛 (2026-09-04) เพิ่มจากคำถามจริงของบิล FTU-260904-V5241 ที่ด่านนี้พลาด:
            //   "ควรแยกตัวจากงานสามี*ช่วงเดือนไหน*" — มี 'ช่วงไหน' ในลิสต์ แต่ประโยคจริงมี 'เดือน' คั่นกลาง
            //   "ธุรกิจหรืองานแบบไหน*เหมาะ*ให้ลูก*เริ่ม*ก่อน" — ถามทางเลือกข้างหน้าชัดๆ แต่ไม่มีคำไหนตรงลิสต์
            //   ⇒ คำถามอนาคตที่ชัดที่สุด 2 ข้อของบิลนั้นไม่ได้บทซีรี่ส์เลย
            'เดือนไหน', 'ปีไหน', 'วันไหน', 'ช่วงเดือน', 'ระยะไหน',
            'อีกนาน', 'นานแค่ไหน', 'นานไหม', 'เกณฑ์', 'โอกาส', 'เหมาะ', 'ควรเริ่ม', 'เริ่มก่อน',
            'อนาคต', 'ข้างหน้า', 'ต่อไป', 'ปีนี้', 'ปีหน้า', 'เดือนหน้า', 'เร็วๆ นี้', 'เร็วๆนี้', 'เร็ว ๆ นี้',
            'จะได้', 'จะมี', 'จะเจอ', 'จะเป็น', 'จะกลับ', 'จะสำเร็จ', 'จะผ่าน', 'จะรอด', 'จะหาย', 'จะดีขึ้น',
            'จะเกิด', 'จะมา', 'จะไป', 'จะย้าย', 'จะเดินทาง', 'จะรวย', 'จะได้งาน', 'จะท้อง', 'จะแต่ง', 'จะเลิก',
            'จะจบ', 'จะหาย', 'จะขาย', 'จะซื้อ', 'จะสอบ', 'จะติด',
            'ได้ไหม', 'ได้มั้ย', 'มีไหม', 'มีมั้ย', 'แนวโน้ม', 'ดวงเป็นยังไง', 'ดวงเป็นอย่างไร', 'ดวงจะ', 'เนื้อคู่', 'คู่แท้',
        ] as $kw) {
    ...
    /**
     * 🎬 (2026-09-04 owner) โหมดซีรี่ส์ — เล่าคำทำนายเป็นเรื่องที่มี "ตัวละคร + เหตุการณ์ข้างหน้า"
     *
     * owner: "เราเคยทำว่าให้สร้างเป็นเหมือนซีรี่ เหตุการณ์ มีตัวละคร ในบางกรณีที่มีคนเข้ามาช่วย
     *   เหตุการณ์ในอนาคตที่ต้องรีบบอก เช่น มีเกณฑ์เดินทางแล้วมีปัญหา อุบัติเหตุ หรือมีคนสร้างเรื่องให้เรา
     *   หรือมีคนนำโชคมาให้ รูปร่างหน้าตาอายุ (มีในคลังความรู้) โรคต่างๆ มีในคลังหมด
     *   แต่เหมือนไม่ถูกหยิบมาใช้เลย"
     *
     * ทำไมของเดิมไม่ทำงาน (วัดจากพรอมต์จริง rd12214, 61,492 ตัวอักษร):
     *   1. คลัง persona/person_role/health/travel/legal ยิงตาม *คีย์เวิร์ดในคำถาม* — คนไม่ถามถึง
     *      "อุบัติเหตุ" ก็ไม่มีวันได้ยินคำเตือน (ทั้งที่นี่คือของที่ต้อง "รีบบอก")
     *   2. บล็อกไหนที่ยิงติด ก็ถูก buildCardTalkPolicy บอกว่า "= เครื่องมือคิดในใจ พูดเฉพาะคำตอบ"
     *      ⇒ โมเดลอ่านเป็น "อย่าเอารูปพรรณ/วัย/โรคออกมาพูด"
     *   3. คำสั่งเรื่องเล่ามีแค่บรรทัดเดียวใน mandate ("แต่งเรื่อง/สมมติฉากได้") — ไม่มีโครง ไม่มีตัวละคร
     *
     * ด่าน 2 ชั้นตาม owner: ถามถึงอนาคต (isFutureOrientedQuestion) **และ** ไพ่มีสัญญาณ
     *   (storySignalLines ไม่ว่าง) — ไม่มีสัญญาณ = ไม่ฉีดบทนี้เลย (ห้ามปั้นเรื่องจากอากาศ)
     *   บทสรุป VIP = "จบยังไง" โดยธรรมชาติ → ข้ามด่านคำถาม ($isFinale)
     *
     * งบ: directive ~900 + สัญญาณ ≤ ~3k (ตัดชิ้นละ 95 ตัว, เฉพาะตำแหน่ง 7/8/2 + 6/10/2)
     *   จ่ายด้วยการตัด elemental+position-dynamic (4k) ออกจาก Q2+ และ physiognomy+person_role
     *   เต็ม 10 ใบ (7k) ออกจากบทสรุป — สุทธิพรอมต์ *เล็กลง* ไม่ใช่บวมขึ้น
     */
    protected function buildStoryModeDirective(FortuneReading $reading, string $userQuestion, bool $isFinale = false): string
    {
        if (! (bool) ($this->settings->enable_celtic_story_mode ?? true)) {
            return '';
        }
        if (! $isFinale && ! $this->isFutureOrientedQuestion($userQuestion)) {
            return '';
        }

        // 🐾 ตัวละครเป็นสัตว์เลี้ยง → คลังรูปพรรณ/วัย/บทบาท "คน" ใช้ไม่ได้ (เคส FTU-260904-S9843)
        //   ⚠️ เทียบกับ KIND_PET เท่านั้น (ค่าจริงของคนคือ 'human' ไม่ใช่ 'person' — เกือบเขียนผิดแล้ว
        //   ฟีเจอร์จะตายเงียบกับลูกค้าทุกคน) · KIND_UNKNOWN = ถือว่าเป็นคนเหมือนตำราอื่น
        try {
            if (($this->resolveSubjectSpecies($reading, $userQuestion)['kind'] ?? null)
                === \App\Services\Fortune\SubjectSpeciesDetector::KIND_PET) {
                return '';
            }
        } catch (\Throwable $e) {
            // ตรวจไม่ได้ → ถือว่าเป็นคน (พฤติกรรมเดิม)
        }
        ...
        try {
            $signals = trim((string) app(\App\Services\FortuneKnowledgeService::class)->storySignalLines($cards));
        } catch (\Throwable $e) {
            return '';
        }
        if ($signals === '') {
        ...
        $scope = $isFinale
            ? 'บทสรุปนี้คือ "ตอนจบของซีรี่ส์" — ทุกเรื่องที่ถามมาต้องเห็นว่าจบยังไง ใครเกี่ยว และจะเจออะไรก่อนถึงตอนจบ'
            : 'คำถามนี้ถามถึง "ข้างหน้า" และไพ่มีสัญญาณให้เล่า';

        return "━━━━━━━━━━━━━━━━━\n"
            ."🎬 โหมดซีรี่ส์ — เล่าคำทำนายเป็นเรื่องที่มีตัวละครและเหตุการณ์\n"
            ."━━━━━━━━━━━━━━━━━\n"
            .$scope."\n"
            ."• 🎭 *ตัวละคร*: ใครเกี่ยวบ้าง (คนรอบตัว / ผู้ขวาง-ผู้ก่อเรื่อง / คนที่จะเข้ามาช่วยหรือนำโชค)\n"
            ."   วาด *รูปพรรณ ช่วงวัย ท่าที นิสัย* ตามคลังด้านล่างเท่านั้น — ให้ลูก \"นึกออกว่าเป็นใคร\"\n"
            ."   (❌ ห้ามระบุชื่อจริง ❌ ห้ามเสกตัวละครที่ไพ่ไม่มี ✅ คลังบอกว่า \"มักเป็น: X\" = ใช้ X เป็นตัวละครได้เลย)\n"
            ."• ⚡ *เหตุการณ์ที่ต้องรีบบอก* (เฉพาะที่คลังด้านล่างชี้): เดินทางแล้วมีเรื่อง/อุบัติเหตุ · คนสร้างเรื่อง-ใส่ร้าย-หักหลัง ·\n"
            ."   คนนำโชค-เงินก้อน-ผู้ใหญ่ช่วย · คดี/สัญญา · โรค-อวัยวะที่ต้องระวัง — *บอกก่อนแม้ลูกไม่ได้ถาม*\n"
            ."   พร้อม *ช่วงเวลา* (ผูกจังหวะในคลัง + ดาวจร/ดาวเสวยอายุถ้ามีผัง) และ *ทางเลี่ยง/ทางออก 1 ข้อ* ที่ทำได้จริง\n"
            ."• 🎞️ *ลำดับเล่า*: ตอนนี้เกิดอะไร → ใครเข้ามาเกี่ยว → จะเจออะไร เมื่อไหร่ → เรื่องจบยังไง → ลูกทำอะไรได้\n"
            ."   เล่าต่อเนื่องเป็นฉากเดียวกัน ไม่ใช่ list หัวข้อ — คนอ่านต้องเห็นภาพเหมือนดูตอนต่อไปของชีวิตตัวเอง\n"
            ."• ✅ อ้างที่มาด้วย \"ไพ่บอกว่า...\" ❌ ห้ามชื่อไพ่/เลขตำแหน่ง ❌ ห้ามบรรยายทีละใบ ❌ ห้ามลอกรูปแบบบรรทัดคลัง\n"
            ."• ⚖️ *คลังว่างส่วนไหน = ไม่มีสัญญาณส่วนนั้น* → ไม่ต้องพูดถึง — เล่าได้เฉพาะที่ไพ่ขึ้นจริง ห้ามเติมเคราะห์เพื่อให้ลุ้น\n"
            ."━━━━━━━━━━━━━━━━━\n"
            ."🗂️ สัญญาณสำหรับเล่า (อ่านจากหน้าไพ่ชุดนี้ — ข้อมูลภายใน ห้ามลอกรูปแบบออกไป):\n"
            .$signals
            ."━━━━━━━━━━━━━━━━━\n\n";
    }
    ...
    protected function buildCardTalkPolicy(?string $userQuestion = null): string
    {
        ...
        return "━━━━━━━━━━━━━━━━━\n"
            ."🎭 กฎการเล่า — พูด \"ความหมายของไพ่\" ไม่ต้องพูด \"ชื่อไพ่\"\n"
            ."━━━━━━━━━━━━━━━━━\n"
            ...
            // 🃏 (2026-09-04 owner) เดิมข้อนี้ห้าม "ไพ่บอกว่า..." ด้วย ⇒ ไพ่หายไปจากคำทำนายทั้งบท
            //   owner สั่งใหม่: *"เอาความหมายและบริบทของไพ่มาพูดเป็นหลัก แต่ไม่ต้องบอกชื่อไพ่
            //   ไม่งั้นจะมีแต่ชื่อไพ่เต็มไปหมด ใช้คำว่า 'ไพ่บอกว่า' แทน"*
            //   ⇒ ที่ห้ามคือ *ชื่อไพ่* กับ *เลขตำแหน่ง* ไม่ใช่การอ้างว่ามาจากไพ่
            ."✅ *ให้อ้างไพ่แบบไม่ระบุใบ — ใช้สำนวนพวกนี้ได้เต็มที่ (ควรใช้ด้วย)*:\n"
            ."   \"ไพ่บอกว่า...\" · \"หน้าไพ่ชี้ชัดว่า...\" · \"ไพ่ที่เปิดออกมาเตือนว่า...\" · \"ไพ่ฝั่งอนาคตบอกว่า...\"\n"
            ."   ⇒ เจ้าชะตาต้องรู้สึกว่า *นี่คือคำตอบจากไพ่ของตัวเอง* ไม่ใช่คำแนะนำชีวิตทั่วไป\n\n"

            ."• ❌ ห้ามเอ่ย *ชื่อไพ่* (\"ไพ่คนบ้ากลับหัว\" / \"ห้าดาบ\" / \"หอคอย\")\n"
            ."• ❌ ห้ามเอ่ย *ชื่อ/เลขตำแหน่งทางเทคนิค* (\"ตำแหน่งที่ 6\" / \"ใบครอส\" / \"ตำแหน่งอุปสรรค\")\n"
            ."   (พูดเป็นภาษาคนได้ เช่น \"ไพ่ฝั่งอนาคต\" / \"ไพ่ที่ขวางอยู่\" — แต่ห้ามเป็นศัพท์เทคนิค/เลขใบ)\n"
            ."• ❌ ห้ามไล่บรรยายทีละใบ ❌ ห้ามปิดท้ายด้วยบรรทัดสรุปไพ่ ❌ ห้ามอธิบายว่าไพ่ใบนั้นแปลว่าอะไรในตำรา\n\n"

            ."✅ *เนื้อหาทุกประโยคยังต้องงอกจากไพ่จริง 100% ตามกฎเหล็กด้านบน*\n"
            ."   ❌ \"ไพ่ห้าดาบที่ตำแหน่งอุปสรรคบอกว่าจะมีคนหักหลัง\"  ← เอ่ยชื่อไพ่+ตำแหน่ง\n"
            ."   ❌ \"มีคนใกล้ตัวกำลังคิดไม่ซื่อกับลูกอยู่\"                ← ลอยเกิน ไม่รู้ว่ามาจากไหน\n"
            ."   ✅ \"ไพ่บอกชัดว่ามีคนใกล้ตัวกำลังคิดไม่ซื่อกับลูก — คนที่พูดดีต่อหน้านั่นแหละ\"\n\n"
            // 🎬 (2026-09-04 owner) เดิมเขียนว่า "คลัง = เครื่องมือคิดในใจ พูดออกมาเฉพาะคำตอบ" ⇒ โมเดลอ่านเป็น
            //   "อย่าเอารูปพรรณ/วัย/โรค/เหตุการณ์ในคลังออกมาพูด" ⇒ คลังที่มีครบไม่เคยถูกใช้ (owner ทักเอง)
            //   ที่ห้ามจริงคือ *รูปแบบ* บรรทัดคลัง — *เนื้อหา* ต้องถูกหยิบมาเล่า
            ."⚠️ *บล็อกความรู้/ตำราด้านบนทั้งหมด = วัตถุดิบของคำตอบ* — หยิบ *เนื้อหา* มาเล่าได้เต็มที่\n"
            ."   (รูปพรรณ-วัยของคนที่เกี่ยว / อวัยวะ-โรค / เดินทาง-คดี-ลาภ / เลข-สี-ฤกษ์ ตามที่คลังให้มา ❌ ห้ามคิดเอง)\n"
            ."   ที่ห้ามคือ *รูปแบบ*: บรรทัดคลังที่ขึ้นต้นว่า \"ตำแหน่ง N [ชื่อตำแหน่ง] — ชื่อไพ่\" = ข้อมูลภายใน ❌ ห้ามลอกออกไปในคำตอบ\n\n"
    ...
    {
        // 👶🔥 (2026-09-03, owner) ใช้ข้อความชุดกลางร่วมกับแชท/Deep 39 — ห้ามเขียนสำนวนที่สองที่นี่
        //   owner directive: เรียกลูกค้าว่า "ลูก" / "ลูกดวง" เสมอ (แทน "เธอ / เจ้าชะตา" ของเดิม)
        //   + โทน "ดุแต่รัก" — Celtic 99 คือเลนที่ลูกค้าจ่ายแพงสุด ต้องได้บุคลิกเดียวกับเลนอื่น
        return "━━━━━━━━━━━━━━━━━\n"
            .\App\Services\FortuneAIService::CUSTOMER_ADDRESS_DIRECTIVE
            ."━━━━━━━━━━━━━━━━━\n"
            .\App\Services\FortuneAIService::FIERCE_TONE_DIRECTIVE
            ."━━━━━━━━━━━━━━━━━\n\n";
    }
    ...
    protected function buildFollowupPrompt(
        ...
        if ($previousQA->isNotEmpty()) {
            ...
            $previousContext = "📜 บริบทต่อเนื่องของรอบนี้ — คำทำนาย Q1 (พื้นดวง) + บทสนทนาที่ผ่านมา:\n"
                ...
                // 🧬 (2026-09-03) "อย่าเล่าซ้ำ" = ห้ามลอกถ้อยคำ ไม่ใช่ห้ามใช้พื้นดวง — ต้องเขียนให้ชัด
                //   ไม่งั้นโมเดลตัดผังดวงทิ้งแล้วตอบไพ่ล้วน (ต้นเหตุที่ลูกค้ารู้สึกว่าไม่สอดคล้องกัน)
                ."🎴 *แต่อย่าลอกถ้อยคำพื้นดวงมาวางซ้ำ* — คำถามใหม่ต้อง \"อ่านไพ่ใหม่ให้ตรงคำถามนั้นโดยเฉพาะ\" (เจาะไพ่ตำแหน่งที่ตอบคำถาม + ตั้งตรง/กลับหัว) ไม่ใช่ลอกประโยค/ไพ่/กรอบเวลาเดิมมาวาง\n"
                ."🌀 *แต่ยังต้องยืนบนพื้นดวงเดิมเสมอ* — ลัคนา/ภพ/ดาวเจ้าชนะ/ดาวจร ที่อ่านไปแล้วคือฐานเดียวกันทั้งรอบ คำตอบใหม่ต้องต่อยอดจากมัน ห้ามพูดขัดกับพื้นดวงที่บอกไปแล้ว\n"
        ...
        if (trim($parkedContext) !== '') {
            $previousContext .= "📌 *สิ่งที่ลูกค้าเล่า/ถาม/ส่งรูปมาไว้ก่อนหน้า* (ตอนยังเปิดไพ่ไม่ครบ หรือตอนแม่หมอขอวันเกิด/เวลาเกิด)\n"
                ."   ⚓ ถือเป็น *ข้อเท็จจริงของบิลนี้* — ลูกค้าพูดไว้จริง แต่ระบบยังตอบให้ไม่ได้ในตอนนั้น\n"
                ."   ✅ ถ้ามีคำถามค้างอยู่ในนี้ที่ยังไม่เคยตอบ → *ตอบให้ด้วยในรอบนี้* (ลูกค้าจ่ายเงินแล้วและรออยู่)\n"
                ."   ✅ ใช้ระบุตัวบุคคล/สัตว์/สถานที่ให้ถูกตัวเสมอ — ห้ามเดาใหม่ทั้งที่ลูกค้าบอกไว้แล้ว\n"
                .'   "'.trim($parkedContext)."\"\n\n";
        }
        ...
        if ($sequence >= 2) {
            ...
            $advisorDirectiveQ2 = $clapbackDirective
                ...
                // ✂️ (2026-09-04) ตัด buildElementalDignityDirective + buildPositionDynamicDirective ออกจาก Q2+
                //   = ทฤษฎี "วิธีอ่านไพ่" 4,000 ตัวอักษรที่ฉีดซ้ำทุกคำถาม ทั้งที่การอ่านเชิงธาตุ/คู่ตำแหน่ง
                //   ทำไปแล้วตอนพื้นดวง Q1 (คำตอบนั้นอยู่ใน previousContext) — บทสรุป VIP ก็ตัดออกด้วยเหตุผลเดียวกัน
                //   ตั้งแต่ 2026-08-07. เส้น Q1 fallback (long path ด้านล่าง) ยังมีครบ
        ...
        $multiBulletHandlingQ1 = $this->detectMultiBulletQuestion($userQuestion)
            ? "━━━━━━━━━━━━━━━━━\n"
                ...
                ."      เช่น \"แม่หมอเห็นลูกถามมาหลายเรื่อง ขอเปิดไพ่เรื่อง [เรื่องแรก] ให้ชัดก่อนนะคะ —\n"
        ...
        if ($reading->getConversationState('celtic_base_chart') && $this->isBlackMagicModeForced($reading)) {
            ...
            // 🃏 (2026-09-06 FTU-260906-K2263) เส้นข้ามวันเกิด — ไม่มีผังดาว
            //   เส้น fallback นี้ต้องรู้เรื่องด้วย ไม่งั้นลูกค้าที่ตกมาทางนี้ (sectioned ล้ม)
            //   จะโดนสั่งให้ "ใช้ดวงดาวเป็นแกนหลัก" ทั้งที่ไม่มีวันเกิด → มั่วราศีทั้งใบ
            $bcCardsOnly = ($birthAstroBlock === '');

            // 🕛 (2026-09-09) เส้น fallback ต้องรู้ "ฐานที่ผังใช้นับภพ" เหมือนเส้น sectioned
            //   ไม่งั้นสั่งให้เขียน "ลัคนา & เรือนชะตา" ทั้งที่ผังไม่มีลัคนา (97.8% ของบิลไม่รู้เวลาเกิด)
            //   = กับดักเดิมย้ายมาซ่อนในเส้นสำรอง ([[rule_feature_built_but_never_wired]] กลับด้าน)
            $bcLagnaBasis = 'none';
            if (! $bcCardsOnly) {
                try {
                    $bcLagnaBasis = (new \App\Services\Fortune\ThaiAstrologyService)->resolveLagnaBasis(
                        $reading->birth_date?->format('Y-m-d'),
                        $reading->birthHourFloat()
                    );
                } catch (\Throwable $e) {
            ...
            $baseChartDirective =
                ...
                ."━━━━━━━━━━━━━━━━━\n"
                .($bcCardsOnly
                    ? "นี่คือคำทำนาย \"พื้นฐานดวง\" ครั้งแรกของเจ้าชะตา — เจ้าชะตา *ไม่สะดวกบอกวันเดือนปีเกิด*\n"
                        ."⇒ อ่านจาก \"ไพ่ทั้ง 10 ใบ + ตำแหน่ง Celtic\" ล้วน ให้ลึกและฟันธงเต็มที่\n"
                        ."❌ ห้ามพูดถึง/เดา ราศี ปีนักษัตร ธาตุ ดาวเจ้าชนะ ดาวเสวยอายุ ลัคนา ภพ/เรือนชะตา — ไม่มีข้อมูล\n"
                        ."❌ ห้ามออกตัวว่าไม่มีวันเกิดเลยอ่านได้ไม่เต็มที่ — ไพ่ 10 ใบพอสำหรับพื้นดวง\n\n"
                ...
                .'📐 โครงสร้าง — *เปิดด้วยพาดหัวเรื่องเด่นก่อน* แล้วตามด้วย '.($bcCardsOnly ? '4' : '5')
                ." ส่วน (ใส่ให้ครบ • ใช้ emoji หัวข้อ • เว้นย่อหน้าให้อ่านง่าย):\n"
                ...
                .($bcCardsOnly
                    ? "  1) 🌟 พื้นฐานนิสัย&พลังประจำตัว — อ่านจากไพ่บุคคล/ไพ่ตำแหน่ง 1 (ตัวตน) และ 7 (มุมมองต่อตัวเอง): นิสัยพื้นฐาน จุดแข็ง-จุดอ่อน ภาพที่คนอื่นเห็น vs ตัวจริง\n"
                        ."     (❌ ห้ามพูดถึง ราศี/ปีนักษัตร/ธาตุ/ดาว — รอบนี้ไม่มีวันเกิด)\n"
                        ."  2) 🔮 ภาพรวมชีวิตช่วงนี้ — แกนเวลาของ Celtic: ตำแหน่ง 3/4 (อดีต) → 1/2/5 (ปัจจุบัน) → 6/9/10 (อนาคต) ร้อยเป็นเส้นเรื่องเดียว\n"
                        ."  3) 💞 ความรัก • 💼 การงาน • 💰 การเงิน • 🌿 สุขภาพ — แต่ละด้านผูก *ไพ่+ตำแหน่ง* ที่เกี่ยวข้อง ฟันธงชัด\n"
                        ."  4) 🍀 โชคลาภ — *คนละเรื่องกับการเงิน*: ลาภลอย/เงินก้อนนอกแผน/รางวัล/มรดก/คนอุปถัมภ์ — อ่านจากไพ่ที่ชี้เรื่องลาภ + ตำแหน่ง 9 (สิ่งที่กำลังมา) และ 10 (ผลลัพธ์)\n"
                        ."     ฟันธง: มีเกณฑ์ไหม/มาทางไหน/ช่วงไหน/มากน้อย — ไม่มีก็บอกตรง ❌ ห้ามฟันเลขหวย-หุ้นเป๊ะ ❌ ห้ามการันตีถูกรางวัล ❌ ห้ามยุให้กู้-ลงทุนเสี่ยง\n"
                    : "  1) 🌟 พื้นฐานดวงจากดวงดาว — ราศี/ปีนักษัตร/ธาตุ + ดาวเจ้าชนะ/ดาวมิตร/ดาวศัตรู + นิสัยพื้นฐาน (จุดแข็ง-จุดอ่อน)\n"
                        ...
                        // 🕛 (2026-09-03, owner) เซคชั่นใหม่ต้องมีในเส้น fallback ด้วย — ไม่งั้นลูกค้าที่ตกมาเส้นนี้
                        //   (sectioned ล้ม / โมเดลไม่ผ่านเกณฑ์ must) จะไม่ได้เซคชั่นที่ owner สั่งเลย
                        .match ($bcLagnaBasis) {
                            'lagna' => "  3) 🕛 ลัคนา & เรือนชะตา — ลัคนาราศีอะไร (ภาพที่คนอื่นเห็น vs ตัวจริง) + ดาวเด่นตกภพไหน (สนามชีวิตที่หนักสุด) + จังหวะเวลาจากเรือนชะตา\n"
                                ."     (ผังคำนวณจากเวลาเกิดจริงที่เจ้าชะตาบอกแล้ว ❌ ห้ามข้าม ❌ ห้ามออกตัว)\n",
                            'moon' => "  3) 🕛 จันทร์ลัคน์ & เรือนชะตา — เจ้าชะตายังไม่บอกเวลาเกิด ⇒ ตำราให้ยกราศีจันทร์เป็นภพที่ 1\n"
                                ."     จันทร์ลัคน์ราศีอะไร (ใจข้างใน/สัญชาตญาณ) + ดาวเด่นตกภพไหน + จังหวะเวลาจากเรือนชะตา\n"
                                ."     ✅ เรียก \"จันทร์ลัคน์\" เท่านั้น ❌ ห้ามเรียกว่าลัคนา ❌ ห้ามพูดถึงเวลาตกฟาก ❌ ห้ามข้าม\n",
                            // ไม่มีฐานให้นับภพ → บอกให้ข้าม *แต่ต้องคงเลขข้อไว้* ไม่งั้นรายการเป็น 1,2,4,5
                            // แล้วโมเดลจะไปเค้นหา "ข้อ 3" มาเติมเอง = มโนลัคนา (กับดักเดิมย้ายที่)
                            default => "  3) (ข้ามข้อนี้ — ผังไม่มีลัคนา/ภพ เพราะไม่ทราบเวลาเกิดและจันทร์ย้ายราศีวันนั้น)\n"
                                ."     ❌ ห้ามเขียนเรื่องลัคนา/เรือนชะตา/ภพ ❌ ห้ามเดาราศีลัคนา — ข้ามไปข้อ 4 ได้เลย\n",
                        }
                    ."  4) 💞 ความรัก • 💼 การงาน • 💰 การเงิน • 🌿 สุขภาพ — แต่ละด้านผูกดาว(ภพ)+ไพ่ที่เกี่ยวข้อง ฟันธงชัด\n"
                    // 🍀 (2026-09-03, owner) โชคลาภต้องมีในเส้น fallback ด้วย — ไม่งั้นลูกค้าที่ตกมาเส้นนี้
                    //   (sectioned ล้ม / โมเดลไม่ผ่านเกณฑ์ must) จะไม่ได้เซคชั่นที่ owner สั่งเลย
                    ."  5) 🍀 โชคลาภ — *คนละเรื่องกับการเงิน*: ลาภลอย/เงินก้อนนอกแผน/รางวัล/มรดก/คนอุปถัมภ์ — อ่านจากภพศุภะ(9)+ลาภะ(11)+พฤหัสบดี\n"
                    ."     ฟันธง: มีเกณฑ์ไหม/มาทางไหน/ช่วงไหน/มากน้อย — ไม่มีก็บอกตรง ❌ ห้ามฟันเลขหวย-หุ้นเป๊ะ ❌ ห้ามการันตีถูกรางวัล ❌ ห้ามยุให้กู้-ลงทุนเสี่ยง\n")
                ...
                .($bcCardsOnly
                    ? "🃏 ต้องอ้างถึงไพ่ครบทั้ง 10 ใบ (ตามตำแหน่ง Celtic) — ห้ามข้ามใบ\n"
                ...
                // 🍀 (2026-09-03) +wealth — เส้นนี้มีหัวข้อ 5) โชคลาภ แล้ว ต้องมีคลังรายไพ่ให้อ่าน ไม่ใช่ให้มโน
                .$this->buildBaseChartSignalKnowledge($reading, ['black_magic', 'health', 'combo', 'wealth']);
        ...
        $q1Classifier =
            ...
            ."🚫 *ห้ามเดาเรื่องที่ลูกยังไม่ได้บอก* — ถ้ายังไม่รู้ว่าจะถามอะไร (เช่น ทักว่า \"พร้อม\") → `[TYPE:C]` ถามกลับ ห้ามสุ่มทำนายเรื่องรัก/งาน/เงินเอง (ฟอร์มทำนายเต็มใช้กับ [TYPE:A] เท่านั้น)\n"
        ...
        return $personaPrefix
            ...
            .$birthAstroBlock
            // 🧬 (2026-09-03) ผังดวงมาแล้วต้องมีกติกาใช้ต่อทันที ไม่งั้นโมเดลอ่านไพ่ล้วนเหมือนเดิม
            .($birthAstroBlock !== '' ? $this->buildFusionDoctrine() : '')
            ...
            ."   • สะท้อนสถานการณ์ เช่น \"จากสิ่งที่ลูกเจอมา…\" / \"แม่หมอรับรู้ได้ว่าลูกกำลัง…\"\n\n"
            ...
            ."   • **คำถามให้ลูกค้าถามตัวเอง 1 ข้อ** (\"ถ้าเป็นเพื่อน ลูกจะแนะนำให้รอไหม?\")\n"
    ...
    protected function buildNextQuestionsDirective(bool $blackMagicForced = false): string
    {
        ...
        $framing = $blackMagicForced
            ...
            : "หลังจบคำทำนาย ให้เสนอ \"คำถามที่เจ้าชะตาควรถามต่อ\" 2 ข้อ ที่ *ต่อยอดเรื่องเดิมที่กำลังดูอยู่*:\n"
                ...
                ."   • เขียนเป็นคำถามธรรมชาติจากปากลูก (เช่น \"ความสัมพันธ์นี้จะไปถึงขั้นแต่งงานไหม\")\n"
                ."   • แต่ละข้อสั้นกระชับ ≤ 60 ตัวอักษร\n\n";
    ...
    /** 📸 เพดานจำนวนรูปที่ "อ่านเก็บไว้เป็นบริบท" ได้ต่อบิล (กันค่า vision บานปลาย) */
    public const MAX_PENDING_IMAGE_CAPTURES = 3;

    /**
     * 📸 (2026-09-04, owner) อ่านรูปเก็บไว้เป็น "บริบท" ตั้งแต่จ่ายเงินสำเร็จ — ยังไม่ต้องตอบคำถาม
     *
     * owner directive: *"การเปิดระบบดูภาพ และรับคำพูด ควรเปิดตั้งแต่จ่ายเงินสำเร็จเลยดีกว่าไหม
     *   จะได้ไม่พลาด"*
     *
     * ใช้กับช่วงที่ยัง **ตอบคำถามด้วยไพ่ไม่ได้**:
     *   • ยังเปิดไพ่ไม่ครบ 10 ใบ (celtic_picking)
     *   • เปิดครบแล้วแต่ระบบกำลังขอวันเกิด / เวลาเกิด
     *
     * ต่างจาก askQuestionWithImage(): ตัวนั้น "ตอบคำถามจากรูป + กินโควต้าคำถาม"
     *   ตัวนี้แค่ **บรรยายว่าเห็นอะไร** แล้ว park ไว้ (ไม่กิน sequence ไม่สร้าง record คำถาม)
     *   ⇒ พอถึงรอบทำนายจริง แม่หมอรู้แล้วว่ารูปนั้นคือ *น้องหมาพันธุ์อะไร* ไม่ใช่คน
     *     (เคส FTU-260904-S9843 — รูปน้องหมาถูกตีกลับที่ด่านวันเกิดโดยไม่มีใครเคยเปิดดู)
     *
     * @param  string  $imageData  URL (FB CDN) หรือ base64 data URL (LINE)
     * @return array{captured:bool, summary:?string, reason:?string}
     */
    public function captureImageAsContext(FortuneReading $reading, string $imageData, string $userText = ''): array
    {
        // 💰 เฉพาะบิลที่จ่ายแล้วเท่านั้น — ห้ามเปิด vision ให้เส้นฟรี (โควต้า/ค่าใช้จ่าย)
        if (! $reading->is_paid) {
            return ['captured' => false, 'summary' => null, 'reason' => 'unpaid'];
        }

        $used = (int) $reading->getConversationState('celtic_image_captures', 0);
        if ($used >= self::MAX_PENDING_IMAGE_CAPTURES) {
            return ['captured' => false, 'summary' => null, 'reason' => 'cap_reached'];
        }

        try {
            $aiService = new FortuneAIService($this->settings);

            $systemPrompt = "คุณคือผู้ช่วยที่ดูรูปแล้วบรรยาย *ข้อเท็จจริงที่เห็น* อย่างสั้นที่สุด\n"
                ."ตอบเป็นภาษาไทย 1-2 ประโยค ไม่เกิน 200 ตัวอักษร ห้ามทำนายดวง ห้ามตีความ ห้ามทักทาย\n\n"
                ."ระบุให้ชัดว่าในรูปคืออะไร เลือก 1 อย่าง:\n"
                ."• 🐾 สัตว์เลี้ยง → ชนิด (หมา/แมว/นก/ปลา...) + *พันธุ์ที่พอเดาได้* + สีขน/ลาย + ขนาด-วัย + ปลอกคอ/สายจูง\n"
                ."• 👤 คน → กี่คน เพศ ช่วงวัย จุดเด่นของรูป\n"
                ."• 🏠 สถานที่/บ้าน · 🧿 วัตถุมงคล/สิ่งของ · 📄 เอกสาร-หน้าจอ · 🧾 สลิปโอนเงิน · 😀 สติกเกอร์/อิโมจิ/มีม\n\n"
                .'ขึ้นต้นด้วยอิโมจิของหมวดที่เลือก แล้วบรรยายต่อในบรรทัดเดียว';

            $userPrompt = $userText !== ''
                ? 'ลูกค้าส่งรูปนี้มาพร้อมข้อความ: "'.mb_substr($userText, 0, 120).'" — บรรยายสิ่งที่เห็นในรูป'
                : 'บรรยายสิ่งที่เห็นในรูปนี้';

            $result = $aiService->chatWithImage(
                $imageData,
                $systemPrompt,
                $userPrompt,
                // ถูก + สั้น — งานนี้แค่ "บอกว่าเห็นอะไร" ไม่ใช่ทำนาย
                ['temperature' => 0.2, 'max_tokens' => 300, 'bypass_vision_gate' => true]
            );

            $summary = trim((string) ($result['response'] ?? ''));
            if ($summary === '') {
                return ['captured' => false, 'summary' => null, 'reason' => 'vision_empty'];
            }

            $summary = mb_substr($summary, 0, 220);
            $note = '[รูปที่ลูกค้าส่งมา] '.$summary;
            if ($userText !== '') {
                $note .= ' (พิมพ์มาด้วยว่า: '.mb_substr(trim($userText), 0, 80).')';
            }

            $reading->parkPendingContext($note);
            $reading->setConversationState('celtic_image_captures', $used + 1);

            Log::info('📸 Celtic: อ่านรูปเก็บเป็นบริบทระหว่างรอขั้นตอน (ไม่กินโควต้าคำถาม)', [
                'reading_id' => $reading->id,
            ...
            return ['captured' => true, 'summary' => $summary, 'reason' => null];
        } catch (\Throwable $e) {
            Log::warning('📸 Celtic: อ่านรูปเก็บเป็นบริบทล้มเหลว (non-blocking)', [
                'reading_id' => $reading->id,
                'error' => $e->getMessage(),
            ]);

            return ['captured' => false, 'summary' => null, 'reason' => 'exception'];
        }
    }
    ...
    public function askQuestionWithImage(FortuneReading $reading, string $imageData, string $userText = ''): array
    {
        ...
        if ((bool) $reading->getConversationState('celtic_birthdate_pending', false)) {
            ...
            //   อ่านรูปเก็บเป็นบริบทก่อน แล้วค่อยตีกลับไปขอวันเกิด — ไม่กินโควต้าคำถาม
            //   เคส FTU-260904-S9843: รูปน้องหมาถูกด่านนี้ตีกลับโดยไม่มีใครเคยเปิดดู
            //   แล้วลูกค้าเข้าใจว่าแม่หมอเห็นแล้ว จึงเล่าต่อว่า "น้อง..." เฉย ๆ
            $captured = $this->captureImageAsContext($reading, $imageData, $userText);
            $keptLine = $captured['captured']
                ? "📸 แม่หมอ *เก็บรูปที่ลูกส่งมาไว้แล้ว* นะคะ ไม่หายไปไหน\n\n"
                : "📸 รูปที่ลูกส่งมา แม่หมอยังเปิดดูไม่ได้ตอนนี้ค่ะ — เดี๋ยวส่งมาใหม่อีกทีนะคะ\n\n";

            return [
                'success' => false,
                'message' => $keptLine
                    ."🌙 ขอเปิด *พื้นดวง* ให้ก่อนนะคะ —\n"
                    ."ขอ *วัน/เดือน/ปีเกิด* ของลูก (เช่น 14/02/2540)\n"
                    .'หรือพิมพ์ *"ข้าม"* ถ้าไม่สะดวก แม่หมอจะเปิดพื้นดวงจากไพ่ให้เลยค่ะ ✨',
            ];
        }

        // 🕛 (2026-09-03) กันรูป/สติกเกอร์แย่ง "พื้นดวง Q1" ตอนระบบรอ *เวลาเกิด* — เหตุผลเดียวกับด่านบน
        //   ขั้นเวลาเกิดจบด้วย "ข้อความ" เท่านั้น (handleCelticBirthTimeStep) → ถ้าปล่อยรูปผ่าน
        //   จะกลายเป็นคำถาม vision กิน sequence แล้วพื้นดวงเปิดตัวไม่เคยถูกสร้าง (เคสเดียวกับ FTU-260625-Q9276)
        if ((bool) $reading->getConversationState('celtic_birthtime_pending', false)) {
            // 📸 (2026-09-04, owner) เหตุผลเดียวกับด่านวันเกิด — อ่านรูปเก็บก่อนตีกลับ
            $capturedT = $this->captureImageAsContext($reading, $imageData, $userText);
            $keptLineT = $capturedT['captured']
                ? "📸 แม่หมอ *เก็บรูปที่ลูกส่งมาไว้แล้ว* นะคะ ไม่หายไปไหน\n\n"
                : "📸 รูปที่ลูกส่งมา แม่หมอยังเปิดดูไม่ได้ตอนนี้ค่ะ — เดี๋ยวส่งมาใหม่อีกทีนะคะ\n\n";
            ...
            return [
                'success' => false,
                'message' => $keptLineT
                    ."🕛 ขอ *เวลาเกิด* ของลูกก่อนนะคะ — พิมพ์เป็นข้อความมาได้เลย\n"
    ...
    protected function buildShortFollowupPrompt(
        ...
        return $personaPrefix
            .$this->buildCardFirstMandate(withAstro: $birthDateAstrology !== '')
            .$this->buildCurrentDateContext()
            .$this->buildCardTalkPolicy($userQuestion)
            .$preChatContext
            .$advisorDirective
            ...
            ."       ห้ามใส่ทุก turn — ใส่เมื่อเรื่องมีน้ำหนักจริงๆ (\"ถ้าเป็นเพื่อน ลูกจะแนะนำให้รอไหม?\")\n\n"
            ...
            ."   ถ้าไม่ชัดว่าลูกอยากให้ดู \"เรื่องอะไร / ใคร / ช่วงเวลาไหน\" — เช่น \"ดูให้หน่อย\" \"แล้วยังไงต่อ\" \"จริงเหรอ\" \"เอาไงดี\" ลอย ๆ ไม่มีหัวข้อ\n"
            ...
            ."   ✅ ถูก: \"เพราะที่ผ่านมาลูกเคยเจ็บมา → ตอนนี้กลัวเจ็บอีก\n"
            ...
            ."      ตัวอย่าง: \"ลูก ขอวันเดือนปีเกิดได้ไหมคะ? แม่หมอจะเช็คดาวเจ้าชนะให้ช่วงเวลาแม่นขึ้น\"\n"
            ...
            .$previousContext
            .$birthDateAstrology
            // 🧬 (2026-09-03 owner) Q2+ คือจุดที่ "พื้นดวงหาย" มากที่สุด — ผังถูกส่งมาตั้งแต่ 2026-05-30
            //   แต่กฎเหล็กด้านบนบอกว่าไพ่คือแหล่งความจริงเดียว 100% + บรรทัด "ห้ามเล่าซ้ำพื้นดวง Q1"
            //   ⇒ โมเดลตีความว่า "ทิ้งผังดวง อ่านไพ่พอ" ⇒ คำตอบรายข้อไม่ต่อกับพื้นดวงที่เพิ่งอ่านไป
            .($birthDateAstrology !== '' ? $this->buildFusionDoctrine() : '')
            ...
            .$this->buildAntiFillerBans()."\n"
            // 🧬 (2026-09-03) เดิมเขียนว่า "ห้ามเล่าซ้ำพื้นดวง" เฉยๆ → โมเดลอ่านเป็น "ห้ามใช้พื้นดวง"
            //   แล้วตัดผังทิ้งทั้งก้อน ⇒ คำตอบรายข้อลอยจากพื้นดวงที่ลูกค้าเพิ่งอ่านไป
            //   ที่ห้ามจริงคือ "ลอกถ้อยคำเดิม" ไม่ใช่ "เลิกยืนบนผังเดิม"
            .'• ❌ ห้าม *ลอกถ้อยคำ* พื้นดวง Q1 หรือไพ่/กรอบเวลาเดิมมาวางซ้ำทุกคำถาม — อ่านไพ่ใหม่ให้ตรงคำถาม '
            .'(✅ แต่ยังต้องยืนบนผังดวงเดิมเสมอ — ต่อยอดจากพื้นดวง ไม่ใช่เริ่มใหม่จากศูนย์ และห้ามพูดขัดกับพื้นดวงที่บอกไปแล้ว)'
            .$offTopicGuard
            .$sandbagBlock."\n\n"
    ...
    protected function buildEnrichmentDirective(FortuneReading $reading, string $userQuestion): string
    {
        ...
        return "━━━━━━━━━━━━━━━━━\n"
            ...
            ."   เช่น \"แม่หมอเห็นไพ่บอกเรื่อง X กับ Y ชัด — ลูกอยากให้แม่หมอเจาะลึกเรื่องไหนเป็นพิเศษ?\"\n"
    ...
    protected function buildLifeCoachDirective(FortuneReading $reading): string
    {
        ...
        return "━━━━━━━━━━━━━━━━━\n"
            ...
            ."  • **ใช้ framework** เช่น \"ลูกกำลังให้ค่าตัวเองจากเขา\" / \"นี่คือ pattern เดิม\" /\n"
            ."     \"3 สิ่งที่ลูกควบคุมได้ vs 1 สิ่งที่ควบคุมไม่ได้\"\n"
    ...
    /**
     * 📅 (2026-09-21) ปฏิทินโหรไทยสำหรับบทสรุป — วันมงคลจริงจากตำรา (พอร์ตจาก GenLotto)
     *
     * บทสรุปมีย่อหน้า "วัน/ช่วงเวลามงคล" ทุกบท แต่เดิมไม่มีตัวเลขป้อนให้ ⇒ AI เดาวันเอง
     * ตอนนี้: ปฏิทินหลวงวันนี้ + กาลโยคปีนี้ + วันมงคล 30 วันข้างหน้า (ถามฤกษ์งานเฉพาะ = ฤกษ์งานนั้น 90 วัน)
     *   ตัดวันกาลกิณีของเจ้าชะตาออกเมื่อรู้วันเกิด
     *
     * ⚠️ ฉีดเฉพาะบทสรุป — รอบถามตอบมีคำสั่งเจ้าของ "ของท้าย (สี-ฤกษ์มงคล) ยกไปบทสรุป" (buildExtrasDeferDirective)
     * ⚠️ ตรวจเจตนาจาก $allQuestionText = คำถามลูกค้าล้วน (ไม่ใช่คำตอบ AI ที่พูดคำว่าฤกษ์เอง)
     */
    protected function buildThaiCalendarDirective(FortuneReading $reading, string $allQuestionText): string
    {
        try {
            $birth = $reading->birth_date;
            if (empty($birth)) {
                // บิล 99 เก็บวันเกิดเป็นข้อความ (celtic_birthdate_text) — ใช้วันแรกที่พบ = วันเกิดเจ้าชะตา
                $dates = (new ThaiAstrologyService)->extractBirthDatesFromText(
                    (string) $reading->getConversationState('celtic_birthdate_text', '')
                );
                $birth = isset($dates[0]['ymd']) ? \Carbon\Carbon::parse($dates[0]['ymd']) : null;
            }
            $hour = $reading->birthTimeIsKnown() ? (float) $reading->birthHourFloat() : null;
            $person = \App\Services\Fortune\AuspiciousTimingDirective::personFromBirth($birth, $hour);

            return (new \App\Services\Fortune\AuspiciousTimingDirective)->generalBlock($allQuestionText, $person);
        } catch (\Throwable $e) {
            Log::warning('Celtic: ปฏิทินโหรไทยล้ม — บทสรุปไม่มีวันมงคลจริง', [
                'reading_id' => $reading->id,
                'error' => $e->getMessage(),
            ]);
    ...
    protected function buildSaiMuDirective(FortuneReading $reading, string $userQuestion = ''): string
    {
        ...
        return "━━━━━━━━━━━━━━━━━\n"
            ...
            // 📅 (2026-09-21) เดิมสั่ง "เลือกวันธงชัย-อธิบดี เลี่ยงวันอุบาทว์-โลกาวินาศ" โดยไม่บอกว่าปีนี้คือวันไหน = ให้ AI เดา
            //   ตอนนี้บทสรุป (ที่เดียวที่เรียกตำรานี้) มีบล็อก 📅 ปฏิทินโหรไทย ที่คำนวณกาลโยค/ฤกษ์บน/ฤกษ์ล่างจริงแล้ว
            ."   ฤกษ์: วัน-เวลามงคลหยิบจากบล็อก 📅 ปฏิทินโหรไทย (คำนวณกาลโยค/ฤกษ์จริงแล้ว) เท่านั้น — ไม่มีบล็อกนั้น = พูดหลักกว้าง ห้ามระบุวันเอง\n"
    ...
    protected function buildBlackMagicDirective(FortuneReading $reading, string $userQuestion, string $previousContext = ''): string
    {
        ...
        if (! $forced) {
            // Detect: คำถามนี้ หรือคำถามก่อนหน้าของ *ลูกค้า* (3 ข้อล่าสุด) เกี่ยวคุณไสย์ไหม
            //
            // 🐛 (2026-09-04) เดิมสแกน $previousContext = *คำตอบเก่าของ AI* ด้วย + มีคีย์เวิร์ด 'เสน่ห์' เปล่าๆ
            //   ⇒ พื้นดวง Q1 เขียนว่า "เลขชะตา 3 ทำให้มี**เสน่ห์** พูดดี" (ประโยคปกติของดวงความรัก)
            //   ⇒ ด่านติด ⇒ ฉีดตำราคุณไสย์เต็ม **7,100 ตัวอักษร ทุกคำถาม** ให้บิลที่ถามเรื่องแฟน
            //   (วัดจริง rd12214 "แฟนคิดยังไงกับหนู" — 7.1k จาก 61.5k ของทั้งพรอมต์คือบล็อกนี้)
            //   แก้: สแกนเฉพาะสิ่งที่ลูกค้าพิมพ์ (celticTopicContext เหมือนตำราอื่น) + 'เสน่ห์' ต้องมีกริยา
            //   "lock รอบ" ของโหมดคุณไสย์จริงยังอยู่ที่ isBlackMagicModeForced ด้านบน — ไม่กระทบ
            $haystack = mb_strtolower($userQuestion.' '.$this->celticTopicContext($reading, $userQuestion));
            $keywords = [
                'คุณไสย', 'มนต์ดำ', 'มนตร์ดำ', 'ทำของ', 'โดนของ', 'ลงของ', 'ปล่อยของ', 'โดนคุณ',
                'ทำเสน่ห์', 'ลงเสน่ห์', 'โดนเสน่ห์', 'เสน่ห์ยาแฝด', 'ยาแฝด', 'ยาสั่ง', 'ของกิน', 'ของฝัง', 'อาคม', 'ลงเลข', 'ลงยันต์', 'เลขยันต์',
        ...
        return $header.$openBlock
            ...
            // 🪬 (2026-09-05 owner) บอก *ชื่อบท* ได้ — ❌ ห้ามพิมพ์ *ตัวบท*
            ."   📿 *อ้างหลักครูบาอาจารย์/พระเกจิที่มีจริง*: บอกได้แค่ *ชื่อบท* ที่เป็นที่รู้จัก (อิติปิโส/พาหุงฯ ฯลฯ) แล้วให้ไปเปิดจากหนังสือสวดมนต์หรือขอจากพระ — 🚫 ❌ ห้ามพิมพ์ตัวบทสวด/คาถาให้เด็ดขาด ❌ ห้ามมั่วชื่อหลวงพ่อ/สำนัก/คาถาที่ไม่มีจริง ไม่แน่ใจ → แนะแนวทางกลางที่ถูกต้อง\n"
    ...
    protected function blackMagicRagKnowledgeBlock(FortuneReading $reading): string
    {
        ...
        return "━━━━━━━━━━━━━━━━━\n"
            ...
            ."   • ใบที่เป็น ✅ เกราะ = จุดแข็ง/บุญคุ้มของลูก → ใช้บอกว่ารอดตรงไหน มีอะไรหนุน\n"
    ...
    protected function buildMuKnowledgeDirective(FortuneReading $reading, string $userQuestion, string $previousContext = '', array $onlyPositions = []): string
    {
        ...
        if (in_array(\App\Models\FortuneKnowledge::CATEGORY_DEITIES, $categories, true)
            ...
            // 🕉️ (2026-09-08 owner) องค์เทพต้องมาจาก "ตำราวันเกิด" ก่อน — ไพ่เป็นตัวสำรอง
            //   เลนแชทนี้ต้องตอบให้ตรงกับบทสรุปพื้นดวง ไม่งั้นลูกค้าถามซ้ำแล้วได้คนละองค์
            //   ([[rule_feature_built_but_never_wired]] — ต่อสายให้ครบทั้ง 2 จุดที่พูดเรื่ององค์)
            $deityService = app(\App\Services\Fortune\PatronDeityDoctrine::class);
            $doctrine = $deityService->resolveForReading($reading);

            // 🎯 (2026-09-08 owner) ชั้นที่ 3 — องค์ที่เด่นเฉพาะ "เรื่องที่ถามรอบนี้"
            //   ใช้ข้อความลูกค้าตรง ๆ ไม่ใช่บริบททั้งบทสนทนา (ยิ่งกว้างยิ่งจับเรื่องมั่ว)
            //   เลนนี้เท่านั้นที่มีคำถามสด — บทสรุปพื้นดวง (g2) เป็น boilerplate ของระบบ
            //   ไม่มีเรื่องให้จับ จึงมีแค่ 2 ชั้น (วันเกิด+ธาตุ) โดยตั้งใจ
            $topicHit = $deityService->topicDeities($userQuestion, (array) ($doctrine['all'] ?? []));

            $deitiesBlock = "\n🕉️ *ถามองค์เทพประจำตัว/อุปถัมภ์ → ฟันธงให้ชัด ห้ามปัด (ลูกค้าจ่าย 99฿ + คนลองของ ต้องตอบได้):*\n"
                .($doctrine !== null
                    ? $deityService->knowledgeBlock($doctrine)
                        ."• ⭐ ยึด *องค์ประธาน + องค์เสริม* ด้านบน (คำนวณจากวันเกิดจริงตามตำรา) — บูชาได้หลายองค์พร้อมกัน\n"
                        ."  ❌ ห้ามเปลี่ยนตัวองค์ตามหน้าไพ่ · ไพ่บอกได้แค่ว่ารอบนี้ท่านหนุนด้านไหนเป็นพิเศษ\n"
                    : "• เจ้าชะตายังไม่ได้บอกวันเกิด → รอบนี้อ่านองค์จากหน้าไพ่ แล้วชวนบอก ว/ด/ป เกิด (มีเวลาเกิดยิ่งดี)\n"
                        ."  เพื่อชี้องค์ประจำตัว *ตามตำราวันเกิด* ให้แม่นกว่านี้\n")
                .$deityService->topicBlock($topicHit)
                .(empty($topicHit['deities']) ? ''
                    : "• 🎯 องค์ในบล็อกนั้นให้แนะเป็น *องค์เสริมเฉพาะเรื่องที่ถาม* ต่อจากองค์ประธาน\n"
                        ."  (บอกด้วยว่าบูชาควบกันได้ ไม่ต้องเลิกบูชาองค์เดิม) ❌ ห้ามยกมาแทนองค์ประธาน\n")
                ."• *มีองค์อุปถัมภ์ไหม* → ตอบว่า \"มี\" เสมอ ❌ ห้ามตอบ \"ไม่มี/แล้วแต่ศรัทธา\"\n"
                ."• *องค์ไหน + ใหญ่หรือเล็ก* → ระบุชื่อองค์ + บอกระดับ: มหาเทพ (ตรีมูรติ/พระศิวะ/พระพรหม/พระนารายณ์/พิฆเนศ/อุมา-ทุรคา) / เทพนพเคราะห์ประจำวันเกิด / เทพชั้นรอง-เทพท้องถิ่น / ครูบา-สิ่งศักดิ์สิทธิ์ (นางกวัก/พญานาค/พญาครุฑ/สีวลี)\n"
                // 🪬 (2026-09-05 owner) ตัด "คาถา" ออกจากลิสต์ที่สั่งให้คาย — ให้บอกชื่อบทแทน
                ."• *บูชาเปิดดวงอย่างไร* → เจาะจงตามองค์นั้น (จากคลังด้านบน): ของบูชา + วันไหว้ + วิธีตั้งจิต + อานิสงส์ที่จะเปิด (โชคลาภ/การงาน/เมตตามหานิยม/แคล้วคลาด)\n"
                ."  🚫 *ตัวบทคาถา ห้ามพิมพ์ให้* — บอกได้แค่ชื่อบท แล้วให้ไปขอของจริงจากสำนัก/พระ/หนังสือสวดมนต์\n"
    ...
    protected function buildExtraKnowledgeDirectives(FortuneReading $reading, string $userQuestion, string $previousContext = '', array $forceGroups = [], array $onlyPositions = []): string
    {
        ...
        $groups = [
            ...
            ['enable_celtic_auspicious', 'detectAuspiciousCategories', '📅 ฤกษ์ยาม/วันมงคล',
                // 📅 (2026-09-21) วันเป๊ะมีแหล่งจริงแล้ว (บล็อกปฏิทินโหรไทยในบทสรุป) — กฎเดิม "ห้ามวันเป๊ะ" จะขัดกับบล็อกนั้น
                '• ฤกษ์จากไพ่ = "ช่วงจังหวะที่หนุน" (อ่านจากตำแหน่ง+ธาตุไพ่) — วัน-เวลาเป๊ะหยิบจากบล็อก 📅 ปฏิทินโหรไทย (ถ้ามี) เท่านั้น ❌ ห้ามแต่งวันเป๊ะเอง'],
     ...
     *   🔮 (2026-09-21) ไพ่กลับหัวไม่นับคะแนน (เดิมพลิกสัญลักษณ์ → หอคอยกลับหัว = "ใช่ชัด") — แม่หมออ่านความหมายกลับหัวเอง
     *
     * Inject: buildFollowupPrompt (Q1) + buildShortFollowupPrompt (Q2+) + buildGrandFinalePrompt
     */
    protected function buildYesNoDirective(FortuneReading $reading, string $userQuestion, string $previousContext = ''): string
    {
        ...
        return "━━━━━━━━━━━━━━━━━\n"
            ...
            // 🔮 (2026-09-21) ไพ่กลับหัวไม่นับในตาชั่ง — ใบสำคัญที่สุดอาจอยู่นอกคะแนน ต้องบอกให้ชั่งเอง
            ."• 🔄 ไพ่ที่เขียนว่า \"ไม่นับคะแนน\" คือไพ่กลับหัว — ถ้าเป็นต.6 หรือ ต.10 ให้ชั่งความหมายกลับหัวของใบนั้น (มักเป็นติดขัด/ล่าช้า/ยังไม่ใช่ตอนนี้) ประกอบการฟันธงด้วย\n"
            ."━━━━━━━━━━━━━━━━━\n\n";
    }
    ...
    protected function buildBirthAstrologyBlockFor(
        ...
        //
        // ⚠️ (2026-09-03) ต้องใช้ `birthTimeIsKnown()` ไม่ใช่ `birthHourFloat() !== null`
        //    ตั้งแต่เก็บค่ามาตรฐาน 12:00 ลงคอลัมน์ birth_time **ทุกบิลมีค่าเสมอ**
        //    ถ้าเช็คแค่ว่ามีค่าไหม ประโยคนี้จะถูกแปะให้ทุกคนว่า "เจ้าชะตาเกิดเวลา 12:00 น."
        //    ⇒ แม่หมออ้างเวลาเกิดที่ลูกค้าไม่เคยบอก ทั้งเลน 99 (เทียบเคส FTU-260903-X0866)
        $confirmedHour = null;
        if ($reading->birthTimeIsKnown()) {
            $confirmedHour = (float) $reading->birthHourFloat();
            $source .= "\nเจ้าชะตาเกิดเวลา ".FortuneReading::hourToTimeString($confirmedHour, false).' น.';
        }
    ...
    protected function buildGrandFinalePrompt(
        ...
        if ($pendingBlock !== '') {
            $pendingBlock = "⏳ *คำถามที่เจ้าชะตาถามทิ้งไว้แต่ยังไม่ได้รับคำตอบ* (รอบปิดก่อน):\n"
                ...
                ."   ❌ ห้ามพูดถึงเรื่อง \"หมดเวลา/ตอบไม่ทัน/ไว้คราวหน้า\" — ลูกจ่ายแล้ว ต้องได้คำตอบตรงนี้\n"
                ."━━━━━━━━━━━━━━━━━\n\n";
        }
        ...
        // 🎬 คำนวณบล็อกซีรี่ส์ก่อน — ใช้ตัดสินว่าจะใช้ตำราเต็ม (โหงวเฮ้ง+ตำแหน่งบุคคล) เป็นทางถอยไหม
        $finaleStoryBlock = $this->buildStoryModeDirective($reading, $allQuestionText, isFinale: true);
        ...
        return $localeDirective
            .$this->buildCardNamingDirective()
            .$this->buildCardFirstMandate(lean: true, withAstro: $finaleAstroBlock !== '')
            ...
            // 🎬 (2026-09-04 owner) โหมดซีรี่ส์แทนที่ physiognomy + person_role เต็ม 10 ใบ (7,000 ตัวอักษร
            //   ที่ยิงเมื่อ allQuestionText มีคำว่า "แฟน/หน้าตา" แต่ถูกสั่งให้เก็บไว้ในใจ = ไม่เคยถูกใช้)
            //   บทสรุป = ตอนจบของซีรี่ส์ → ข้ามด่านคำถาม ฉีดเสมอเมื่อไพ่มีสัญญาณ (~3k, เฉพาะตำแหน่งตัวละคร/เหตุการณ์)
            //
            // ⚠️ (2026-09-04 บทเรียนจาก FTU-260904-V5241) ต้องมี *ทางถอย* — ห้ามแทนที่แบบไม่มีเงื่อนไข
            //   บิลนั้นโดนตีตราผิดว่าเป็น "ดวงสุนัข" ⇒ ซีรี่ส์คืนค่าว่าง ⇒ บทสรุปเลย **ไม่ได้ทั้งซีรี่ส์
            //   และไม่ได้ทั้งตำราเดิม** = ไม่มีความรู้เรื่องคน/ลักษณะเลยสักบรรทัด (แย่กว่าก่อนแก้)
            //   ซีรี่ส์ว่างด้วยเหตุใดก็ตาม → ถอยไปใช้ตำราเต็มแบบเดิม
            .($finaleStoryBlock !== ''
                ? $finaleStoryBlock
            ...
            // 📅 (2026-09-21) ปฏิทินโหรไทยจริง — ย่อหน้า "วัน/ช่วงเวลามงคล" ท้ายบทมีทุกบท
            //   เดิมไม่มีตัวเลขจริงป้อนให้เลย ⇒ AI เดาวันมงคลเอง (ดู buildThaiCalendarDirective)
            .$this->buildThaiCalendarDirective($reading, $allQuestionText)
            ...
            ."   • ❌ *ห้ามขัดกับคำตอบเดิม* (เคยบอกว่า \"ได้\" แล้วบทสรุปบอก \"ไม่ได้\" = ลูกจับได้ทันที เสียความน่าเชื่อถือทั้งรอบ)\n"
            ...
            .$pendingBlock
            .$finaleAstroContext
            // 🧬 (2026-09-03) บทสรุป VIP = บทที่ลูกค้าเก็บไว้อ่านซ้ำ/ทำเสียงอ่าน — ต้องเป็นเนื้อเดียวที่สุด
            .($finaleAstroBlock !== '' ? $this->buildFusionDoctrine(lean: true) : '')
            .$deepContext
            ."📆 บริบทช่วงเวลาปัจจุบัน: เดือน{$currentMonth} ปี {$currentYearBE} — ใช้ผูกการทำนายช่วงเวลา\n\n"

            // 👶🔥 (2026-09-03, owner) บทสรุปสุดท้ายเคยเป็นพรอมต์เดียวที่ *ไม่ได้* รับกฎสรรพนาม
            ...
            // 🃏 (2026-09-04 owner) เดิมห้าม "ไพ่บอกว่า..." ด้วย ⇒ บทสรุปไม่มีร่องรอยไพ่เลย (อ่านเหมือนบทโหราศาสตร์)
            //   owner: ใช้ "ไพ่บอกว่า" แทนชื่อไพ่ — ที่ห้ามคือชื่อไพ่กับเลขตำแหน่ง ไม่ใช่การอ้างว่ามาจากไพ่
            // 🃏 (2026-09-04) วัดบทสรุปจริง rd12274 แล้วพบ "ไพ่บอกว่า/หน้าไพ่/ไพ่ชี้" = **0 ครั้ง** ทั้งบท 6,093 ตัว
            //   เพราะเขียนว่า "ได้" (อนุญาต) ⇒ โมเดลเลือกไม่ใช้ + หัวข้อกฎคือ "ห้ามอธิบายไพ่" ยิ่งตอกย้ำให้เงียบ
            //   ⇒ ลูกค้าจ่ายค่าเปิดไพ่ 10 ใบ แล้วบทที่เก็บไว้อ่านซ้ำไม่มีคำว่าไพ่เลยสักคำ
            ."   ❌ ห้ามไล่บรรยายทีละใบ  ✅ *ต้องอ้างที่มาแบบไม่ระบุใบอย่างน้อย 2-3 จุดในบท*: \"ไพ่บอกว่า...\" / \"หน้าไพ่ชี้ชัดว่า...\"\n"
            ."   ✅ เนื้อหาทุกประโยค *ต้องงอกจากไพ่จริง* ตามกฎเหล็กด้านบน — พูดเป็นคำฟันธงของแม่หมอ: \"ไพ่บอกว่าเรื่องนี้จะจบลงแบบ... ราวเดือน...\"\n"
            ."   ⚠️ *บล็อกความรู้/ตำราด้านบน = วัตถุดิบ* — ห้ามลอก *รูปแบบ* บรรทัดคลัง (\"ตำแหน่ง N [ชื่อตำแหน่ง] — ชื่อไพ่\") ออกไปในบท\n"
            ."      แต่ *เนื้อหา* ในคลัง (รูปพรรณ-วัยของตัวละคร / อวัยวะ-โรค / เหตุการณ์เดินทาง-คดี-ลาภ / เลข-สี-ฤกษ์-ทิศ)\n"
            ."      *ต้องถูกหยิบมาเล่าตามที่คลังให้มา* — ❌ ห้ามเก็บไว้ในใจแล้วพูดแต่ข้อสรุปแห้งๆ ❌ ห้ามคิดตัวเลข/สี/ฤกษ์เอง\n\n"
            ...
            ."   เปิดด้วยคำเรียกอบอุ่น (\"ลูก\" / \"ลูกดวง\") → พูดถึงเรื่องที่ถามมาทั้งรอบ ผูกเป็นเส้นเรื่องเดียว\n"
            ."   ไม่ใช่ \"คำถาม 1 คือ A คำถาม 2 คือ B\" — ต้องเล่าให้เห็นว่าทุกเรื่องที่ถามมาโยงกันยังไง\n"
            ."   ปิดย่อหน้าด้วยประโยคทำนองว่า \"รอบนี้แม่หมอจะตอบให้ครบทุกข้อที่ลูกถามมา ไม่ให้ค้างคาใจ\"\n\n"
            ...
            ."      เช่น \"ที่ลูกถามว่าจะได้กลับไปคืนดีกันไหม — คำตอบคือ ได้ แต่ไม่ใช่รอบนี้ ต้องรอราวเดือน...\"\n"
            ...
            ."   ✅ *ถ้ามีคำถามจริงแค่ 1-2 ข้อ → ห้ามจบสั้น* — ลูกจ่าย 99฿ เท่ากับคนที่ถามเยอะ\n"
            ...
            ."   ✅ ฟันธง 1 ภาพใหญ่ — ชีวิตช่วงนี้ของลูกกำลังเดินไปทางไหน และจะลงเอยแบบไหน\n"
            ...
            ."   ตัวอย่าง: คนวัยกลางคน + ธาตุน้ำ → \"แม่น้ำที่เคยเชี่ยวกราก เมื่อพบทะเลก็ต้องนิ่งสงบ — ลูกถึงจุดที่ใจต้องเรียนรู้สงบ\"\n"
            ...
            ."      → พูดเป็นคำตอบตรงๆ ของแม่หมอแทน: \"เรื่องนี้คำตอบคือ...\" / \"สิ่งที่ขวางลูกอยู่คือ...\"\n"
