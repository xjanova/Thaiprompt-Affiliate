            ...
            <div class="flex items-center gap-4 mb-4">
                ...
                <div>
                    <h1 class="text-4xl font-bold bg-gradient-to-r from-blue-600 to-cyan-600 bg-clip-text text-transparent dark:from-blue-400 dark:to-cyan-400">
                        ➕ เพิ่มหลักสูตรฝึกอบรม
                    </h1>
                    <p class="mt-2 text-gray-600 dark:text-gray-400">สร้างหลักสูตรฝึกอบรมใหม่สำหรับพนักงาน</p>
                </div>
            </div>
        </div>

        <form action="{{ route('admin.hrm.training.courses.store') }}" method="POST" class="space-y-6">
            @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="course_category"
                            ...
                            <option value="technical" {{ old('course_category') == 'technical' ? 'selected' : '' }}>เทคนิค</option>
                            <option value="soft_skills" {{ old('course_category') == 'soft_skills' ? 'selected' : '' }}>ทักษะนุ่ม</option>
                            <option value="leadership" {{ old('course_category') == 'leadership' ? 'selected' : '' }}>ความเป็นผู้นำ</option>
                            <option value="compliance" {{ old('course_category') == 'compliance' ? 'selected' : '' }}>การปฏิบัติตาม</option>
                            <option value="safety" {{ old('course_category') == 'safety' ? 'selected' : '' }}>ความปลอดภัย</option>
                            <option value="product" {{ old('course_category') == 'product' ? 'selected' : '' }}>ผลิตภัณฑ์</option>
                            <option value="sales" {{ old('course_category') == 'sales' ? 'selected' : '' }}>การขาย</option>
                            <option value="other" {{ old('course_category') == 'other' ? 'selected' : '' }}>อื่นๆ</option>
                        </select>
                    ...
                    <div>
                        ...
                        <input type="number"
                               ...
                               value="{{ old('course_duration_hours') }}"
                               required
                               step="0.5"
                               min="0"
                               placeholder="เช่น 8"
                    ...
                    <div>
                        ...
                        <select name="training_method"
                            ...
                            <option value="classroom" {{ old('training_method') == 'classroom' ? 'selected' : '' }}>🏫 ในห้องเรียน</option>
                            <option value="online" {{ old('training_method') == 'online' ? 'selected' : '' }}>💻 ออนไลน์</option>
                            <option value="hybrid" {{ old('training_method') == 'hybrid' ? 'selected' : '' }}>🔄 ผสมผสาน</option>
                            <option value="on_the_job" {{ old('training_method') == 'on_the_job' ? 'selected' : '' }}>👷 ในงาน</option>
                            <option value="workshop" {{ old('training_method') == 'workshop' ? 'selected' : '' }}>🛠️ เวิร์คช็อป</option>
                            <option value="seminar" {{ old('training_method') == 'seminar' ? 'selected' : '' }}>🎤 สัมมนา</option>
                        </select>
                    ...
                    <div>
                        ...
                        <input type="number"
                               ...
                               value="{{ old('cost_per_participant') }}"
                               step="0.01"
                               min="0"
                    ...
                    <div>
                        ...
                        <select name="status"
                            ...
                            <option value="archived" {{ old('status') == 'archived' ? 'selected' : '' }}>จัดเก็บ</option>
                        </select>
                        @error('status')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="prerequisites"
                                  ...
                                  class="w-full px-4 py-3 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 focus:border-transparent transition-all duration-200 @error('prerequisites') border-red-500 @enderror">{{ old('prerequisites') }}</textarea>
                        @error('prerequisites')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
