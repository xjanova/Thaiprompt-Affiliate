...
class SmsPaymentController extends Controller
{
    ...
    public function approveOrder(Request $request, $identifier): JsonResponse
    {
        ...
        if ($type === 'fortune') {
            ...
            // 🧹 (2026-09-07) ไม่มีใบผูกอยู่ = admin อนุมัติมือทั้งที่ยอดไม่ตรงเป๊ะ
            //    ลองผูก SMS ที่ค้างคิว admin ให้บิลนี้ ไม่งั้นธง requires_admin_review ค้างถาวร
            $notification = $notification ?: app(SmsPaymentService::class)
                ->reconcileOrphanNotificationForBill($model);
        ...
        foreach ($identifiers as $identifier) {
            ...
            if ($type === 'fortune') {
                ...
                if ($model->is_paid && $alreadyDelivered) {
                    ...
                    // 🧹 (2026-09-07) เหมือน approveOrder — bulk ก็ต้องล้างธง SMS ที่ค้างคิว
                    $notification = $notification ?: app(SmsPaymentService::class)
                        ->reconcileOrphanNotificationForBill($model);
    ...
    private function executeFortuneApproveAction(array $payload, FortuneReading $reading, SmsCheckerDevice $device, string $ipAddress): JsonResponse
    {
        ...
        if (! $notification && $force) {
            $notification = app(SmsPaymentService::class)
                ->reconcileOrphanNotificationForBill($reading, $billAmount);
        }
