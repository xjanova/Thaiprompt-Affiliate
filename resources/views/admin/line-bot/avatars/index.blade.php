...
<div class="min-h-screen bg-gray-50 dark:bg-slate-900 py-6 px-4 sm:px-6 lg:px-8" x-data="{
    ...
    <div class="mb-8 relative overflow-hidden rounded-3xl bg-gradient-to-br from-[#00B900] via-[#00E600] to-[#00C900] p-8 shadow-2xl shadow-green-500/30">
        ...
        <div class="relative flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            <div class="flex items-center gap-4">
                <div class="w-16 h-16 rounded-2xl bg-white/20 backdrop-blur-md flex items-center justify-center shadow-xl">
            ...
            <button @click="showUploadModal = true"
                    class="px-6 py-3 bg-white/20 backdrop-blur-md border-2 border-white/40 text-white rounded-xl hover:bg-white/30 transition-all duration-300 font-bold flex items-center gap-2 shadow-lg hover:shadow-xl hover:scale-105">
    ...
    <!-- Filter Tabs -->
    <div class="mb-6 bg-white dark:bg-slate-800 rounded-2xl shadow-xl border border-gray-100 dark:border-slate-700 p-2">
        <div class="flex flex-wrap gap-2">
            <button @click="filterType = 'all'"
                    :class="filterType === 'all' ? 'bg-gradient-to-r from-[#00B900] to-[#00E600] text-white shadow-lg' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-slate-700'"
            ...
            <button @click="filterType = 'image'"
                    :class="filterType === 'image' ? 'bg-gradient-to-r from-blue-500 to-cyan-500 text-white shadow-lg' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-slate-700'"
                    class="px-6 py-3 rounded-xl font-bold transition-all duration-300 flex items-center gap-2">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <button @click="filterType = 'gif'"
                    :class="filterType === 'gif' ? 'bg-gradient-to-r from-purple-500 to-pink-500 text-white shadow-lg' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-slate-700'"
                    class="px-6 py-3 rounded-xl font-bold transition-all duration-300 flex items-center gap-2">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <button @click="filterType = 'lottie'"
                    :class="filterType === 'lottie' ? 'bg-gradient-to-r from-orange-500 to-red-500 text-white shadow-lg' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-slate-700'"
                    class="px-6 py-3 rounded-xl font-bold transition-all duration-300 flex items-center gap-2">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <button @click="filterType = 'video'"
                    :class="filterType === 'video' ? 'bg-gradient-to-r from-indigo-500 to-purple-500 text-white shadow-lg' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-slate-700'"
                    class="px-6 py-3 rounded-xl font-bold transition-all duration-300 flex items-center gap-2">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-4">
            @foreach($avatars as $avatar)
                <div x-show="filterType === 'all' || filterType === '{{ $avatar->type }}'"
                     class="group bg-white dark:bg-slate-800 rounded-2xl shadow-lg border border-gray-100 dark:border-slate-700 overflow-hidden hover:shadow-2xl hover:scale-105 transition-all duration-300">
                        ...
                        <div class="flex items-center gap-2">
                            @if(!$avatar->is_default)
                                <form action="{{ route('admin.line-bot.avatars.set-default', $avatar->id) }}" method="POST" class="flex-1">
                                    @csrf
                                    <button type="submit"
                                            class="w-full px-3 py-2 bg-gradient-to-r from-[#00B900] to-[#00E600] text-white rounded-lg hover:shadow-lg transition-all duration-300 text-sm font-bold flex items-center justify-center gap-1">
                            ...
                            @else
                                <div class="flex-1 px-3 py-2 bg-gray-100 dark:bg-slate-700 text-gray-400 dark:text-gray-500 rounded-lg text-sm font-bold text-center">
                                    ค่าเริ่มต้น
                                </div>
                            @endif
    ...
    @else
        <!-- Empty State -->
        <div class="bg-white dark:bg-slate-800 rounded-3xl shadow-xl border border-gray-100 dark:border-slate-700 p-12 text-center">
    ...
    <div x-show="showUploadModal"
        ...
        <div class="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl max-w-2xl w-full overflow-hidden transform transition-all duration-300"
            ...
            <form method="POST" action="{{ route('admin.line-bot.avatars.store') }}" enctype="multipart/form-data" class="p-6 space-y-6">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-4 gap-3">
                        <button type="button" @click="uploadType = 'image'"
                                :class="uploadType === 'image' ? 'bg-gradient-to-br from-blue-500 to-cyan-500 text-white shadow-lg scale-105' : 'bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-slate-600'"
                        ...
                        <button type="button" @click="uploadType = 'gif'"
                                :class="uploadType === 'gif' ? 'bg-gradient-to-br from-purple-500 to-pink-500 text-white shadow-lg scale-105' : 'bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-slate-600'"
                                class="p-4 rounded-xl transition-all duration-300 flex flex-col items-center gap-2">
                            <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 4v16M17 4v16M3 8h4m10 0h4M3 12h18M3 16h4m10 0h4M4 20h16a1 1 0 001-1V5a1 1 0 00-1-1H4a1 1 0 00-1 1v14a1 1 0 001 1z"/>
                            </svg>
                        ...
                        <button type="button" @click="uploadType = 'lottie'"
                                :class="uploadType === 'lottie' ? 'bg-gradient-to-br from-orange-500 to-red-500 text-white shadow-lg scale-105' : 'bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-slate-600'"
                                class="p-4 rounded-xl transition-all duration-300 flex flex-col items-center gap-2">
                            <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664z"/>
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
                        ...
                        <button type="button" @click="uploadType = 'video'"
                                :class="uploadType === 'video' ? 'bg-gradient-to-br from-indigo-500 to-purple-500 text-white shadow-lg scale-105' : 'bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-slate-600'"
                                class="p-4 rounded-xl transition-all duration-300 flex flex-col items-center gap-2">
                            <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z"/>
                            </svg>
                ...
                <div>
                    ...
                    <div class="flex gap-3">
                        <button type="button" @click="sourceType = 'upload'"
                                :class="sourceType === 'upload' ? 'bg-[#00B900] text-white' : 'bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-gray-400'"
                                class="flex-1 px-4 py-3 rounded-xl font-bold transition-all duration-300">
                            อัพโหลดไฟล์
                        </button>
                        <button type="button" @click="sourceType = 'url'"
                                :class="sourceType === 'url' ? 'bg-[#00B900] text-white' : 'bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-gray-400'"
                                class="flex-1 px-4 py-3 rounded-xl font-bold transition-all duration-300">
                            URL ลิงก์
                        </button>
                    </div>
                ...
                <div x-show="sourceType === 'upload'">
                    ...
                    <input type="file" name="file" accept="image/*,video/*,.gif,.json"
                           class="w-full px-4 py-3 border-2 border-dashed border-gray-300 dark:border-slate-600 bg-gray-50 dark:bg-slate-700 text-gray-900 dark:text-white rounded-xl focus:ring-4 focus:ring-green-500/20 focus:border-[#00B900] transition-all duration-300">
                ...
                <div>
                    ...
                    <textarea name="description" rows="3" placeholder="อธิบายเกี่ยวกับ Avatar นี้..."
                              class="w-full px-4 py-3 border-2 border-gray-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-gray-900 dark:text-white rounded-xl focus:ring-4 focus:ring-green-500/20 focus:border-[#00B900] transition-all duration-300"></textarea>
                </div>
                ...
                <div class="space-y-3">
                    <label class="flex items-center gap-3 cursor-pointer group">
                        <input type="checkbox" name="is_active" value="1" checked
                               class="w-5 h-5 text-[#00B900] bg-gray-100 border-gray-300 rounded focus:ring-green-500 dark:focus:ring-green-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
                ...
                <div class="flex gap-3 pt-4 border-t border-gray-200 dark:border-slate-700">
                    ...
                    <button type="submit"
                            class="flex-1 px-6 py-3 bg-gradient-to-r from-[#00B900] to-[#00E600] text-white rounded-xl hover:shadow-2xl hover:shadow-green-500/50 transition-all duration-300 transform hover:-translate-y-1 font-bold flex items-center justify-center gap-2">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12"/>
                        </svg>
                        อัพโหลด
