...
<div class="max-w-5xl mx-auto">
    ...
    <div class="glass-fusion dark:bg-gray-800 rounded-xl shadow-md p-6 mb-6" hover:scale-105 transition-transform border border-white/20 dark:border-white/10>
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-4">
                <a href="{{ route('admin.roles.show', $role) }}" class="text-gray-600 dark:text-gray-400 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-200">
                    ← กลับ
                </a>
                <div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        <div class="glass-fusion dark:bg-gray-800 rounded-xl shadow p-6" hover:scale-105 transition-transform border border-white/20 dark:border-white/10>
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm text-gray-600 dark:text-gray-400 dark:text-gray-400">สิทธิ์ปัจจุบัน</p>
        ...
        <div class="glass-fusion dark:bg-gray-800 rounded-xl shadow p-6" hover:scale-105 transition-transform border border-white/20 dark:border-white/10>
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm text-gray-600 dark:text-gray-400 dark:text-gray-400">สิทธิ์ที่มีทั้งหมด</p>
                    <p class="text-3xl font-bold text-gray-900 dark:text-white dark:text-gray-200 mt-2">{{ $permissions->flatten()->count() }}</p>
                </div>
                <div class="text-4xl">📋</div>
            </div>
        </div>
        ...
        <div class="glass-fusion dark:bg-gray-800 rounded-xl shadow-md p-6 mb-6" hover:scale-105 transition-transform border border-white/20 dark:border-white/10>
            @if($permissions->count() > 0)
                <div class="mb-4 flex items-center justify-between">
                    ...
                    <div class="flex gap-2">
                        ...
                        <button type="button"
                                onclick="deselectAll()"
                                class="px-3 py-1 text-xs bg-gray-100/50 dark:bg-gray-800/50 text-gray-700 dark:text-gray-300 rounded hover:bg-gray-200 dark:bg-gray-700 transition">
                            ยกเลิกทั้งหมด
                        </button>
                    </div>
                </div>
                ...
                <div class="space-y-4">
                    @foreach($permissions as $category => $categoryPermissions)
                        <div class="border border-gray-200 dark:border-gray-700 dark:border-gray-700 rounded-xl overflow-hidden">
                            <!-- Category Header -->
                            <div class="bg-gradient-to-r from-indigo-50 to-purple-50 dark:from-indigo-900/30 dark:to-purple-900/30 px-4 py-3 border-b border-gray-200 dark:border-gray-700 dark:border-gray-700">
                                <div class="flex items-center justify-between">
                            ...
                            <div class="p-4">
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                                    @foreach($categoryPermissions as $permission)
                                        <label class="flex items-start cursor-pointer hover:bg-gray-100/50 dark:bg-gray-800/50/50 dark:bg-gray-800/50 dark:hover:bg-gray-700 p-3 rounded-xl transition-all duration-200 border border-transparent hover:border-indigo-200 dark:hover:border-indigo-800"
                                               data-category="{{ $category }}">
                                            <input type="checkbox"
                                                   name="permissions[]"
                                                   value="{{ $permission->id }}"
                                                   {{ in_array($permission->id, $rolePermissions) ? 'checked' : '' }}
                                                   class="mt-1 rounded border-gray-300 dark:border-gray-600 text-indigo-600 focus:ring-indigo-500 permission-checkbox"
                                                   data-category="{{ $category }}">
                                            <div class="ml-3 flex-1">
                                                ...
                                                @endif
                                                <div class="text-xs text-gray-400 dark:text-gray-500 dark:text-gray-400 mt-1">
                                                    <code class="bg-gray-100/50 dark:bg-gray-800/50 dark:bg-gray-800 px-1 py-0.5 rounded">{{ $permission->name }}</code>
                                                </div>
                                            </div>
                                        </label>
                                    @endforeach
            ...
            @else
                <div class="text-center py-8">
                    ...
                    <p class="text-gray-500 dark:text-gray-400 dark:text-gray-400 text-lg">ยังไม่มีสิทธิ์ในระบบ</p>
                </div>
            @endif
            ...
            <!-- Buttons -->
            <div class="flex items-center justify-end gap-3 pt-6 border-t border-gray-200 dark:border-gray-700 dark:border-gray-700 mt-6">
