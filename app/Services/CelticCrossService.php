...
class CelticCrossService
{
        ...
        $groups = [
            ...
            [
                ...
                'spec' => "เขียน 4 ด้านนี้ (ขึ้นหัวข้อด้วย emoji ตามนี้เป๊ะ • แต่ละด้านผูกดาว(ภพ)+ไพ่ที่เกี่ยวข้อง • ฟันธงชัด • ไม่ต้องทำพาดหัวเรื่องเด่นซ้ำ):\n"
                    ...
                    ."💰 การเงิน\n"
                    ."🌿 สุขภาพ\n"
                    ."❌ ห้ามแปะ สีมงคล/เลขมงคล/ฤกษ์/วันมงคล/ทิศ/บทสรุปรวม/ธรรมะทิ้งท้าย ในด้านใดๆ — เก็บไว้บทสรุป VIP ตอนจบเท่านั้น (รอบนี้เพิ่งเปิดดวง)\n"
                    .'ปิดท้ายด้วย 1 บรรทัดสั้นเท่านั้น: ชวนเจ้าชะตาพิมพ์ถามเจาะลึกเรื่องที่อยากรู้ได้เลย (❌ ไม่สรุปยาว ❌ ไม่ใส่ธรรมะ/มงคล)',
            ],
        ];
    ...
    protected function buildMainPrompt(FortuneReading $reading, string $userQuestion, array $cards): string
    {
        ...
        $saiMuDirective = $this->buildSaiMuDirective($reading, $userQuestion);
    ...
    protected function buildFollowupPrompt(
        ...
        if ($sequence >= 2) {
            ...
            $advisorDirectiveQ2 = $clapbackDirective
                ...
                .$this->buildSaiMuDirective($reading, $userQuestion)
        ...
        // 🔮 (2026-05-28) สายมู directive — ให้คำแนะนำสายมูได้จริง ไม่มั่ว (judgment-based)
        $saiMuDirective2 = $this->buildSaiMuDirective($reading, $userQuestion);
     ...
     */
    protected function isBlackMagicModeForced(FortuneReading $reading): bool
    {
    ...
    protected function buildBlackMagicDirective(FortuneReading $reading, string $userQuestion, string $previousContext = ''): string
    {
        ...
        return $header.$openBlock
            ...
            ."   + เน้นทำเองได้ก่อน (สวดมนต์/แผ่เมตตา/ทำบุญอุทิศ/รักษาศีล) — ❌ ห้ามแนะพิธีแพง/ชี้ร้าน-อาจารย์เจาะจงเพื่อหากิน\n\n"
    ...
    protected function buildGrandFinalePrompt(
        ...
        return $localeDirective
            ...
            ."ย่อหน้า 13 — *สิ่งที่ต้องระวัง + ฤกษ์/ของมงคลเฉพาะตัว* (60-90 คำ):\n"
            ."   ⚠️ สิ่งที่ต้องระวัง (จากดาวศัตรู + ไพ่เตือน) + ทางแก้/เสริมดวงที่ทำเองได้ (❌ ไม่ขายความกลัว ไม่พิธีแพง)\n"
            ."   📅 วัน/ช่วงเวลามงคล (ดาวมิตร+ไพ่) + สีมงคล + เลขมงคล + ทิศ — สั้น กระชับ ตรงดวงคนนี้ (ไม่มั่วตัวเลขถ้าไพ่/ดาวไม่ได้ชี้)\n\n"

            ."ย่อหน้า 14 — *คำคมจากลาส่วนตัว* (40-60 คำ — signature):\n"
            ...
            ."📏 ความยาวรวม: 1100-1600 คำ (มี 15 ย่อหน้า: 0 พาดหัวฟันธง + 1-14 — แต่ละย่อหน้าสั้นกระชับ)\n"
