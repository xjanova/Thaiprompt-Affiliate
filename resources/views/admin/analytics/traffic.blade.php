...
<div class="space-y-6">
    ...
    <div class="bg-gradient-to-r from-orange-600 via-amber-600 to-yellow-600 rounded-xl shadow-2xl p-6 text-white">
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center gap-3">
                <select id="hoursFilter" class="px-4 py-2 glass-fusion text-orange-600 rounded-xl font-semibold">
                    <option value="6">Last 6 Hours</option>
                    <option value="24" selected>Last 24 Hours</option>
                    <option value="168">Last 7 Days</option>
                </select>
                <a href="{{ route('admin.dashboard') }}" class="px-4 py-2 glass-fusion text-orange-600 rounded-xl hover:bg-orange-50 transition font-semibold">
                    Back
                </a>
            </div>
        </div>
    ...
    <div id="loadingState" class="glass-fusion rounded-xl shadow-lg p-8 text-center" border border-white/20 dark:border-white/10>
        ...
        <p class="mt-4 text-gray-600 dark:text-gray-400">Loading traffic analytics...</p>
    </div>
    ...
    <div id="contentArea" class="hidden space-y-6">
        ...
        <div class="grid md:grid-cols-4 gap-4">
            <div class="glass-fusion rounded-xl shadow-lg p-6" hover:scale-105 transition-transform border border-white/20 dark:border-white/10>
                ...
                <div class="mt-2 text-xs text-gray-500 dark:text-gray-400" id="requestTrend">-</div>
            </div>

            <div class="glass-fusion rounded-xl shadow-lg p-6" hover:scale-105 transition-transform border border-white/20 dark:border-white/10>
                <div class="flex items-center justify-between mb-2">
                    <span class="text-gray-600 dark:text-gray-400 text-sm">Unique Visitors</span>
                    <span class="text-green-600 text-2xl">👥</span>
                </div>
                <div class="text-3xl font-bold text-gray-900 dark:text-white" id="uniqueVisitors">-</div>
                <div class="mt-2 text-xs text-gray-500 dark:text-gray-400" id="visitorTrend">-</div>
            </div>

            <div class="glass-fusion rounded-xl shadow-lg p-6" hover:scale-105 transition-transform border border-white/20 dark:border-white/10>
                <div class="flex items-center justify-between mb-2">
                    <span class="text-gray-600 dark:text-gray-400 text-sm">Status 2xx</span>
                    <span class="text-green-600 text-2xl">✅</span>
                </div>
                <div class="text-3xl font-bold text-gray-900 dark:text-white" id="status2xx">-</div>
                <div class="mt-2 text-xs text-gray-500 dark:text-gray-400">Success responses</div>
            </div>

            <div class="glass-fusion rounded-xl shadow-lg p-6" hover:scale-105 transition-transform border border-white/20 dark:border-white/10>
                <div class="flex items-center justify-between mb-2">
                    <span class="text-gray-600 dark:text-gray-400 text-sm">Status 4xx/5xx</span>
                    <span class="text-red-600 text-2xl">⚠️</span>
                </div>
                <div class="text-3xl font-bold text-gray-900 dark:text-white" id="statusErrors">-</div>
                <div class="mt-2 text-xs text-gray-500 dark:text-gray-400">Error responses</div>
            </div>
        </div>
        ...
        <div class="glass-fusion rounded-xl shadow-lg overflow-hidden" border border-white/20 dark:border-white/10>
            <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
                <h3 class="text-lg font-bold text-gray-900 dark:text-white">Top Endpoints (by Request Count)</h3>
            </div>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-100/50 dark:bg-gray-800/50/50 dark:bg-gray-800/50">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Avg Response (ms)</th>
                        </tr>
                    </thead>
                    <tbody id="endpointsTable" class="glass-fusion divide-y divide-gray-200">
                        <tr>
...
async function loadData() {
    ...
    try {
        const response = await fetch(`{{ route('admin.analytics.traffic') }}?hours=${hours}&ajax=1`);
...
function updateEndpointsTable(endpoints) {
    ...
    if (endpoints.length === 0) {
        tbody.innerHTML = '<tr><td colspan="3" class="px-6 py-4 text-center text-gray-500 dark:text-gray-400">No data available</td></tr>';
        return;
    }
