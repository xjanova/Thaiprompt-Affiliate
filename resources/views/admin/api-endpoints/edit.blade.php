...
@section('title', 'แก้ไข API Endpoint')
...
<div class="max-w-4xl mx-auto">
        ...
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">
                    แก้ไขรายละเอียด API endpoint
                </p>
            </div>
            <a href="{{ route('admin.api-management.endpoints.show', $apiEndpoint) }}"
    ...
    <!-- Form -->
    <form action="{{ route('admin.api-management.endpoints.update', $apiEndpoint) }}" method="POST" class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-6">
            ...
            <div>
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="name" value="{{ old('name', $apiEndpoint->name) }}" required
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="path" value="{{ old('path', $apiEndpoint->path) }}" required
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    ...
                    <div>
                        ...
                        <select name="method" required
                            ...
                            @foreach($methods as $method)
                                <option value="{{ $method }}" {{ old('method', $apiEndpoint->method) == $method ? 'selected' : '' }}>
                                    {{ $method }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="text" name="category" value="{{ old('category', $apiEndpoint->category) }}"
                               list="categories"
                ...
                <div class="mb-4">
                    ...
                    <textarea name="description" rows="3"
                              ...
                              placeholder="คำอธิบายเกี่ยวกับ endpoint นี้">{{ old('description', $apiEndpoint->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="number" name="priority" value="{{ old('priority', $apiEndpoint->priority ?? 0) }}"
            ...
            <div class="border-t border-gray-200 dark:border-gray-700 pt-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                        <label class="flex items-center">
                            <input type="checkbox" name="is_active" value="1" {{ old('is_active', $apiEndpoint->is_active) ? 'checked' : '' }}
                    ...
                    <div>
                        <label class="flex items-center">
                            <input type="checkbox" name="requires_auth" value="1" {{ old('requires_auth', $apiEndpoint->requires_auth) ? 'checked' : '' }}
                                   class="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="allowed_roles" value="{{ old('allowed_roles', is_array($apiEndpoint->allowed_roles) ? implode(', ', $apiEndpoint->allowed_roles) : $apiEndpoint->allowed_roles) }}"
            ...
            <div class="border-t border-gray-200 dark:border-gray-700 pt-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div>
                        ...
                        <input type="number" name="rate_limit_per_minute" value="{{ old('rate_limit_per_minute', $apiEndpoint->rate_limit_per_minute) }}" min="1"
                    ...
                    <div>
                        ...
                        <input type="number" name="rate_limit_per_hour" value="{{ old('rate_limit_per_hour', $apiEndpoint->rate_limit_per_hour) }}" min="1"
                    ...
                    <div>
                        ...
                        <input type="number" name="rate_limit_per_day" value="{{ old('rate_limit_per_day', $apiEndpoint->rate_limit_per_day) }}" min="1"
    ...
    <!-- Danger Zone -->
    <div class="bg-red-50 dark:bg-red-900 dark:bg-opacity-20 rounded-lg shadow-md p-6 mt-6 border border-red-200 dark:border-red-800">
        <h3 class="text-lg font-semibold text-red-800 dark:text-red-200 mb-4">⚠️ Danger Zone</h3>
        <p class="text-sm text-red-700 dark:text-red-300 mb-4">
            การลบ endpoint นี้จะลบข้อมูลทั้งหมด รวมถึงประวัติการใช้งาน การดำเนินการนี้ไม่สามารถย้อนกลับได้
        </p>
        <form action="{{ route('admin.api-management.endpoints.destroy', $apiEndpoint) }}" method="POST"
              onsubmit="return confirm('คุณแน่ใจหรือไม่ว่าต้องการลบ endpoint นี้? การดำเนินการนี้ไม่สามารถย้อนกลับได้')">
            @csrf
            @method('DELETE')
