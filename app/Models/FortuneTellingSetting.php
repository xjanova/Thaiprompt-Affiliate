...
EOT;
    ...
    public function getCommentReplyTemplate(bool $isReturning = false): string
    {
        ...
        // ⛔ (2026-08-22) ห้ามใส่ {name} — ชื่อจากโปรไฟล์ FB เป็นชื่อเต็ม
        //    ("สุวิสา ทองไทย") คนจริงไม่เรียกกันแบบนั้นในคอมเมนต์ = ป้ายบอกว่าเป็นบอท
        if ($isReturning) {
            return '🌙 แวะมาอีกแล้วนะคะ ✨ วันนี้อยากให้ดูเรื่องอะไรคะ';
        }

        return 'สวัสดีค่ะ 🔮 อยากรู้ดวงเรื่องไหนคะ';
    }
    ...
    public function getCommentEngagementPrompt(): string
    {
        ...
        return <<<'PROMPT'
คุณเป็นแม่หมอประจำเพจ ตอบเฉพาะเรื่องดูดวงเท่านั้น
...
สร้างข้อความ 2 ชุด:
1. COMMENT_REPLY: ข้อความสั้นตอบในคอมเม้นต์ (ไม่เกิน 100 ตัวอักษร) เชื่อมโยงกับสิ่งที่เขาคอมเม้นต์
   ⛔ ห้ามเรียกชื่อลูกค้าเด็ดขาด — ชื่อจากโปรไฟล์เป็นชื่อเต็ม คนจริงไม่เรียกกันแบบนั้น
      พอเห็นชื่อเต็มในคอมเมนต์ คนอ่านรู้ทันทีว่าเป็นบอท
      ถ้าจำเป็นต้องเรียก ใช้คำว่า "เจ้าชะตา" หรือไม่เรียกอะไรเลย
   ⛔ ห้ามขอให้กดไลก์ กดติดตาม กดแชร์ แท็กเพื่อน หรือพิมพ์คำเฉพาะ
      Facebook นับเป็น engagement bait และลดการมองเห็นทั้งเพจ
   ✅ ให้จบด้วย "คำถามสั้นๆ ที่ตอบได้ใน 1-3 คำ" เพื่อชวนคุยต่อ
      เช่น "เกิดวันอะไรคะ" / "ตอนนี้ห่วงเรื่องไหนที่สุดคะ"
   ✅ พิมพ์เหมือนคนจริง สั้น เป็นกันเอง ใช้ ค่ะ/นะคะ (แม่หมอเป็นผู้หญิง ห้ามใช้ ครับ)
