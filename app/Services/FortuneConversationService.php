...
class FortuneConversationService
{
    ...
    /**
     * 🛡️ (2026-05-13) Shield 1+2+3 — กันบิลซ้อนซ้อน
     *
     * ปัญหาที่แก้ (user report 2026-05-13):
     *   "คนใจร้อนพิมพ์รัวๆ → บอทตอบรับทุกอย่าง → สร้างบิลซ้อน"
     *   เคส:
     *     A. บิลกำลัง AI gen → ลูกค้ารัว → ระบบสร้างบิลใหม่ (มี gen_processing flag คุ้มแล้ว 50%)
     *     B. บิลเสร็จยังไม่อ่าน → ลูกค้าโวยวาย → ระบบสร้างบิลใหม่ ❌
     *     C. บิลค้างไม่จ่าย → โวยวาย → ระบบสร้างบิลใหม่ → UPA collision ❌
     *
     * Strategy:
     *   Shield 1 — Cache lock 10s (กัน concurrent race)
     *   Shield 2 — Pre-create checks (pending bill / generating / unread ready)
     *   Shield 3 — Throttle keyword 30s (กันรัวพิมพ์ "ดูดวง")
     */
    /**
     * 🛡️ (2026-05-13) Shield 2 — หา Deep pending bill (PENDING_PAYMENT, UPA reserved)
     *
     * Mirror ของ findPendingCelticBill — ใช้เมื่อลูกค้าค้างบิล Deep 39฿ Pay-First
     * แล้วพิมพ์ "ดูดวง" ซ้ำ → reuse UPA เดิมแทนสร้างใหม่ (กัน UPA collision)
     */
    protected function findPendingDeepBill(string $userId): ?FortuneReading
    {
        if (empty($userId)) {
            return null;
        }
        ...
        $candidate = FortuneReading::where(function ($q) use ($userId) {
            ...
            ->whereIn('conversation_status', [
                FortuneReading::STATUS_PENDING_PAYMENT,
                FortuneReading::STATUS_COLLECTING_BIRTHDATE, // pay-first ที่จ่ายแล้วรอวันเกิด
            ])
            ->where('is_paid', false)
            ->whereNotNull('unique_payment_amount_id')
            ->latest('updated_at')
    ...
    /**
     * 🛡️ (2026-05-13) Shield 2 — หา Deep reading ที่ AI กำลัง gen อยู่
     *
     * เกณฑ์:
     *   - reading_type=deep
     *   - is_paid=true
     *   - deep_response=null (ยังไม่เสร็จ)
     *   - paid_at >= now()-5min (กัน orphan ที่ AI fail หลายชั่วโมง)
     *   - มี gen_processing cache flag
     */
    protected function findGeneratingDeepReading(string $userId): ?FortuneReading
    {
        if (empty($userId)) {
            return null;
        }

        // เช็ค gen_processing flag ก่อน (เร็วกว่า DB query)
        if (! Cache::has("fortune:gen_processing:{$userId}")) {
            return null;
        }
    ...
    /**
     * 🛡️ (2026-05-13) Shield 2 — หา Deep reading ที่เสร็จแล้วแต่ยังไม่ได้อ่าน
     *
     * เกณฑ์:
     *   - reading_type=deep
     *   - is_paid=true
     *   - deep_response มี (AI ตอบครบ)
     *   - paid_at >= now()-24hr
     *   - ยังไม่ได้ acknowledge (ไม่มี conversation_state.reading_acknowledged_at)
     */
    protected function findRecentReadyDeepReading(string $userId): ?FortuneReading
    {
        if (empty($userId)) {
            return null;
        }
        ...
        return FortuneReading::where(function ($q) use ($userId) {
            ...
            ->whereNotNull('deep_response')
            ->where('paid_at', '>=', now()->subHours(24))
            ->whereRaw("(conversation_state IS NULL OR JSON_EXTRACT(conversation_state, '$.reading_acknowledged_at') IS NULL)")
            ->latest('paid_at')
            ->first();
    }
    ...
    protected function startDeepReadingFlow(string $facebookUserId, ?array $userProfile = null, ?string $forceTier = null): array
    {
        try {
            ...
            // 🛡️ (2026-05-13) Shield 1 — Atomic race guard (Cache::add)
            //   กัน FB webhook รัวๆ ส่ง 2 concurrent → สร้าง 2 readings
            //   Cache::add atomic: return false ถ้า key มีอยู่แล้ว → race-safe
            //   TTL 15s auto-expire — ไม่ต้อง explicit release (กัน orphan lock)
            $lockKey = "fortune:create_reading_lock:{$facebookUserId}";
            if (! Cache::add($lockKey, true, now()->addSeconds(15))) {
                Log::info('Fortune: Shield 1 — create_reading lock active, silent skip', [
                    'facebook_user_id' => $facebookUserId,
                    'force_tier' => $forceTier,
                ]);
            ...
            // 🛡️ Shield 3 — Throttle keyword cooldown 30s
            //   ภายใน 30s นับจากครั้งล่าสุด → silent_skip + announce 1 ครั้ง (ครั้งแรกเท่านั้น)
            //   ⚠️ forceTier != null → ลูกค้ากดปุ่ม "39"/"99" จาก quick reply → skip throttle
            //       (UX: ผู้ใช้ตั้งใจกดเลือกแพคเกจ ไม่ใช่รัวพิมพ์)
            if ($forceTier === null) {
                $throttleKey = "fortune:start_deep_throttle:{$facebookUserId}";
                if (Cache::has($throttleKey)) {
                    $announceKey = "fortune:start_deep_announce:{$facebookUserId}";
                    if (! Cache::has($announceKey)) {
                        Cache::put($announceKey, true, now()->addSeconds(30));
                        Log::info('Fortune: Shield 3 — throttle hit, announce once', [
                            'facebook_user_id' => $facebookUserId,
                        ]);

                        return [
                            'action' => 'throttle_announce',
                            'message' => "🌙 แม่หมอกำลังจัดการคำขอของเจ้าชะตาให้อยู่ค่ะ\n"
                                ."✨ รอสักครู่นะ — อย่าเพิ่งพิมพ์ซ้ำ ✨\n\n"
                                .'ถ้าไม่ได้รับการตอบกลับใน 1-2 นาที พิมพ์ "เช็คสถานะ" ดูได้นะคะ 🙏',
                            'reading' => null,
                        ];
                    }

                    // ครั้งต่อๆ ไป — silent skip
                    Log::debug('Fortune: Shield 3 — throttle silent skip', [
                        'facebook_user_id' => $facebookUserId,
                    ]);
            ...
            // 🛡️ Shield 2 — Pre-create checks (กันบิลซ้อน) — ลำดับความสำคัญ:
            //   1. กำลัง AI gen → "กำลังเปิดดวงให้อยู่" (ลูกค้าใจร้อน — ไม่สร้างใหม่)
            //   2. มีบิลเสร็จยังไม่อ่าน < 24hr → push คำทำนายเดิม (ลูกค้าลืม)
            //   3. มีบิลค้างจ่าย UPA active → re-push QR (ลูกค้าโวยวาย — ใช้บิลเดิม)
            //   ⚠️ forceTier='celtic' → skip shield 2 (มี findResumableCelticReading อยู่ใน startCelticCrossFlow แล้ว)

            if ($forceTier !== 'celtic') {
                $generatingReading = $this->findGeneratingDeepReading($facebookUserId);
                if ($generatingReading) {
                    Log::info('Fortune: Shield 2 — มี reading กำลัง AI gen → silent + announce', [
                        'facebook_user_id' => $facebookUserId,
                        'reading_id' => $generatingReading->id,
                        'paid_at' => $generatingReading->paid_at?->toIso8601String(),
                    ]);

                    return [
                        'action' => 'shield_generating',
                        'message' => "🔮 แม่หมอกำลังเปิดดวงให้เจ้าชะตาอยู่นะคะ\n\n"
                            ."⏳ ใช้เวลา 1-3 นาที — ขอให้รอสักครู่ค่ะ ✨\n"
                            ."จะส่งคำทำนายให้ทันทีเมื่อพร้อม 🙏\n\n"
                            .'(พิมพ์ "เช็คสถานะ" ถ้าอยากดูความคืบหน้า)',
                        'reading' => $generatingReading,
                    ];
                }

                $readyReading = $this->findRecentReadyDeepReading($facebookUserId);
                if ($readyReading) {
                    Log::info('Fortune: Shield 2 — มี reading เสร็จยังไม่อ่าน < 24hr → ส่งซ้ำ', [
                        'facebook_user_id' => $facebookUserId,
                        'reading_id' => $readyReading->id,
                        'paid_at' => $readyReading->paid_at?->toIso8601String(),
                    ]);

                    // mark acknowledged — กันส่งซ้ำรอบหน้า
                    $readyReading->setConversationState('reading_acknowledged_at', now()->toIso8601String());

                    return [
                        'action' => 'shield_ready_resend',
                        'message' => "🔮 ✨ เจ้าชะตาคะ — แม่หมอเปิดดวงให้แล้วเมื่อ "
                            .$readyReading->paid_at->diffForHumans()."\n"
                            ."ส่งคำทำนายให้อ่านอีกครั้งนะคะ 🙏\n\n"
                            .'──────────────────────'."\n\n"
                            .($readyReading->deep_response ?? 'คำทำนายของเจ้าชะตา'),
                        'reading' => $readyReading,
                    ];
                }

                $pendingBill = $this->findPendingDeepBill($facebookUserId);
                if ($pendingBill) {
                    $upa = $pendingBill->uniquePaymentAmount;
                    $payAmount = number_format((float) $upa->unique_amount, 2);
                    $remainMinutes = max(0, (int) now()->diffInMinutes($upa->expires_at, false));

                    Log::info('Fortune: Shield 2 — มีบิล Deep pending ค้าง → reuse บิลเดิม', [
                        'facebook_user_id' => $facebookUserId,
                        'reading_id' => $pendingBill->id,
                        'bill_reference' => $pendingBill->bill_reference,
                        'pay_amount' => $payAmount,
                        'remain_minutes' => $remainMinutes,
                    ]);

                    return [
                        'action' => 'shield_pending_bill',
                        'message' => "💸 เจ้าชะตาคะ — บิลของเจ้ายังรอจ่ายอยู่ค่ะ\n\n"
                            ."🔖 เลขที่บิล: {$pendingBill->bill_reference}\n"
                            ."💰 ยอดที่ต้องโอน: *{$payAmount} บาท* (ทศนิยมต้องตรงเป๊ะ)\n"
                            ."⏳ เหลือเวลา: {$remainMinutes} นาที\n\n"
                            ."📌 หลังโอนเสร็จ — ระบบจะเช็คให้อัตโนมัติ\n"
                            .'❌ ถ้าอยากเริ่มใหม่ พิมพ์ "ยกเลิก" ก่อน',
                        'reading' => $pendingBill,
                    ];
                }
            }
    ...
    {
        // 🎯 (2026-05-04) Simplify อีกขั้น — รูปแบบเดียว + บอกแค่ปีต้องครบ 4 หลัก
        //    user feedback: "ลูกค้ายังงงเรื่องการพิมพ์วันที่ — ใส่แค่รูปแบบเดียวให้ดู
        //                    แค่บอกต้องใส่ พ.ศ. หรือ ค.ศ. ให้ครบ 4 ตัว"
        // 🇱🇦 (2026-05-03) Lao Phase 2 — locale-aware via FortuneLocaleService::lo()
        return \App\Services\FortuneLocaleService::lo(
            "🎂 *กรุณาพิมพ์วันเดือนปีเกิดของเจ้าชะตา*\n\n"
            ."ตัวอย่าง:  *1/1/2521*\n\n"
            ."💡 *ปีต้องครบ 4 หลัก* — ใส่ พ.ศ. หรือ ค.ศ. ก็ได้ค่ะ\n"
            ."    (เช่น 2521 = พ.ศ.  หรือ  1978 = ค.ศ.)\n\n"
            .'💡 ถ้าไม่แน่ใจ พิมพ์ "ทีละส่วน" หมอจะถามทีละอย่างให้นะคะ',
            "🎂 *ກະລຸນາພິມວັນເດືອນປີເກີດຂອງເຈົ້າຊາຕາ*\n\n"
            ."ຕົວຢ່າງ:  *1/1/2521*\n\n"
            ."💡 *ປີຕ້ອງຄົບ 4 ຫລັກ* — ໃສ່ ພ.ສ. ຫຼື ຄ.ສ. ກໍ່ໄດ້ເດີ\n"
            ."    (ເຊັ່ນ 2521 = ພ.ສ.  ຫຼື  1978 = ຄ.ສ.)\n\n"
    ...
    protected function classifyPendingPaymentIntent(string $message): string
    {
        ...
        // ทำไมต้องเสียเงิน / ขอฟรี — เน้น "ทำไม" เพื่อ trigger reasoning เต็ม
        $whyPay = ['ทำไมต้องจ่าย', 'ทำไมต้องเสีย', 'ทำไมเสียเงิน', 'ทำไมต้องเสียเงิน', 'ทำไมเก็บเงิน'];
        foreach ($whyPay as $kw) {
            if (str_contains($text, $kw)) {
                return 'why_pay — ลูกค้าถามเหตุผลที่ต้องจ่าย ตอบให้ครบ: '
                    .'(1) แม่หมอใช้พลังเปิดดาว+เรียงไพ่ ไม่ได้เกิดเองฟรี '
        ...
        foreach ($free as $kw) {
            if (str_contains($text, $kw)) {
                return 'wants_free — ลูกค้าขอฟรี อธิบายเมตตา ไม่ฮาร์ดเซล: ค่าครูทำให้คำทำนายมีน้ำหนัก จิตจริง ดาวจึงเรียงตรง';
            }
        }
