...
<div class="space-y-6" x-data="{ language: 'th' }">
    ...
    <div class="container mx-auto px-4 py-6">
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-6">
            <!-- Stage 1: Lead -->
            <div class="glass-fusion dark:bg-gray-800 rounded-xl shadow-lg p-4 border-t-4 border-orange-500 transform hover:scale-105 transition-transform duration-200" border border-white/20 dark:border-white/10>
                <h6 class="text-sm font-medium text-gray-600 dark:text-gray-400 dark:text-gray-400 mb-2" data-translate>ลีด</h6>
                <p class="text-2xl font-bold text-gray-900 dark:text-white mb-1">{{ $pipelineStats['lead'] ?? '0' }}</p>
                <p class="text-xs text-gray-500 dark:text-gray-400 dark:text-gray-400">${{ number_format($pipelineValue['lead'] ?? 0, 0) }}</p>
            </div>

            <!-- Stage 2: Qualified -->
            <div class="glass-fusion dark:bg-gray-800 rounded-xl shadow-lg p-4 border-t-4 border-amber-500 transform hover:scale-105 transition-transform duration-200" border border-white/20 dark:border-white/10>
                <h6 class="text-sm font-medium text-gray-600 dark:text-gray-400 dark:text-gray-400 mb-2" data-translate>ผ่านคุณสมบัติ</h6>
                <p class="text-2xl font-bold text-gray-900 dark:text-white mb-1">{{ $pipelineStats['qualified'] ?? '0' }}</p>
                <p class="text-xs text-gray-500 dark:text-gray-400 dark:text-gray-400">${{ number_format($pipelineValue['qualified'] ?? 0, 0) }}</p>
            </div>

            <!-- Stage 3: Proposal -->
            <div class="glass-fusion dark:bg-gray-800 rounded-xl shadow-lg p-4 border-t-4 border-yellow-500 transform hover:scale-105 transition-transform duration-200" border border-white/20 dark:border-white/10>
                <h6 class="text-sm font-medium text-gray-600 dark:text-gray-400 dark:text-gray-400 mb-2" data-translate>ข้อเสนอ</h6>
                <p class="text-2xl font-bold text-gray-900 dark:text-white mb-1">{{ $pipelineStats['proposal'] ?? '0' }}</p>
                <p class="text-xs text-gray-500 dark:text-gray-400 dark:text-gray-400">${{ number_format($pipelineValue['proposal'] ?? 0, 0) }}</p>
            </div>

            <!-- Stage 4: Negotiation -->
            <div class="glass-fusion dark:bg-gray-800 rounded-xl shadow-lg p-4 border-t-4 border-blue-500 transform hover:scale-105 transition-transform duration-200" border border-white/20 dark:border-white/10>
                <h6 class="text-sm font-medium text-gray-600 dark:text-gray-400 dark:text-gray-400 mb-2" data-translate>เจรจา</h6>
                <p class="text-2xl font-bold text-gray-900 dark:text-white mb-1">{{ $pipelineStats['negotiation'] ?? '0' }}</p>
                <p class="text-xs text-gray-500 dark:text-gray-400 dark:text-gray-400">${{ number_format($pipelineValue['negotiation'] ?? 0, 0) }}</p>
            </div>

            <!-- Stage 5: Closed Won -->
            <div class="glass-fusion dark:bg-gray-800 rounded-xl shadow-lg p-4 border-t-4 border-green-500 transform hover:scale-105 transition-transform duration-200" border border-white/20 dark:border-white/10>
                ...
                <p class="text-xs text-gray-500 dark:text-gray-400 dark:text-gray-400">${{ number_format($pipelineValue['won'] ?? 0, 0) }}</p>
            </div>

            <!-- Stage 6: Closed Lost -->
            <div class="glass-fusion dark:bg-gray-800 rounded-xl shadow-lg p-4 border-t-4 border-red-500 transform hover:scale-105 transition-transform duration-200" border border-white/20 dark:border-white/10>
                ...
                <p class="text-xs text-gray-500 dark:text-gray-400 dark:text-gray-400">${{ number_format($pipelineValue['lost'] ?? 0, 0) }}</p>
            </div>
        </div>
        ...
        <div class="glass-fusion dark:bg-gray-800 rounded-xl shadow-lg" border border-white/20 dark:border-white/10>
            <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700 dark:border-gray-700 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <h3 class="text-lg font-bold text-gray-900 dark:text-white" data-translate>รายละเอียดไปป์ไลน์</h3>
                <div>
                    <select class="px-4 py-2 bg-gray-100/50 dark:bg-gray-800/50/50 dark:bg-gray-800/50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 dark:border-gray-600 rounded-xl text-gray-900 dark:text-white focus:ring-2 focus:ring-orange-500 dark:focus:ring-orange-400 focus:border-transparent transition" id="filterStage">
                    ...
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        <thead class="bg-gray-100/50 dark:bg-gray-800/50/50 dark:bg-gray-800/50 dark:bg-gray-700/50">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 dark:text-gray-300 uppercase tracking-wider" data-translate>ขั้นตอน</th>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 dark:text-gray-300 uppercase tracking-wider" data-translate>โอกาส</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 dark:text-gray-300 uppercase tracking-wider" data-translate>คาดว่าจะปิด</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 dark:text-gray-300 uppercase tracking-wider" data-translate>ติดต่อล่าสุด</th>
                        ...
                        <tbody class="glass-fusion dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                            ...
                            <tr class="hover:bg-gray-100/50 dark:bg-gray-800/50/50 dark:bg-gray-800/50 dark:hover:bg-gray-700/50 transition-colors duration-150">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full {{ $lead->stage_color ?? 'bg-gray-100/50 dark:bg-gray-800/50 text-gray-900 dark:text-white dark:bg-gray-700 dark:text-gray-200' }}">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm">
                                    <div class="flex gap-2">
                                        ...
                                        <button onclick="updateStage({{ $lead->id }})" class="inline-flex items-center px-3 py-2 bg-blue-100 hover:bg-blue-200 dark:bg-blue-900/30 dark:hover:bg-blue-900/50 text-blue-700 dark:text-blue-300 rounded-xl transition-colors duration-200">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
