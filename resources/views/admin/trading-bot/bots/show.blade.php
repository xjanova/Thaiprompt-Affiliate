<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('admin.trading-bot.bots.index') }}" class="text-gray-600 dark:text-gray-400 hover:underline">← Back to List</a>
        </div>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="glass-fusion dark:bg-gray-800 shadow-xl rounded-xl p-6" hover:scale-105 transition-transform border border-white/20 dark:border-white/10>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                        <h3 class="font-semibold text-gray-700 dark:text-gray-300 dark:text-gray-300 mb-3">Bot Information</h3>
                    ...
                    <div>
                        <h3 class="font-semibold text-gray-700 dark:text-gray-300 dark:text-gray-300 mb-3">Performance</h3>
                        <div class="space-y-2 text-sm">
                    ...
                    <div>
                        <h3 class="font-semibold text-gray-700 dark:text-gray-300 dark:text-gray-300 mb-3">Admin Actions</h3>
                        <div class="space-y-2">
                            @if($bot->status === 'running')
                                <form action="{{ route('admin.trading-bot.bots.stop', $bot) }}" method="POST">
                                    @csrf
                                    <button type="submit" class="w-full bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-xl">
                                        ⏸ Force Stop
                                    </button>
                                </form>
                            @else
                                <form action="{{ route('admin.trading-bot.bots.start', $bot) }}" method="POST">
                                    @csrf
                                    <button type="submit" class="w-full bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-xl">
                                        ▶ Force Start
                                    </button>
                                </form>
                            @endif
                            <form action="{{ route('admin.trading-bot.bots.destroy', $bot) }}" method="POST" onsubmit="return confirm('Delete this bot?')">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="w-full bg-gray-600 hover:bg-gray-700 text-white px-4 py-2 rounded-xl">
                                    🗑️ Delete Bot
                                </button>
                            </form>
                        </div>
