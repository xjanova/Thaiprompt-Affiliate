...
<div class="container mx-auto px-4 py-8 max-w-4xl">
    ...
    <form action="{{ route('admin.tarot.categories.store') }}" method="POST" class="bg-white rounded-lg shadow p-6">
        @csrf

        <div class="grid grid-cols-2 gap-6 mb-6">
            <div>
                <label class="block text-sm font-medium mb-2">ชื่อภาษาไทย *</label>
                <input type="text" name="name_th" required class="w-full rounded-lg border-gray-300">
            </div>
            <div>
                <label class="block text-sm font-medium mb-2">ชื่อภาษาอังกฤษ *</label>
                <input type="text" name="name_en" required class="w-full rounded-lg border-gray-300">
            </div>
        </div>
        ...
        <div class="grid grid-cols-3 gap-6 mb-6">
            <div>
                ...
                <input type="text" name="icon" placeholder="fa-heart" class="w-full rounded-lg border-gray-300">
            </div>
            <div>
                <label class="block text-sm font-medium mb-2">สี (Hex)</label>
                <input type="color" name="color" class="w-full h-10 rounded-lg border-gray-300">
            </div>
            <div>
                <label class="block text-sm font-medium mb-2">ราคา (บาท) *</label>
                <input type="number" name="price" step="0.01" min="0" value="0" required class="w-full rounded-lg border-gray-300">
            </div>
        </div>
