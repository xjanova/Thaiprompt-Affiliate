...
@section('title', 'แก้ไข Cashback Setting')
...
<div class="space-y-6">
    ...
    <div class="bg-gradient-to-br from-green-600 via-emerald-600 to-teal-600 rounded-2xl shadow-2xl p-8 text-white">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-3xl font-bold mb-2">แก้ไข Cashback Setting</h2>
                <p class="text-green-100 text-sm">อัพเดตเงื่อนไขและอัตรา Cashback</p>
            </div>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl shadow-lg p-8">
        <form action="{{ route('admin.cashback.update', $cashback) }}" method="POST" class="space-y-6" x-data="{ type: '{{ $cashback->type }}', valueType: '{{ $cashback->value_type }}' }">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <label class="relative flex items-center p-4 border-2 border-gray-300 dark:border-slate-600 rounded-lg cursor-pointer hover:border-green-500 dark:hover:border-green-500 transition" :class="type === 'global' ? 'border-green-500 bg-green-50 dark:bg-green-900/20' : ''">
                        <input type="radio" name="type" value="global" x-model="type" {{ $cashback->type === 'global' ? 'checked' : '' }} class="mr-3" required>
                        <div>
                    ...
                    <label class="relative flex items-center p-4 border-2 border-gray-300 dark:border-slate-600 rounded-lg cursor-pointer hover:border-green-500 dark:hover:border-green-500 transition" :class="type === 'product' ? 'border-green-500 bg-green-50 dark:bg-green-900/20' : ''">
                        <input type="radio" name="type" value="product" x-model="type" {{ $cashback->type === 'product' ? 'checked' : '' }} class="mr-3" required>
                        <div>
            ...
            <div x-show="type === 'product'" x-transition>
                ...
                <select name="product_id" class="w-full px-4 py-2 border border-gray-300 dark:border-slate-600 dark:bg-slate-700 dark:text-white rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent">
                    ...
                    @foreach($products as $product)
                        <option value="{{ $product->id }}" {{ old('product_id', $cashback->product_id) == $product->id ? 'selected' : '' }}>
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <label class="relative flex items-center p-4 border-2 border-gray-300 dark:border-slate-600 rounded-lg cursor-pointer hover:border-green-500 dark:hover:border-green-500 transition" :class="valueType === 'percentage' ? 'border-green-500 bg-green-50 dark:bg-green-900/20' : ''">
                        <input type="radio" name="value_type" value="percentage" x-model="valueType" {{ $cashback->value_type === 'percentage' ? 'checked' : '' }} class="mr-3" required>
                        <div>
                    ...
                    <label class="relative flex items-center p-4 border-2 border-gray-300 dark:border-slate-600 rounded-lg cursor-pointer hover:border-green-500 dark:hover:border-green-500 transition" :class="valueType === 'fixed' ? 'border-green-500 bg-green-50 dark:bg-green-900/20' : ''">
                        <input type="radio" name="value_type" value="fixed" x-model="valueType" {{ $cashback->value_type === 'fixed' ? 'checked' : '' }} class="mr-3" required>
                        <div>
            ...
            <div>
                ...
                <div class="relative">
                    <input type="number" name="value" value="{{ old('value', $cashback->value) }}" step="0.01" min="0" max="100" required class="w-full px-4 py-2 border border-gray-300 dark:border-slate-600 dark:bg-slate-700 dark:text-white rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent" placeholder="0.00">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="number" name="min_order_amount" value="{{ old('min_order_amount', $cashback->min_order_amount) }}" step="0.01" min="0" class="w-full px-4 py-2 border border-gray-300 dark:border-slate-600 dark:bg-slate-700 dark:text-white rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent" placeholder="0.00">
                ...
                <div>
                    ...
                    <input type="number" name="max_cashback_amount" value="{{ old('max_cashback_amount', $cashback->max_cashback_amount) }}" step="0.01" min="0" class="w-full px-4 py-2 border border-gray-300 dark:border-slate-600 dark:bg-slate-700 dark:text-white rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent" placeholder="0.00">
            ...
            <div>
                ...
                <textarea name="description" rows="3" class="w-full px-4 py-2 border border-gray-300 dark:border-slate-600 dark:bg-slate-700 dark:text-white rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent" placeholder="คำอธิบายเพิ่มเติมเกี่ยวกับ Cashback นี้">{{ old('description', $cashback->description) }}</textarea>
                @error('description')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex items-center">
                <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $cashback->is_active) ? 'checked' : '' }} class="mr-3 rounded border-gray-300 dark:border-slate-600 text-green-600 focus:ring-green-500">
    ...
    <div class="bg-red-50 dark:bg-red-900/20 border-2 border-red-200 dark:border-red-800 rounded-xl p-6">
        <h3 class="text-lg font-bold text-red-900 dark:text-red-300 mb-3">⚠️ โซนอันตราย</h3>
        <p class="text-sm text-red-800 dark:text-red-300 mb-4">การลบการตั้งค่านี้จะไม่สามารถกู้คืนได้</p>
        <form action="{{ route('admin.cashback.destroy', $cashback) }}" method="POST" onsubmit="return confirm('คุณแน่ใจหรือไม่ที่จะลบการตั้งค่านี้? การดำเนินการนี้ไม่สามารถย้อนกลับได้!')">
            @csrf
            @method('DELETE')
