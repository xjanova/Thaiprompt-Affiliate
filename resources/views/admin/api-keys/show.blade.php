...
<div class="max-w-7xl mx-auto">
    ...
    <div class="bg-gradient-to-r from-green-500 to-emerald-600 rounded-xl shadow-lg p-6 mb-6 text-white">
        <div class="flex items-start justify-between">
            <div class="flex-1">
                ...
                <div class="glass-fusion bg-opacity-20 rounded-xl p-4" border border-white/20 dark:border-white/10>
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <button onclick="copyApiKey('{{ session('api_key') }}')"
                                class="px-4 py-3 glass-fusion text-green-600 rounded hover:bg-green-50 transition flex-shrink-0">
                            📋 คัดลอก
                        </button>
                    </div>
                </div>
        ...
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-gray-600 dark:text-gray-400 dark:text-gray-400 mt-2">
                    <code class="px-2 py-1 bg-gray-100/50 dark:bg-gray-800/50 dark:bg-gray-700 rounded text-xs font-mono">{{ $apiKey->prefix }}</code>
                </p>
                @if($apiKey->description)
                <p class="text-sm text-gray-600 dark:text-gray-400 dark:text-gray-400 mt-2">{{ $apiKey->description }}</p>
                @endif
            </div>
            <div class="flex gap-2">
                <a href="{{ route('admin.api-management.keys.edit', $apiKey) }}"
                   class="px-4 py-2 bg-yellow-600 hover:bg-yellow-700 text-white rounded-xl transition">
                    แก้ไข
                </a>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <div class="style="background: var(--arrow-x-primary-gradient)" rounded-xl shadow-lg p-6 text-white">
            <div class="flex items-center justify-between">
                ...
                <div class="w-14 h-14 glass-fusion bg-opacity-20 rounded-xl flex items-center justify-center" border border-white/20 dark:border-white/10>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <div class="glass-fusion dark:bg-gray-800 rounded-xl shadow-md p-6" hover:scale-105 transition-transform border border-white/20 dark:border-white/10>
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white dark:text-gray-200 mb-4">📋 ข้อมูล API Key</h3>

            <div class="space-y-3">
                @if($apiKey->user)
                <div>
                    <label class="text-sm text-gray-500 dark:text-gray-400 dark:text-gray-400">เจ้าของ</label>
                    <p class="text-gray-900 dark:text-gray-200">
                        {{ $apiKey->user->name }}
                        <span class="text-sm text-gray-500 dark:text-gray-400">({{ $apiKey->user->email }})</span>
                    </p>
                </div>
                @endif

                <div>
                    <label class="text-sm text-gray-500 dark:text-gray-400 dark:text-gray-400">สถานะ</label>
                    <p class="text-gray-900 dark:text-gray-200">
                        @php
                ...
                @if($apiKey->scopes)
                <div>
                    <label class="text-sm text-gray-500 dark:text-gray-400 dark:text-gray-400">Scopes</label>
                    <p class="text-gray-900 dark:text-gray-200">
                ...
                <div>
                    <label class="text-sm text-gray-500 dark:text-gray-400 dark:text-gray-400">IP Addresses ที่อนุญาต</label>
                ...
                <div>
                    <label class="text-sm text-gray-500 dark:text-gray-400 dark:text-gray-400">สร้างเมื่อ</label>
                    <p class="text-gray-900 dark:text-gray-200">{{ $apiKey->created_at->format('d/m/Y H:i:s') }}</p>
                </div>

                @if($apiKey->last_used_at)
                <div>
                    <label class="text-sm text-gray-500 dark:text-gray-400 dark:text-gray-400">ใช้งานล่าสุด</label>
                ...
                <div>
                    <label class="text-sm text-gray-500 dark:text-gray-400 dark:text-gray-400">วันหมดอายุ</label>
        ...
        <div class="glass-fusion dark:bg-gray-800 rounded-xl shadow-md p-6" hover:scale-105 transition-transform border border-white/20 dark:border-white/10>
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white dark:text-gray-200 mb-4">⏱️ โควต้าและข้อจำกัด</h3>

            @if($apiKey->monthly_quota)
            <div class="mb-6">
                <div class="flex justify-between items-center mb-2">
                    <label class="text-sm text-gray-500 dark:text-gray-400 dark:text-gray-400">การใช้งานรายเดือน</label>
                ...
                <p class="text-xs text-gray-500 dark:text-gray-400 dark:text-gray-400 mt-1">{{ number_format($percentage, 1) }}% ใช้ไปแล้ว</p>
            </div>
            @endif

            <div class="space-y-3">
                <div class="flex justify-between">
                    <span class="text-sm text-gray-500 dark:text-gray-400 dark:text-gray-400">จำกัดต่อนาที</span>
                    <span class="text-sm text-gray-900 dark:text-gray-200">{{ $apiKey->rate_limit_per_minute ?? 'ไม่จำกัด' }}</span>
                </div>
                <div class="flex justify-between">
                    <span class="text-sm text-gray-500 dark:text-gray-400 dark:text-gray-400">จำกัดต่อชั่วโมง</span>
                    <span class="text-sm text-gray-900 dark:text-gray-200">{{ $apiKey->rate_limit_per_hour ?? 'ไม่จำกัด' }}</span>
                </div>
                <div class="flex justify-between">
                    <span class="text-sm text-gray-500 dark:text-gray-400 dark:text-gray-400">จำกัดต่อวัน</span>
            ...
            <div class="mt-6 pt-6 border-t border-gray-200 dark:border-gray-700 dark:border-gray-700">
                <h4 class="text-sm font-semibold text-gray-900 dark:text-white dark:text-gray-200 mb-3">สถิติการใช้งาน</h4>
                <div class="space-y-2">
                    ...
                    <div class="flex justify-between text-sm">
                        <span class="text-gray-600 dark:text-gray-400 dark:text-gray-400">เดือนนี้</span>
    ...
    <div class="glass-fusion dark:bg-gray-800 rounded-xl shadow-md p-6 mb-6" hover:scale-105 transition-transform border border-white/20 dark:border-white/10>
        <h3 class="text-lg font-semibold text-gray-900 dark:text-white dark:text-gray-200 mb-4">🔝 Endpoints ที่ใช้บ่อยที่สุด</h3>

        <div class="space-y-3">
            @foreach($topEndpoints as $log)
                @if($log->apiEndpoint)
                <div class="flex items-center justify-between p-3 bg-gray-100/50 dark:bg-gray-800/50/50 dark:bg-gray-800/50 dark:bg-gray-700 rounded-xl">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <div class="text-xs text-gray-500 dark:text-gray-400 dark:text-gray-400 font-mono">{{ $log->apiEndpoint->path }}</div>
                        </div>
                    </div>
    ...
    <div class="glass-fusion dark:bg-gray-800 rounded-xl shadow-md overflow-hidden" border border-white/20 dark:border-white/10>
        <div class="p-6 border-b border-gray-200 dark:border-gray-700 dark:border-gray-700">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white dark:text-gray-200">📊 การใช้งานล่าสุด</h3>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead class="bg-gray-100/50 dark:bg-gray-800/50/50 dark:bg-gray-800/50 dark:bg-gray-900">
                    <tr>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 dark:text-gray-400 uppercase">เวลา</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 dark:text-gray-400 uppercase">Endpoint</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 dark:text-gray-400 uppercase">Response Code</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 dark:text-gray-400 uppercase">Response Time</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 dark:text-gray-400 uppercase">IP Address</th>
                    </tr>
                </thead>
