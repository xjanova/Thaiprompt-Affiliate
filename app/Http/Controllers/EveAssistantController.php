...
class EveAssistantController extends Controller
{
    ...
    private function replyPromisesSearch(string $reply): bool
    {
        ...
        return (bool) preg_match(
            '/(กำลัง(?:ค้น|หา|จัดหา|เลือก)|ขอเวลา|เดี๋ยว[^\n]{0,14}(?:หา|ค้น|จัด)|(?:หา|ค้น|จัดหา|เลือก)ให้|'
            .'รอสักครู่|รอแป๊บ|สักครู่นะคะ|ให้ทีมงาน|จัดให้|ไปดูให้|ขอดูให้|เช็กให้|ตรวจสอบให้)/u',
            $reply
        );
    }
