...
function productLabelPrinter() {
    return {
        ...
        async searchProducts() {
            ...
            try {
                const response = await fetch(`{{ route('admin.pos.labels.api.search-products') }}?q=${this.searchQuery}`);
        ...
        async preview() {
            ...
            try {
                const response = await fetch('{{ route('admin.pos.labels.api.preview') }}', {
                    method: 'POST',
                    headers: {
        ...
        async print() {
            ...
            try {
                const response = await fetch('{{ route('admin.pos.labels.api.print') }}', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': '{{ csrf_token() }}'
