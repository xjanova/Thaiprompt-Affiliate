...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div class="bg-white rounded-xl shadow-lg p-6" x-data="{ open: false }">
            <div class="flex items-center mb-4">
                <span class="text-3xl mr-3">💵</span>
                <h3 class="text-lg font-bold text-gray-800">ฝากเงิน (Admin)</h3>
            </div>
            ...
            <div x-show="open" x-transition class="mt-4" style="display: none;">
                <form action="{{ route('admin.wallet.deposit') }}" method="POST" class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="number" name="amount" step="0.01" min="0.01" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">หมายเหตุ</label>
                        <input type="text" name="description" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent">
                    </div>
        ...
        <div class="bg-white rounded-xl shadow-lg p-6" x-data="{ open: false }">
            <div class="flex items-center mb-4">
                <span class="text-3xl mr-3">💸</span>
                <h3 class="text-lg font-bold text-gray-800">ถอนเงิน</h3>
            </div>
            ...
            <div x-show="open" x-transition class="mt-4" style="display: none;">
                <form action="{{ route('admin.wallet.withdraw') }}" method="POST" class="space-y-4">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">จำนวนเงิน</label>
                        <input type="number" name="amount" step="0.01" min="0.01" max="{{ $wallet->balance }}" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-transparent">
                        <p class="text-xs text-gray-500 mt-1">คงเหลือ: {{ number_format($wallet->balance, 2) }} {{ $wallet->currency }}</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">PIN <span class="text-red-500">*</span></label>
                        <input type="password" name="pin" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-transparent">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">หมายเหตุ</label>
                        <input type="text" name="description" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-transparent">
                    </div>
        ...
        <div class="bg-white rounded-xl shadow-lg p-6" x-data="{ open: false }">
            <div class="flex items-center mb-4">
                <span class="text-3xl mr-3">📤</span>
                <h3 class="text-lg font-bold text-gray-800">โอนเงิน</h3>
            </div>
            ...
            <div x-show="open" x-transition class="mt-4" style="display: none;">
                <form action="{{ route('admin.wallet.transfer') }}" method="POST" class="space-y-4">
                    @csrf
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">ที่อยู่กระเป๋าเงินปลายทาง</label>
                        <input type="text" name="wallet_address" required placeholder="TPW..." class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">จำนวนเงิน</label>
                        <input type="number" name="amount" step="0.01" min="0.01" max="{{ $wallet->balance }}" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">PIN <span class="text-red-500">*</span></label>
                        <input type="password" name="pin" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">หมายเหตุ</label>
                        <input type="text" name="description" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    </div>
    ...
    <div class="bg-white rounded-xl shadow-lg p-6">
        <div class="flex items-center justify-between mb-6">
            ...
            <a href="{{ route('admin.wallet.transactions') }}" class="text-indigo-600 hover:text-indigo-800 font-semibold text-sm">ดูทั้งหมด →</a>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    @empty
                    <tr>
                        <td colspan="5" class="px-6 py-4 text-center text-gray-500">ไม่มีธุรกรรม</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <a href="{{ route('admin.wallet.settings') }}" class="bg-white rounded-xl shadow-lg p-6 hover:shadow-xl transition-all transform hover:scale-105">
            <div class="flex items-center">
                <div class="flex-shrink-0">
                    <span class="text-4xl">⚙️</span>
                </div>
                <div class="ml-4">
                    <h4 class="text-lg font-bold text-gray-800">การตั้งค่า</h4>
                    <p class="text-sm text-gray-600">จัดการ PIN, 2FA และความปลอดภัย</p>
                </div>
            </div>
        </a>

        <a href="{{ route('admin.wallet.logs') }}" class="bg-white rounded-xl shadow-lg p-6 hover:shadow-xl transition-all transform hover:scale-105">
            <div class="flex items-center">
                <div class="flex-shrink-0">
                    <span class="text-4xl">🔒</span>
                </div>
                <div class="ml-4">
                    <h4 class="text-lg font-bold text-gray-800">ประวัติความปลอดภัย</h4>
                    <p class="text-sm text-gray-600">ดูกิจกรรมและการแจ้งเตือน</p>
                </div>
            </div>
        </a>
    </div>
