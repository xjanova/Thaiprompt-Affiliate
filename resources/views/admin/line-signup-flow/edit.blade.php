...
<div class="space-y-8 py-4">
    ...
    {{-- Form Section --}}
    <form method="POST" action="{{ route('admin.line-signup-flow.update', $flow->id) }}" x-data="signupFlowForm()">
        @csrf
        @method('PATCH')
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <input type="text"
                                   name="step_key"
                                   id="step_key"
                                   value="{{ old('step_key', $flow->step_key) }}"
                                   required
                                   pattern="[a-z0-9_]+"
                        ...
                        <div>
                            ...
                            <input type="number"
                                   name="step_order"
                                   id="step_order"
                                   value="{{ old('step_order', $flow->step_order) }}"
                                   required
                                   min="1"
                                   class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100"
                                   placeholder="1">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <textarea name="message_text"
                                      ...
                                      x-model="messageText"
                                      class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100"
                                      placeholder="ใส่ข้อความที่จะส่งไปให้ผู้ใช้...">{{ old('message_text', $flow->message_text) }}</textarea>
                                ...
                                @foreach($inputTypes as $type => $info)
                                    <label class="relative cursor-pointer">
                                        <input type="radio"
                                               ...
                                               value="{{ $type }}"
                                               x-model="inputType"
                                               {{ old('input_type', $flow->input_type) === $type ? 'checked' : '' }}
                                               class="peer sr-only">
                        ...
                        <div>
                            ...
                            <select name="next_step_key"
                                ...
                                @foreach($availableSteps as $step)
                                    <option value="{{ $step->step_key }}" {{ old('next_step_key', $flow->next_step_key) === $step->step_key ? 'selected' : '' }}>
                        ...
                        <div class="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                            ...
                            <label class="relative inline-flex items-center cursor-pointer">
                                <input type="checkbox"
                                       ...
                                       id="is_skippable"
                                       value="1"
                                       {{ old('is_skippable', $flow->is_skippable) ? 'checked' : '' }}
                                       class="sr-only peer">
                                <div class="w-11 h-6 bg-gray-300 dark:bg-gray-600 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-indigo-600 dark:peer-checked:bg-indigo-500"></div>
                            </label>
                        </div>
                        ...
                        <div class="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                            ...
                            <label class="relative inline-flex items-center cursor-pointer">
                                <input type="checkbox"
                                       ...
                                       id="require_ai"
                                       value="1"
                                       {{ old('require_ai', $flow->require_ai) ? 'checked' : '' }}
                                       class="sr-only peer">
                                <div class="w-11 h-6 bg-gray-300 dark:bg-gray-600 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-indigo-600 dark:peer-checked:bg-indigo-500"></div>
                            </label>
                        </div>
...
function signupFlowForm() {
    return {
        messageText: '{{ old('message_text', $flow->message_text) }}',
        inputType: '{{ old('input_type', $flow->input_type) }}',
