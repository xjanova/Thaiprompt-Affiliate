...
# ลบ entry เดิมที่มี marker เดียวกัน (กัน duplicate ตอน path เปลี่ยน)
FILTERED="$(echo "$CURRENT" | grep -v -F "${MARKER}" | grep -v -F "artisan schedule:run.*${PROJECT_PATH}" || true)"
