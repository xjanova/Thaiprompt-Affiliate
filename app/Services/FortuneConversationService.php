...
class FortuneConversationService
{
    ...
    protected function buildWelcomeGuideMessage(bool $isMidConversation = false): string
    {
        if ($isMidConversation) {
            ...
            // 🌙 (2026-05-23) Deep 39฿ ปิด — ชี้ Celtic 99฿ เป็นหลัก ถ้า Deep เปิดค่อยใส่ทั้งคู่
            $deepEnabled = $this->settings->isDeepReadingEnabled();
            $celticEnabled = (bool) ($this->settings->enable_celtic_cross ?? false);
            $deepPrice = (int) ($this->settings->deep_reading_price ?? 39);
            if ($deepEnabled && $celticEnabled) {
                return "🌙 อืม... ขออ่านอีกรอบนะเจ้าชะตา หรือกด 💎 ดูดวงละเอียด {$deepPrice}฿ / 🔮 ไพ่ 10 ใบ 99฿ ได้เลยค่ะ ✨";
            }
            if ($celticEnabled) {
                return '🌙 อืม... ขออ่านอีกรอบนะเจ้าชะตา หรือกด 🔮 ไพ่ 10 ใบ 99฿ ได้เลยค่ะ ✨';
            }
            if ($deepEnabled) {
                return "🌙 อืม... ขออ่านอีกรอบนะเจ้าชะตา หรือกด 💎 ดูดวงละเอียด {$deepPrice}฿ ได้เลยค่ะ ✨";
            }

            return '🌙 อืม... ขออ่านอีกรอบนะเจ้าชะตา พิมพ์เรื่องที่อยากให้แม่หมอช่วยดูได้เลยค่ะ ✨';
        }
    ...
    protected function buildPayFirstObjectionReply(FortuneReading $reading, UniquePaymentAmount $uniqueAmount): array
    {
        ...
        // 🌙 (2026-05-23) ใช้ $amount จากบิลจริง (number_format ทศนิยม) — ไม่ฮาร์ดโค้ด 39
        //   ลูกค้าสับสนเพราะ Deep ปิดแล้วแต่ quotes ยังพูดถึง "39 บาท"
        $quotes = [
            ...
            "🌙 *เจ้าชะตาขอน้ำจากบ่อ — ก็ต้องเอาถังลงไปตักก่อนค่ะ*\n"
                ."ค่าครู ฿{$amount} คือถังที่จะตักดวงดาวขึ้นมาให้ดู",
            ...
            "🌙 *ทุกอย่างในชีวิตมีต้นทุน — ความสงสัยก็เช่นกันค่ะ*\n"
                ."฿{$amount} แลก 'คำตอบที่ค้างคาใจ' ถือว่าเบาที่สุดแล้ว",
    ...
    protected function normalizeUserInput(string $text): string
    {
        ...
        //
        // 🐛 (2026-06-06) FIX: bare "นะ" เคยกินพยางค์ท้ายของคำว่า "สถานะ" (status)
        //   เคสจริง บิล FTU-260606-C8706 (reading 5068): ลูกค้าพิมพ์ "เช็คสถานะ"
        //   → normalize ตัด "นะ" เหลือ "เช็คสถา" → isPaymentClaimRequest() ไม่ match
    ...
    protected function looksLikeLowValueRamble(string $message): bool
    {
        ...
        $ventMarkers = [
            ...
            // 🎰 (2026-05-28) ขอเลขเด็ด/หวย — บอทให้ไม่ได้ (บาปกรรม) → ขอวนๆ = loop ไม่ converting
            //   เคส สนิท สาม่าน 128 turns: ขอเลขเด็ดซ้ำๆ. ใช้คำเฉพาะ (เลี่ยง false positive "ขอเลขบัญชี")
            'เลขเด็ด', 'ใบ้หวย', 'ขอหวย', 'เลขดัง', 'เลขเด่น', 'เลขล็อก', 'เลขล็อค', 'หวยงวด', 'เลขรัฐบาล', 'เลขใต้ดิน',
        ];
...
PROMPT;
        ...
        try {
            ...
            if ($flowContext) {
                $contextMap = [
                    ...
                    'pending_payment' => 'ผู้ใช้มีบิลรอชำระอยู่ — ต้องโอนเงินตามยอดในบิล หรือพิมพ์ "ยกเลิก". ถ้าถาม "ดูก่อนจ่ายทีหลังได้ไหม/ทำไมต้องจ่ายก่อน/ฟรีไหม" → บอกสุภาพว่าบริการนี้ชำระก่อนนะคะ เพราะแม่หมอเปิดไพ่ทำนายให้ทันทีหลังโอน (เป็นค่าครู/ค่าเปิดไพ่) — ไม่กดดัน ถ้ายังไม่พร้อมบอก "ไว้คราวหน้าได้นะคะ"',
                    // 💳 (2026-05-09) Stripe payment states
                    'awaiting_payment_method' => 'ผู้ใช้กำลังอยู่ขั้น "เลือกวิธีชำระเงิน" — มี 2 ปุ่ม: "QR ไทย" หรือ "บัตร ตปท." (Visa/Mastercard +15 บาท). *ถ้าผู้ใช้ถามคำถาม ต้องตอบคำถามนั้นก่อน*: "ดูดวงก่อนแล้วจ่ายทีหลังได้ไหม/ทำไมต้องจ่ายก่อน/ฟรีไหม" → บอกสุภาพตามจริงว่าบริการนี้ชำระก่อนรับบริการนะคะ เพราะแม่หมอจะเปิดไพ่ทำนายให้ทันทีหลังชำระ (เป็นค่าครู/ค่าเปิดไพ่) ไม่ใช่ระบบดูฟรีก่อน — *ไม่กดดัน ไม่ตื้อ* ถ้ายังไม่พร้อมบอก "ไม่เป็นไรค่ะ ไว้พร้อมแล้วทักมาได้เลยนะคะ" / ราคา → บอกตามจริง / เชื่อได้ไหม → ให้กำลังใจ. ตอบเสร็จระบบจะแนบปุ่มวิธีจ่ายให้เอง',
                    ...
                    'tier_choice' => (function () {
                        ...
                        return "ผู้ใช้กำลังอยู่ขั้น \"เลือกแพคเกจ\" — มี {$tierDescr} — ตอบคำถามเขาให้เข้าใจก่อน แล้วค่อยใบ้ให้เลือกแพคเกจตอนท้าย ห้ามบังคับเลือกทันที. ถ้าถาม \"ดูก่อนจ่ายทีหลังได้ไหม/ทำไมต้องจ่ายก่อน/ฟรีไหม\" → บอกสุภาพว่าบริการนี้ชำระก่อนรับบริการนะคะ (แม่หมอเปิดไพ่ทำนายให้ทันทีหลังชำระ) ไม่กดดัน ถ้ายังไม่พร้อมบอก \"ไว้คราวหน้าได้นะคะ\"";
                    })(),
                ];
                $contextHint = $contextMap[$flowContext] ?? '';
            }

            // 🆕 (2026-05-28) นำด้วย "คำถามดิบ" ก่อน — ให้ RAG admin Q&A จับคู่ตรง (เดิมห่อ meta-prompt → RAG ค้นผิดข้อความ)
            //   user spec: "บอทควรตอบ ไม่ใช่ส่งแต่กล่องเก็บเงิน ไม่ต้องตื้อ ตอบแบบที่แอดมินตอบ"
            $promptForAI = "\"{$messageText}\"\n\n";
            if ($contextHint !== '') {
                $promptForAI .= "(บริบทระบบ: {$contextHint})\n\n";
            }
            $promptForAI .= "ตอบแบบแม่หมอที่อบอุ่นเหมือนแอดมินตัวจริง (ใช้ ค่ะ/นะคะ) กระชับ 1-3 ประโยค:\n"
                ."1. ถ้าเป็น *คำถาม* (เช่น ราคา / ดูก่อนจ่ายทีหลัง / ทำไมต้องจ่ายก่อน / ฟรีไหม / เชื่อได้ไหม) → *ตอบคำถามนั้นตรงๆ ก่อน* ตามบริบทระบบ อย่าเลี่ยงไปบอกให้กดปุ่มเฉยๆ\n"
                ."2. ถ้าผู้ใช้สับสนเรื่องขั้นตอน — ใบ้สั้นๆ ว่าควรทำอะไร (ระบบจะเติมรายละเอียด/ปุ่มต่อท้ายเอง)\n"
    ...
    protected function parseBirthDate(string $text): ?string
    {
        ...
        // 🆕 (2026-06-08) Normalize คำเชื่อมวันที่ภาษาไทยที่พิมพ์ติดกัน/ไม่เว้นวรรค (ผู้สูงวัย)
        //   เคสจริง (FB): "วันที่29เดือนมกราคม2516วันจันค่ะ" → "29 มกราคม 2516" → parse ได้
        //   แทรกช่องว่างแทนคำเชื่อม (เดือน/ปี/พ.ศ./ค.ศ./ที่) + ชื่อวันในสัปดาห์ + คำลงท้ายสุภาพ
        //   เพื่อให้ เลข/ชื่อเดือน/ปี แยก token ก่อนเข้า pattern ด้านล่าง (ไม่ต้องพึ่ง AI fallback)
