...
class FortuneConversationService
{
    ...
    protected function getFallbackMessage(string $name): string
    {
        return "🔮 สวัสดีค่ะ คุณ{$name} ✨\n\n".
               ...
               "บอกหมอจันทราได้เลยนะคะว่าอยากรู้เรื่องอะไร:\n".
    ...
    protected function getFallbackFortuneResponse(string $messageText, ?array $userProfile = null): string
    {
        ...
        foreach ($greetings as $greeting) {
            if (str_contains($text, $greeting)) {
                return "🔮 สวัสดีค่ะ คุณ{$name} ✨\n\n".
                       ...
                       "ถามมาได้เลยนะคะ แล้วอย่าลืมบอกวันเดือนปีเกิดให้หมอจันทราด้วยนะคะ จะได้ทำนายได้แม่นยำยิ่งขึ้น 🎂\n\n".
        ...
        // ข้อความอื่นๆ ทั่วไป
        return "🔮 สวัสดีค่ะ คุณ{$name}\n\n".
               "เพจดูดวงหมอจันทรายินดีต้อนรับค่ะ หมอจันทราพร้อมดูดวงให้ค่ะ ✨\n\n".
    ...
    protected function getRandomFallback(string $category, string $name): string
    {
        $responses = [
            'love' => [
                "🔮 คุณ{$name} คะ หมอจันทราเห็นว่าช่วงนี้ดวงความรักกำลังมีการเปลี่ยนแปลงค่ะ\n\n".
                ...
                "🎨 สีมงคล: ชมพู, แดง\n\n".
                'ถ้าบอกวันเดือนปีเกิดให้หมอจันทรา จะได้ทำนายได้แม่นยำยิ่งขึ้นนะคะ 🎂',

                "🔮 คุณ{$name} คะ หมอจันทราขอบอกตรงๆ เลยนะคะ\n\n".
            ...
            'work' => [
                "🔮 คุณ{$name} คะ หมอจันทราเห็นดวงการงานช่วงนี้ค่ะ\n\n".
                ...
                "🔮 คุณ{$name} คะ หมอจันทราขอทำนายดวงการงานให้นะคะ\n\n".
            ...
            'money' => [
                "🔮 คุณ{$name} คะ หมอจันทราเห็นดวงการเงินค่ะ\n\n".
                ...
                "🔮 คุณ{$name} คะ หมอจันทราขอบอกเรื่องการเงินนะคะ\n\n".
            ...
            'health' => [
                "🔮 คุณ{$name} คะ หมอจันทราเห็นดวงสุขภาพค่ะ\n\n".
            ...
            'general' => [
                "🔮 คุณ{$name} คะ หมอจันทรายินดีดูดวงให้ค่ะ ✨\n\n".
                ...
                "🔢 เลขมงคล: 7, 16\n".
                "📅 วันมงคล: วันพฤหัสบดี\n\n".
                "บอกวันเดือนปีเกิดให้หมอจันทรานะคะ จะได้ทำนายได้แม่นยำยิ่งขึ้น 🎂\n".
                'ถ้าคำทำนายถูกใจ ฝากส่งต่อให้เพื่อนๆ มาลองดูดวงด้วยกันนะคะ 🔮✨',

                "🔮 คุณ{$name} คะ หมอจันทราขอทำนายดวงให้นะคะ\n\n".
    ...
    protected function handleCheckRemaining(string $facebookUserId): array
    {
        ...
        if ($remaining > 0) {
            ...
            $message .= 'หมอจันทราพร้อมทำนายให้ค่ะ 🔮✨';
        } else {
    ...
    protected function askFortuneConfirmation(string $facebookUserId, string $messageText, ?array $userProfile = null): array
    {
        ...
        if ($remaining > 0) {
            $message .= "💫 จะให้หมอจันทราดูดวงให้ไหมคะ?\n";
        ...
        if ($this->isDeclineResponse($messageText)) {
            ...
            return [
                ...
                'message' => "🔮 ไม่เป็นไรค่ะ คุณ{$name}\n\n".
                             "เมื่อไหร่อยากดูดวง ทักมาหาหมอจันทราได้ตลอดนะคะ ✨\n".
                             'ขอให้โชคดีค่ะ 🙏',
                'reading' => $reading,
            ];
        }
    ...
    protected function askForQuestionBeforeReading(string $facebookUserId, string $messageText, ?array $userProfile = null): array
    {
        ...
        if ($hasSpecificQuestion) {
            ...
            $message .= "✨ หมอจันทราพร้อมทำนายให้แล้วค่ะ\n\n";
    ...
    protected function handlePendingPayment(FortuneReading $reading, string $messageText): array
    {
        ...
        if (! $uniqueAmount || $uniqueAmount->expires_at < now()) {
            ...
            return [
                ...
                'message' => "⏰ บิลดูดวงละเอียดหมดอายุแล้วค่ะ\n\n".
                             ...
                             'หรือพิมพ์คำถามใหม่มาได้เลยค่ะ หมอจันทราพร้อมดูดวงให้ค่ะ 🔮✨',
                'reading' => $reading,
            ];
        }
        ...
        $message = "🔮 หมอจันทรารอคำทำนายละเอียดให้อยู่ค่ะ\n\n";
    ...
    protected function preFilterMessage(string $text): array
    {
        ...
        if ($length === 0) {
            return [
                'valid' => false,
                'reason' => 'empty',
                'message' => '🔮 พิมพ์ข้อความมาได้เลยนะคะ หมอจันทราพร้อมช่วยดูดวงให้ค่ะ ✨',
            ];
        }
    ...
    protected function getOffTopicMessage(string $category): string
    {
        ...
        return "{$specificMessage}\n\n".
               ...
               "💰 การเงิน โชคลาภ\n".
               "🏥 สุขภาพ\n\n".
               'ทักมาได้เลยค่ะ หมอจันทราพร้อมทำนายให้ค่ะ 🔮✨';
    }
    ...
    protected function handlePendingSaveResponse(string $userId, string $messageText, ?array $userProfile = null): ?array
    {
        ...
        if (str_contains($normalizedText, 'ไม่ฝากคำถาม') || str_contains($normalizedText, 'ไม่ฝาก') || $normalizedText === 'ไม่') {
            ...
            return [
                'action' => 'ai_chat_response',
                'message' => "ไม่เป็นไรค่ะ! ถ้ามีคำถามอื่น พิมพ์ถามหมอจันทราได้เลยนะคะ 😊\n\nหรือพิมพ์ ดูดวง เพื่อเข้าสู่การทำนายค่ะ ✨",
                'reading' => null,
            ];
        }
...
PROMPT;
    ...
    protected function buildBasicPrompt(?array $userProfile, string $question, string $userContext = '', ?string $detectedCategory = null): string
    {
        ...
        return "คุณชื่อ \"แม่หมอจันทรา\" เป็นหมอดูสาวสวยวัย 35 ปี ผู้เชี่ยวชาญโหราศาสตร์ไทย (โหราศาสตร์เจ้าชนะ) และโหราศาสตร์สากล ได้รับการถ่ายทอดวิชาจากครูบาอาจารย์สายลังกา พูดจาเพราะ อบอุ่นเป็นกันเอง น่าเชื่อถือ ใช้คำแทนตัวว่า \"หมอจันทรา\" ทำนายแม่นยำมาก ฟันธงแต่อ่อนโยน ไม่เกิน 500 คำ
...
🔮 **เปิดเรื่อง**: ทักทายอบอุ่น บอกว่าหมอจันทราดูดวงด้วยหลักโหราศาสตร์เจ้าชนะ อ้างอิงดาวที่ส่งผลช่วงนี้ เช่น \"หมอจันทราดูจากตำแหน่งดาว[ชื่อดาว]ที่กำลังโคจรผ่านภพ[ชื่อภพ]ของ{$genderPrefix}{$name}...\"
...
7. ใช้ภาษาไทย อบอุ่น เป็นกันเอง น่าเชื่อถือ มีศาสตร์รองรับทุกคำทำนาย";
    ...
    protected function buildPerQuestionDeepPrompt(
        ...
        // ลำดับที่ 2: ใช้ prompt hardcode เดิม (default)
        $prompt = "คุณชื่อ \"แม่หมอจันทรา\" เป็นหมอดูสาวสวยวัย 35 ปี ผู้เชี่ยวชาญศาสตร์โหราศาสตร์โบราณของไทย (หลักเจ้าชนะ) โหราศาสตร์สากล ไพ่ทาโรต์ และเลขศาสตร์ ได้รับวิชาจากครูบาอาจารย์สายลังกา มีประสบการณ์ 15 ปี ทำนายด้วยหลักวิชาโบราณล้วนๆ ทุกคำทำนายมีศาสตร์รองรับ พูดจาเพราะ อบอุ่น ใช้คำว่า \"หมอจันทรา\" แทนตัวเอง
...
- บอกว่า \"หมอจันทราได้รับคำถาม {$totalQuestions} ข้อจาก{$genderPrefix}{$name} หมอจันทราจะทำนายให้อย่างละเอียดทีละข้อด้วยหลักโหราศาสตร์เจ้าชนะนะคะ\"
...
- บอกจุดแข็งจุดอ่อนของดวงชะตา + บุคลิกภาพ' : "- บอกว่าหมอจันทราใช้ศาสตร์โหราศาสตร์โบราณหลักเจ้าชนะในการวิเคราะห์ดวงชะตาของ{$genderPrefix}{$name}").'
