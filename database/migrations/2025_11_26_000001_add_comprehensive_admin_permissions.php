 ...
 *
 * รองรับ 26+ modules ตามโครงสร้างเมนูแอดมินปัจจุบัน
 *
 * @version 3.197.0
 */
return new class extends Migration
{
    ...
    {
        // รายการ permissions ใหม่ที่ต้องเพิ่ม
        $newPermissions = [
            // ============================================
        ...
        // กำหนด permissions ใหม่ให้กับ super_admin โดยอัตโนมัติ
        ...
        // กำหนด permissions พื้นฐานให้กับ admin role
        ...
        if ($adminRole) {
            ...
            foreach ($basicAdminPermissions as $permissionName) {
    ...
    {
        // ลบ permissions ที่เพิ่มใหม่ (ตาม category)
        $newCategories = [
            'tickets', 'ai_core', 'ai_bots', 'chatbot', 'smart_sliders',
            'hotels', 'service_booking', 'ecommerce', 'pos', 'nfc',
            'crypto', 'mlm', 'email', 'line_oa', 'academy', 'learning',
            'marketing', 'hrm', 'accounting', 'notifications', 'security',
            'pages_seo', 'analytics', 'themes', 'languages', 'content_media',
            'ai_gen', 'settings', 'payment', 'roles',
        ];

        // ลบ role_permissions ที่เกี่ยวข้องก่อน
        $permissionIds = DB::table('permissions')
            ->whereIn('category', $newCategories)
            ->pluck('id');
