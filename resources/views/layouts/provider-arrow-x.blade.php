{{--
    Arrow X V3 Layout สำหรับ Provider Dashboard
    ใช้สำหรับหน้า Provider ทั้งหมด รวมถึงหน้าลงทะเบียน
--}}
<!DOCTYPE html>
...
<head>
    ...
    {{-- Font Awesome 6.7.2 - ใช้เวอร์ชันล่าสุดที่เสถียร --}}
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.7.2/css/all.min.css" referrerpolicy="no-referrer" />
    {{-- Fallback: โหลดจาก Font Awesome CDN โดยตรง --}}
    ...
    <style>
        ...
        :root {
            --provider-primary: #10b981;
            --provider-primary-dark: #059669;
            --provider-secondary: #06b6d4;
            --provider-gradient: linear-gradient(135deg, #10b981, #06b6d4);
        }
    </style>
...
<body class="h-full font-sans overflow-hidden flex"
    ...
    {{-- Background Gradient - Provider Theme (สีเขียว-น้ำเงิน) --}}
    ...
    <div class="fixed inset-0 -z-10 overflow-hidden pointer-events-none"
        ...
        <div class="absolute top-1/4 left-1/4 rounded-full animate-pulse transition-all duration-500"
             :style="'width: var(--bg-circle-size); height: var(--bg-circle-size); filter: blur(var(--bg-circle-blur)); animation-duration: var(--bg-animation-duration); ' +
                 ($store.theme.isDark
                     ? 'background: linear-gradient(to bottom right, #10b981, #059669); opacity: 0.15'
                     : 'background: linear-gradient(to bottom right, #10b981, #059669); opacity: var(--bg-circle-opacity)')">
        </div>
        ...
        <div class="absolute bottom-1/4 right-1/4 rounded-full animate-pulse transition-all duration-500"
             ...
             :style="'width: var(--bg-circle-size); height: var(--bg-circle-size); filter: blur(var(--bg-circle-blur)); animation-duration: var(--bg-animation-duration); ' +
                 ($store.theme.isDark
                     ? 'background: linear-gradient(to bottom right, #06b6d4, #0891b2); opacity: 0.15'
                     : 'background: linear-gradient(to bottom right, #06b6d4, #0891b2); opacity: var(--bg-circle-opacity)')">
        </div>
        ...
        <div class="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 rounded-full animate-pulse transition-all duration-500"
             ...
             :style="'width: var(--bg-circle-size); height: var(--bg-circle-size); filter: blur(var(--bg-circle-blur)); animation-duration: var(--bg-animation-duration); ' +
                 ($store.theme.isDark
                     ? 'background: linear-gradient(to bottom right, #3b82f6, #2563eb); opacity: 0.15'
                     : 'background: linear-gradient(to bottom right, #3b82f6, #2563eb); opacity: var(--bg-circle-opacity)')">
        </div>
    </div>

    {{-- Provider Sidebar Component --}}
    @hasSection('show-sidebar')
    ...
    <script>
    /**
     * Theme change events handler
     */
    window.addEventListener('theme-changed', (event) => {
        console.log('🎨 Theme changed in Provider Dashboard:', event.detail.isDark ? 'Dark' : 'Light');
        ...
        window.dispatchEvent(new CustomEvent('provider-theme-changed', {
            detail: event.detail
        }));
    });
