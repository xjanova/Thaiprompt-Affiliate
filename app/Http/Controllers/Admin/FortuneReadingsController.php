...
class FortuneReadingsController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // 🏷️ (2026-09-13) บิลยกเลิกส่วนใหญ่ไม่มีสถานะของตัวเอง (= completed + ไม่จ่าย + มีเหตุผลยกเลิก)
        //    เดิมตัวเลือก "cancelled" / "expired" ค้นคอลัมน์ตรง ๆ ⇒ เจอแค่ 60 ใบเก่า ขาดบิลยกเลิกจริง 1,410 ใบ
        //    และ "completed" ปนบิลยกเลิกมาด้วย — ดูเงื่อนไขทั้งหมดที่ FortuneReading::scopeFilterConversationStatus()
        if ($request->filled('conversation_status')) {
            $query->filterConversationStatus((string) $request->conversation_status);
        }
