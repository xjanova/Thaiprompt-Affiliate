...
class MobileApiController extends Controller
{
    ...
    // =====================================================
    // สินค้า / ตะกร้า / checkout / ร้านค้า / ออเดอร์ฝั่งร้าน
    // 🛒 (2026-09-25) ย้ายไป MobileShopController, SellerOrderApiController และ AddressApiController
    //    (ของเดิมในไฟล์นี้พังทุกเส้น: คอลัมน์ไม่มีจริง, checkout ไม่ครบคอลัมน์ NOT NULL, โค้ดส่วนลดฝังค่า)
    // =====================================================
    ...
    // =====================================================
    // Rider APIs → ย้ายไป App\Http\Controllers\Api\V1\RiderApiController แล้ว (2026-09-25)
    // (สมัคร/เอกสาร/สิทธิ์/เปิดรับงาน/ตำแหน่ง/งาน/รายได้ — ใช้ RiderJobService เป็นจุดเดียวที่เปลี่ยนสถานะงาน)
    // =====================================================
    ...
    /**
     * ลบ Push Token ของผู้ใช้ปัจจุบัน (เรียกตอนออกจากระบบ)
     *
     * 🔔 (2026-09-25) CC-21: เดิมลบแค่ตาราง user_notification_tokens แต่ push จริงส่งผ่าน
     *   mobile_devices.push_token (ExpoPushService::sendToUser) → เครื่องที่ logout แล้วยังได้
     *   แจ้งเตือนแชทออเดอร์/งานไรเดอร์ของบัญชีเดิม ตอนนี้ถอดทั้งสองที่
     *   - mobile_devices: ปลด user_id ออก (คงแถวเครื่องไว้ทำสถิติ/broadcast ทั่วไป)
     *   - แตะได้เฉพาะแถวที่ผูกกับผู้ใช้คนนี้เท่านั้น (กันถอด token ของคนอื่น)
     *
     * รับได้ทั้ง DELETE /push/token, DELETE /mobile/push-token และ POST /push/token/remove
     * body: { token: string, device_id?: string } — token ว่างแต่มี device_id ก็ได้
     */
    public function removePushToken(Request $request): JsonResponse
    {
        ...
        $validated = $request->validate([
            'token' => 'required_without:device_id|nullable|string|max:500',
            'device_id' => 'required_without:token|nullable|string|max:100',
        ], [
            'token.required_without' => 'กรุณาระบุ token หรือ device_id',
            'device_id.required_without' => 'กรุณาระบุ token หรือ device_id',
        ]);

        $removed = \App\Services\PushTokenService::detachForUser(
            (int) $user->id,
    ...
    public function transferMoney(Request $request): JsonResponse
    {
        ...
        try {
            ...
            // ล็อกแถววอลเลตผู้โอนก่อนตรวจยอด (กดโอนพร้อมกันหลายครั้งต้องไม่ทะลุยอด/ภาระ COD)
            $senderWallet = \App\Models\Wallet::whereKey($senderWallet->id)->lockForUpdate()->first() ?? $senderWallet;
            ...
            // 🛵 ไรเดอร์ที่ถือเงินสด COD ของลูกค้าอยู่ โอนออกได้เฉพาะส่วนที่ไม่ติดภาระนำส่ง
            //    (กันโอนเงินค้ำประกันออกแล้วเก็บเงินสดไว้เอง — RiderJob::codReserveForUser)
            $codReserve = \App\Models\RiderJob::codReserveForUser((int) $user->id);
            if ($codReserve > 0 && round((float) $senderWallet->balance - $codReserve, 2) < round($totalDeduction, 2)) {
                DB::rollBack();

                return response()->json([
                    'success' => false,
                    'code' => 'COD_RESERVED',
                    'message' => 'ยอด '.number_format($codReserve, 2).' บาท ถูกกันไว้สำหรับนำส่งเงินเก็บปลายทางของงานไรเดอร์ '
                        .'ตอนนี้โอนได้สูงสุด '.number_format(max(0.0, (float) $senderWallet->balance - $codReserve), 2).' บาท (รวมค่าธรรมเนียม)',
                ], 409);
            }
     ...
     *
     * ใช้สำหรับ: Wallet topup, Payment pages, Profile settings, หน้าร้าน/เครือข่ายที่อยู่บนเว็บ
     *
     * 🔐 (2026-09-25) ปิดช่อง open redirect + login CSRF (audit PLAY-16 / SHOP-15):
     *   - redirect_path ต้องเป็น path ภายในใต้ prefix ที่อนุญาต (App\Support\WebSessionRedirect)
     *     ไม่ผ่าน → 422 INVALID_REDIRECT_PATH (ไม่ fallback เงียบ ให้แอปรู้ว่าส่งผิด)
     *   - query_params ถูกกรองแล้วเก็บไว้ฝั่ง server คู่กับ token — URL ที่ได้มีแค่ ?token=
     *     (ฝั่งรับ token ไม่เชื่อ query string ที่ติดมากับลิงก์อีกต่อไป)
     *   - token ใช้ได้ครั้งเดียว อายุ 5 นาที · การล็อกอินจริงเกิดตอน POST (มี CSRF) บนหน้ายืนยัน
     */
    public function generateWebSessionToken(Request $request): JsonResponse
    {
        try {
            ...
            $rawPath = $request->input('redirect_path');
            if ($rawPath !== null && ! is_string($rawPath)) {
                $rawPath = '';
            }

            $redirectPath = \App\Support\WebSessionRedirect::sanitizePath(
                $rawPath === null ? \App\Support\WebSessionRedirect::DEFAULT_PATH : $rawPath
            );
            ...
            if ($redirectPath === null) {
                return response()->json([
                    ...
                    'message' => 'ปลายทางที่ต้องการเปิดไม่ได้รับอนุญาต',
                    'data' => [
                        'allowed_prefixes' => \App\Support\WebSessionRedirect::ALLOWED_PREFIXES,
                    ],
                ], 422);
            }

            $queryParams = \App\Support\WebSessionRedirect::sanitizeQuery($request->input('query_params'));

            // สร้าง one-time token (เก็บเฉพาะ hash ใน cache — token ดิบอยู่แค่ใน URL ที่ส่งให้แอป)
            $token = Str::random(64);
            $tokenHash = hash('sha256', $token);
            $ttl = \App\Support\WebSessionRedirect::TTL_SECONDS;

            \Cache::put('web_session_token:'.$tokenHash, [
                'user_id' => $user->id,
            ...
            $webUrl = rtrim(config('app.url'), '/').'/mobile-web-session?token='.$token;
