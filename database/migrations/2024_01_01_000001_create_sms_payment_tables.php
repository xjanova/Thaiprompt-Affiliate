...
/**
 * สร้างตารางสำหรับระบบ SMS Payment Checker
 *
 * ตาราง:
 * - sms_checker_devices: อุปกรณ์ Android ที่ลงทะเบียน
 * - sms_payment_notifications: SMS notifications ที่ได้รับ
 * - unique_payment_amounts: จำนวนเงินเฉพาะสำหรับจับคู่
 * - sms_payment_nonces: nonce สำหรับป้องกัน replay attack
 */
    ...
    public function up(): void
    {
        ...
        if (! Schema::hasTable('unique_payment_amounts')) {
            Schema::create('unique_payment_amounts', function (Blueprint $table) {
                ...
                $table->string('transaction_type', 50)->nullable();  // order, topup, etc.
