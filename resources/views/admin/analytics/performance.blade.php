...
<div class="space-y-6">
    <!-- Header -->
    <div class="style="background: var(--arrow-x-success-gradient)" rounded-xl shadow-2xl p-6 text-white">
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-4">
                <div class="w-14 h-14 glass-fusion rounded-xl flex items-center justify-center backdrop-blur-sm" border border-white/20 dark:border-white/10 border border-white/20 dark:border-white/10>
            ...
            <div class="flex items-center gap-3">
                <select id="hoursFilter" class="px-4 py-2 glass-fusion text-green-600 rounded-xl font-semibold">
                    <option value="1">Last Hour</option>
                    <option value="6">Last 6 Hours</option>
                    <option value="24" selected>Last 24 Hours</option>
                    <option value="168">Last 7 Days</option>
                </select>
                <a href="{{ route('admin.dashboard') }}" class="px-4 py-2 glass-fusion text-green-600 rounded-xl hover:bg-green-50 transition font-semibold">
                    Back
                </a>
            </div>
        </div>
    ...
    <!-- Loading State -->
    <div id="loadingState" class="glass-fusion rounded-xl shadow-lg p-8 text-center" border border-white/20 dark:border-white/10 border border-white/20 dark:border-white/10>
        <div class="inline-block animate-spin rounded-full h-12 w-12 border-b-2 border-green-600"></div>
        <p class="mt-4 text-gray-600 dark:text-gray-400 dark:text-gray-400">Loading performance data...</p>
    </div>
    ...
    <div id="contentArea" class="hidden space-y-6">
        ...
        <div class="grid md:grid-cols-4 gap-4">
            <div class="glass-fusion rounded-xl shadow-lg p-6" hover:scale-105 transition-transform border border-white/20 dark:border-white/10 hover:scale-105 transition-transform border border-white/20 dark:border-white/10>
                <div class="flex items-center justify-between mb-2">
                    <span class="text-gray-600 dark:text-gray-400 dark:text-gray-400 text-sm">Avg Response Time</span>
                    <span class="text-blue-600 text-2xl">⚡</span>
                </div>
                <div class="text-3xl font-bold text-gray-900 dark:text-white"><span id="avgResponse">-</span>ms</div>
                <div class="mt-2">
                    <div class="w-full bg-gray-200 dark:bg-gray-700 dark:bg-gray-700 rounded-full h-1.5">
            ...
            <div class="glass-fusion rounded-xl shadow-lg p-6" hover:scale-105 transition-transform border border-white/20 dark:border-white/10 hover:scale-105 transition-transform border border-white/20 dark:border-white/10>
                <div class="flex items-center justify-between mb-2">
                    <span class="text-gray-600 dark:text-gray-400 dark:text-gray-400 text-sm">CPU Usage</span>
            ...
            <div class="glass-fusion rounded-xl shadow-lg p-6" hover:scale-105 transition-transform border border-white/20 dark:border-white/10 hover:scale-105 transition-transform border border-white/20 dark:border-white/10>
                <div class="flex items-center justify-between mb-2">
                    <span class="text-gray-600 dark:text-gray-400 dark:text-gray-400 text-sm">Memory Usage</span>
            ...
            <div class="glass-fusion rounded-xl shadow-lg p-6" hover:scale-105 transition-transform border border-white/20 dark:border-white/10 hover:scale-105 transition-transform border border-white/20 dark:border-white/10>
                <div class="flex items-center justify-between mb-2">
                    <span class="text-gray-600 dark:text-gray-400 dark:text-gray-400 text-sm">Throughput</span>
                    <span class="text-green-600 text-2xl">📈</span>
                </div>
                <div class="text-3xl font-bold text-gray-900 dark:text-white"><span id="throughput">-</span>/s</div>
                <div class="mt-2 text-xs text-gray-500 dark:text-gray-400 dark:text-gray-400">Requests per second</div>
            </div>
        </div>
        ...
        <!-- Performance Details -->
        <div class="glass-fusion rounded-xl shadow-lg p-6" hover:scale-105 transition-transform border border-white/20 dark:border-white/10 hover:scale-105 transition-transform border border-white/20 dark:border-white/10>
            <h3 class="text-lg font-bold text-gray-900 dark:text-white mb-4">Performance Breakdown</h3>
...
async function loadData() {
    ...
    try {
        const response = await fetch(`{{ route('admin.analytics.performance') }}?hours=${hours}&ajax=1`);
...
function updateDetails(data) {
    ...
    container.innerHTML = Object.entries(details).map(([key, value]) => `
        <div class="bg-gray-100/50 dark:bg-gray-800/50/50 dark:bg-gray-800/50/50 dark:bg-gray-800/50 rounded-xl p-4">
            <div class="text-sm text-gray-600 dark:text-gray-400 dark:text-gray-400">${key.replace(/_/g, ' ').toUpperCase()}</div>
