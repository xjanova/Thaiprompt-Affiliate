...
/**
 * สร้างตารางระบบบัญชีที่ขาดหายไปอย่างปลอดภัย
 *
 * Migration นี้จะสร้างตารางระบบบัญชีทั้งหมดที่ยังไม่มีอยู่ในฐานข้อมูล
 * โดยจะเช็คว่าตารางมีอยู่แล้วหรือไม่ก่อนสร้าง เพื่อป้องกันข้อผิดพลาด
 */
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        if (! Schema::hasTable('accounting_chart_of_accounts')) {
            Schema::create('accounting_chart_of_accounts', function (Blueprint $table) {
                $table->id();
                $table->foreignId('user_id')->constrained('users')->cascadeOnDelete();
                $table->string('code'); // e.g., 1100, 2100
                $table->string('name');
                $table->string('name_eng')->nullable();
                ...
                $table->unique(['user_id', 'code'], 'acct_chart_user_code_unique');
            });
        }
        ...
        if (! Schema::hasTable('accounting_contacts')) {
            Schema::create('accounting_contacts', function (Blueprint $table) {
                ...
                $table->index(['user_id', 'type']);
                $table->unique(['user_id', 'code'], 'acct_contacts_user_code_unique');
            });
        }
        ...
        if (! Schema::hasTable('accounting_products')) {
            Schema::create('accounting_products', function (Blueprint $table) {
                ...
                $table->index(['user_id', 'type']);
                $table->unique(['user_id', 'code'], 'acct_products_user_code_unique');
            });
        }
        ...
        if (! Schema::hasTable('accounting_invoices')) {
            Schema::create('accounting_invoices', function (Blueprint $table) {
                ...
                $table->unique(['user_id', 'document_number'], 'acct_inv_user_docnum_unique');
            });
        }
        ...
        if (! Schema::hasTable('accounting_expenses')) {
            Schema::create('accounting_expenses', function (Blueprint $table) {
                ...
                $table->index(['user_id', 'status']);
                $table->index(['user_id', 'document_date']);
                $table->unique(['user_id', 'document_number'], 'acct_exp_user_docnum_unique');
            });
        }
        ...
        if (! Schema::hasTable('accounting_payments')) {
            Schema::create('accounting_payments', function (Blueprint $table) {
                ...
                $table->unique(['user_id', 'payment_number'], 'acct_pay_user_paynum_unique');
            });
        }
        ...
        if (! Schema::hasTable('accounting_journal_entries')) {
            Schema::create('accounting_journal_entries', function (Blueprint $table) {
                ...
                $table->unique(['user_id', 'journal_number'], 'acct_je_user_jnum_unique');
            });
        }
    ...
    {
        // ลบตารางตามลำดับที่ถูกต้อง (ตารางที่มี foreign key ก่อน)
