...
trait ProSessionTrait
{
    ...
    protected function buildDeepProSessionPrompt(FortuneReading $reading, string $name, int $remainingMin): string
    {
        ...
        // 💎 (2026-06-08) Upsell VIP 99 — เมื่อลูกค้าถามเจาะลึก/ละเอียดอ่อน/ไสยศาสตร์/มนต์ดำ/ออกทะเล/ถามเยอะ
        //   user spec: "พื้นดวง 39 = ถามทั่วๆ ไป. ถ้าเจาะลึกเป็นประเด็น/ละเอียดอ่อน → เปิดไพ่ 10 ใบ 99฿ พลังไพ่มากกว่า"
        ...
        $sellRule = $celticEnabledUpsell
            ? "💎 *เสนอได้เฉพาะ VIP ไพ่ 10 ใบ {$celticPriceUpsell} บาท* ตามกติกา [💎 เมื่อไรเสนอ VIP] ด้านล่างเท่านั้น — ห้ามพูดราคา/แพคเกจอื่น"
            : '❌ *ห้ามแนะนำราคา/ขายแพคเกจ* — ห้าม mention ราคาแพคเกจใดๆ';
        $newTopicRule = $celticEnabledUpsell
            ? "🔮 *เรื่องที่ต้องเจาะลึก/เปิดไพ่ใหม่* → เสนอ VIP ไพ่ 10 ใบ {$celticPriceUpsell} บาท (ดู [💎] ด้านล่าง)"
            : "❌ *ห้ามทำนายเรื่องใหม่ที่ต้องเปิดไพ่ใหม่* — บอกว่า \"เรื่องนี้ต้องเปิดไพ่ใหม่ค่ะ คุณ{$name}\"";
        $upsellBlock = '';
        if ($celticEnabledUpsell) {
            $upsellBlock = "\n\n[💎 เมื่อไรให้เสนอ VIP — ไพ่ 10 ใบ {$celticPriceUpsell} บาท]\n"
                ."ดูพื้นดวง 39 นี้ = ตอบ *คำถามทั่ว ๆ ไป* จากดวงเดิม + ไพ่ใบเดียวที่เปิดให้เท่านั้น\n"
                ."ถ้าลูกค้า:\n"
                ."  • ถามเจาะลึกเป็นประเด็น / อยากรู้ละเอียด เฉพาะเจาะจง\n"
                ."  • ถามเรื่องละเอียดอ่อน — ไสยศาสตร์ / มนต์ดำ / โดนของ / คุณไสย / เนื้อคู่เจาะจง / คดีความ\n"
                ."  • เริ่มถามออกนอกเรื่อง หรือถามเยอะขึ้นเรื่อย ๆ เกินภาพรวมพื้นดวง\n"
                ."→ ให้แม่หมอ *เสนออย่างนุ่มนวล ให้เกียรติ ไม่ตื๊อ*:\n"
                .'  "เรื่องนี้ลึก/ละเอียดอ่อนนะคะ — พื้นดวงใบเดียวอาจตอบได้ไม่สุด ถ้าอยากให้แม่หมอเจาะให้ชัด '
                ."ต้อง *เปิดไพ่ 10 ใบ (VIP {$celticPriceUpsell} บาท)* — *พลังของไพ่ 10 ใบจะมากกว่า* เห็นภาพชัด ฟันธงได้ลึกกว่าค่ะ "
                ."สนใจพิมพ์ *ดูดวง* แล้วเลือกแพคเกจ VIP ได้เลยนะคะ ✨\"\n"
                ."- ถ้าลูกค้าไม่เอา → ตอบพื้นดวงทั่วไปต่อได้ตามปกติ ไม่กดดัน\n";
        }
