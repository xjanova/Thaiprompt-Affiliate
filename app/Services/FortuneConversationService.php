...
class FortuneConversationService
{
    ...
    protected function getPayFirstPaymentMessage(FortuneReading $reading, UniquePaymentAmount $uniqueAmount): string
    {
        ...
        // เพิ่มบัญชีธนาคาร / PromptPay (QR code มาก่อน — ลูกค้าโอนได้เลย)
        $message .= $this->getBankAccountsListMessage();

        $message .= "\n⚠️ *กรุณาโอนยอด ฿{$amount} (ตรงทศนิยม)*\n"
            ."ระบบจะตรวจสอบอัตโนมัติด้วยทศนิยมที่ต่างกัน\n\n";

        // 🆕 (2026-05-13) "ทำไมต้องเก็บค่าครู" — user spec: ย้ายมาหลัง QR + ยอด
        //   เหตุผล: ลูกค้าเห็น QR/ยอดก่อน (action) → อ่านที่มาค่าครู (meaning) ภายหลัง
        //   เงินไป: ไหว้ครู / ไถ่ชีวิตโคกระบือ / งานธุดงควัตร วัดท่าซุง / ถ่ายเทพลังหมดกรรม
        $message .= "──────────────────────\n"
            ."🪔 *ค่าครูเล็กน้อยนี้ — แม่หมอนำไป:*\n"
            ."  • พิธีไหว้ครู สืบทอดวิชาให้ครูบาอาจารย์\n"
            ."  • ไถ่ชีวิตโคกระบือ — ต่อบุญต่ออายุให้เจ้าชะตา\n"
            ."  • ทำบุญงานธุดงควัตร ปลายปีที่ *วัดท่าซุง*\n"
            ."  • และพิธีบุญอื่นๆ ที่ส่งกุศลกลับให้ลูกชะตา ✨\n\n"
            ."🌸 *เมื่อเจ้าชะตาช่วยตัวเองด้วยใจตั้งใจ —\n"
            ."   พลังเก่าที่ติดขัดจะถ่ายเท กรรมจะคลี่คลายไปเอง*\n"
            ."   นี่คือเหตุที่หมอ \"ขอค่าครู\" ไม่ใช่ \"คิดเงิน\"\n"
            ."   เป็นการให้เจ้าชะตาเปิดทางพลังให้ตนเอง 🙏\n\n"
            ."💛 *ถ้าตอนนี้รู้สึกว่าฝืด ไม่มี ก็ไม่เป็นไร —*\n"
            ."   เดี๋ยวเจ้าชะตาต้องมีแน่นอน มันจะผ่านช่วงเวลานี้ไปได้\n"
            ."   ไม่ต้องกังวล แม่หมอรอเปิดดวงให้เสมอ ✨\n"
            ."──────────────────────\n\n";

        // 🆕 ย้ำยอดอีกครั้ง (กันลูกค้าลืมหลังอ่าน block ค่าครู)
        $message .= "💎 *ย้ำอีกครั้ง — ยอดที่ต้องโอน:* ฿{$amount}\n"
            ."🔖 บิล: {$billRef}\n\n"
