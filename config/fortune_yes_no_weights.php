 ...
 *   ⚠️ (2026-06-28) ทุกแถบ "ฟันธงนำเสมอ" — เงื่อนไข/เวลาเป็นตัวเสริมให้สำเร็จ ไม่ใช่คำกั๊กลบล้างคำตอบ
 *   - คะแนนรวม >= +5 = ✨ ใช่ชัด (ฟันธงด้านบวกได้เลย)
 *   - +2..+4 = ได้/ใช่ + เงื่อนไข-เวลาที่ทำให้สำเร็จ
 *   - -1..+1 = เอนตามไพ่เด่นสุด (ต.6/10) แล้วฟันธงข้างนั้น (ห้ามตอบลอย "ขึ้นกับตัวเอง")
 *   - -2..-4 = ยังไม่ได้ตอนนี้ + สิ่งที่ต้องเปลี่ยน/ช่วงที่จะพลิก
...
return [
    ...
    // 🔮 (2026-09-21) ใช้กับไพ่ "ตั้งตรง" เท่านั้น — ไพ่กลับหัวไม่นับคะแนน (FortuneKnowledgeService::yesNoVerdictFor)
    //   เดิมกลับสัญลักษณ์ → หอคอยกลับหัวกลายเป็น +3 "ใช่ชัด" · ตำราให้ไพ่กลับหัวไม่ตรงกัน จึงให้แม่หมออ่านความหมายกลับหัวเอง
    //   ("rev_score" ที่คอมเมนต์เดิมอ้างถึงไม่เคยมีในโค้ด)
    'card_weights' => [
        // ───── Major Arcana ─────
    ...
    // ════════════ การตัดสินผลลัพธ์ ════════════
    // 🎯 (2026-06-28) ฟันธงนำเสมอ — "เงื่อนไข/เวลา" เป็นตัวเสริมให้สำเร็จ ไม่ใช่คำกั๊กที่ลบล้างคำตอบ
    //   (ห้ามตอบ "ได้ แต่ก็ไม่แน่นอน/ขึ้นกับหลายอย่าง" — กั๊กจนลูกค้าสับสน)
    'verdicts' => [
        'strong_yes' => ['threshold' => 5, 'text' => '✨✨ ได้/ใช่ชัด — ไพ่หนุนเต็มที่ ฟันธงทางบวกได้เลย', 'icon' => '✅'],
        'lean_yes' => ['threshold' => 2, 'text' => '✨ ได้/ใช่ — ฟันธงทางบวก แล้วเสริม "เงื่อนไข/ช่วงเวลาที่ทำให้สำเร็จ" (ได้ ถ้า.../ได้ ภายใน...)', 'icon' => '🟢'],
        'unclear' => ['threshold' => -1, 'text' => '⚖️ เอนตามไพ่เด่นสุด (ดูตำแหน่ง 6/10) แล้วฟันธงข้างนั้น + ระบุจังหวะ/สิ่งที่พลิกผล — ห้ามตอบลอยว่า "ขึ้นกับตัวเอง"', 'icon' => '🟡'],
        'lean_no' => ['threshold' => -4, 'text' => '⚠️ ยังไม่ได้/ยังไม่ใช่ตอนนี้ — ฟันธงตรง แล้วบอก "ต้องเปลี่ยนอะไร / ช่วงไหนถึงจะพลิกเป็นได้"', 'icon' => '🔶'],
        'strong_no' => ['threshold' => -999, 'text' => '⚠️⚠️ ไม่/ยังไม่ใช่ — ไพ่ไม่หนุนทิศนี้ชัด ฟันธงตรง + เสนอทางใหม่ที่ไพ่หนุน', 'icon' => '🔴'],
    ],
];
