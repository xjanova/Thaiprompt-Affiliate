...
@section('title', 'แก้ไขเทมเพลตการประเมินผล')
...
<div class="space-y-6">
    ...
    <div class="bg-gradient-to-r from-purple-500 to-indigo-600 rounded-xl shadow-lg p-6 text-white">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold mb-1">แก้ไขเทมเพลตการประเมินผล</h1>
                <p class="text-purple-100">แก้ไขข้อมูล: {{ $template->name ?? 'เทมเพลต' }}</p>
            </div>
    ...
    <!-- Form -->
    <form method="POST" action="{{ route('admin.hrm.performance.templates.update', $template) }}" class="space-y-6">
        @csrf
        @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" rows="3"
                              ...
                              placeholder="อธิบายรายละเอียดของเทมเพลตนี้">{{ old('description', $template->description) }}</textarea>
                ...
                <div>
                    ...
                    <select name="review_type"
                        ...
                        <option value="annual" {{ old('review_type', $template->review_type) == 'annual' ? 'selected' : '' }}>ประจำปี</option>
                        <option value="quarterly" {{ old('review_type', $template->review_type) == 'quarterly' ? 'selected' : '' }}>รายไตรมาส</option>
                        <option value="monthly" {{ old('review_type', $template->review_type) == 'monthly' ? 'selected' : '' }}>รายเดือน</option>
                        <option value="probation" {{ old('review_type', $template->review_type) == 'probation' ? 'selected' : '' }}>ทดลองงาน</option>
                        <option value="project" {{ old('review_type', $template->review_type) == 'project' ? 'selected' : '' }}>ตามโครงการ</option>
                    </select>
                ...
                <div>
                    ...
                    <input type="number" name="max_score" value="{{ old('max_score', $template->max_score ?? 100) }}" min="1" step="1"
                ...
                @if(isset($template->criteria) && count($template->criteria) > 0)
                    ...
                    <div class="criteria-item border border-gray-300 dark:border-gray-600 rounded-lg p-4">
                        <div class="flex justify-between items-start mb-2">
                            <span class="text-sm font-medium text-gray-700 dark:text-gray-300">หัวข้อที่ {{ $index + 1 }}</span>
                            @if($index > 0)
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-12 gap-4">
                            <div class="md:col-span-5">
                                ...
                                <input type="text" name="criteria[{{ $index }}][name]" value="{{ old('criteria.'.$index.'.name', $criteria->name ?? $criteria['name'] ?? '') }}"
                                       class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-slate-700 dark:text-white rounded-lg focus:ring-2 focus:ring-purple-500"
                                       placeholder="เช่น คุณภาพงาน">
                            </div>
                            <div class="md:col-span-5">
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">คำอธิบาย</label>
                                <input type="text" name="criteria[{{ $index }}][description]" value="{{ old('criteria.'.$index.'.description', $criteria->description ?? $criteria['description'] ?? '') }}"
                                       class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-slate-700 dark:text-white rounded-lg focus:ring-2 focus:ring-purple-500"
                            ...
                            <div class="md:col-span-2">
                                ...
                                <input type="number" name="criteria[{{ $index }}][weight]" value="{{ old('criteria.'.$index.'.weight', $criteria->weight ?? $criteria['weight'] ?? '') }}" min="0" max="100" step="1"
                                       class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-slate-700 dark:text-white rounded-lg focus:ring-2 focus:ring-purple-500"
                                       placeholder="10">
                            </div>
                        </div>
            ...
            <div class="grid grid-cols-1 gap-4">
                <div>
                    ...
                    <select name="rating_scale"
                            class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-slate-700 dark:text-white rounded-lg focus:ring-2 focus:ring-purple-500">
                        <option value="5" {{ old('rating_scale', $template->rating_scale ?? 5) == 5 ? 'selected' : '' }}>มาตรวัด 5 ระดับ (1-5)</option>
                        <option value="10" {{ old('rating_scale', $template->rating_scale) == 10 ? 'selected' : '' }}>มาตรวัด 10 ระดับ (1-10)</option>
                        <option value="100" {{ old('rating_scale', $template->rating_scale) == 100 ? 'selected' : '' }}>มาตรวัด 100 คะแนน (0-100)</option>
                    </select>
                ...
                <div>
                    ...
                    <textarea name="rating_guidelines" rows="4"
                              class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-slate-700 dark:text-white rounded-lg focus:ring-2 focus:ring-purple-500"
                              placeholder="ระบุคำแนะนำหรือเกณฑ์การให้คะแนนในแต่ละระดับ">{{ old('rating_guidelines', $template->rating_guidelines) }}</textarea>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let criteriaCount = container.querySelectorAll('.criteria-item').length;
