...
<div class="p-6">
    ...
    <div class="mb-6">
        <div class="flex items-center mb-4">
            ...
            <h1 class="text-3xl font-bold text-gray-900 dark:text-white">เพิ่มผู้ติดต่อ</h1>
        </div>
        <p class="text-gray-600 dark:text-gray-400">กรอกข้อมูลผู้ติดต่อใหม่ (ลูกค้าหรือผู้ขาย)</p>
    </div>

    <!-- Form -->
    <form action="{{ route('admin.accounting.contacts.store') }}" method="POST" class="max-w-4xl">
        @csrf
            ...
            <div>
                ...
                <div class="grid grid-cols-3 gap-4">
                    <label class="flex items-center p-4 border-2 rounded-lg cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700 border-gray-300 dark:border-gray-600">
                        <input type="checkbox" name="is_customer" value="1" {{ old('is_customer', true) ? 'checked' : '' }} class="mr-3">
                        <div>
                    ...
                    <label class="flex items-center p-4 border-2 rounded-lg cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700 border-gray-300 dark:border-gray-600">
                        <input type="checkbox" name="is_vendor" value="1" {{ old('is_vendor') ? 'checked' : '' }} class="mr-3">
                        <div>
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <textarea name="address" rows="2"
                                  class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white"
                                  placeholder="บ้านเลขที่, ถนน">{{ old('address') }}</textarea>
                        @error('address')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
