...
class ProductQueryParser
{
    ...
    /**
     * 💭 คำค้นนี้คือ "สิ่งที่คนอยากได้ในชีวิต" ไม่ใช่ของที่ขายได้ใช่ไหม
     *
     * เปิดเป็น public เพื่อให้ **น้อง Eve ใช้ตัวเดียวกับบอทแม่หมอ** — เดิม Eve ไม่มีด่านนี้
     * วัดจริงบนพร็อด 2026-08-28: "อยากได้แฟน" → Eve ค้นแล้วเสนอ **สร้อยคอทอง 18k**
     * ให้คนที่กำลังพูดเรื่องความรัก (บอทแม่หมอกันได้แล้วเพราะมีด่านนี้)
     *
     * ⚠️ เทียบทั้งคำเดิมและคำที่ตัด "มี" นำหน้า — เหตุผลเดียวกับ isUsableQuery()
     */
    public static function isAbstractWish(string $query): bool
    {
        foreach ([trim($query), self::stripLeadingHave($query)] as $probe) {
            if ($probe !== '' && preg_match(self::ABSTRACT_WISH, $probe)) {
                return true;
            }
        }
