...
<div class="max-w-5xl mx-auto">
        ...
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center gap-2">
                <a href="{{ route('admin.roles.permissions', $role) }}"
                   class="px-4 py-2 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white rounded-xl transition-all duration-200 flex items-center gap-2">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
        <div class="glass-fusion dark:bg-gray-800 rounded-xl shadow p-6" hover:scale-105 transition-transform border border-white/20 dark:border-white/10>
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm text-gray-600 dark:text-gray-400 dark:text-gray-400">สิทธิ์ทั้งหมด</p>
                    <p class="text-3xl font-bold text-gray-900 dark:text-white dark:text-gray-200 mt-2">{{ $role->permissions->count() }}</p>
                </div>
        ...
        <div class="glass-fusion dark:bg-gray-800 rounded-xl shadow p-6" hover:scale-105 transition-transform border border-white/20 dark:border-white/10>
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm text-gray-600 dark:text-gray-400 dark:text-gray-400">ผู้ใช้ที่มีบทบาทนี้</p>
                    <p class="text-3xl font-bold text-gray-900 dark:text-white dark:text-gray-200 mt-2">{{ $role->users->count() }}</p>
                </div>
                <div class="text-4xl">👥</div>
            </div>
        </div>
        ...
        <div class="glass-fusion dark:bg-gray-800 rounded-xl shadow p-6" hover:scale-105 transition-transform border border-white/20 dark:border-white/10>
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm text-gray-600 dark:text-gray-400 dark:text-gray-400">ประเภท</p>
                    <p class="text-lg font-bold text-gray-900 dark:text-white dark:text-gray-200 mt-2">
                        ...
                        @else
                            <span class="px-3 py-1 bg-gray-100/50 dark:bg-gray-800/50 text-gray-900 dark:text-white dark:bg-gray-900 dark:text-gray-200 rounded-full text-sm">กำหนดเอง</span>
                        @endif
                    </p>
                </div>
    ...
    <div class="glass-fusion dark:bg-gray-800 rounded-xl shadow-md p-6 mb-6" hover:scale-105 transition-transform border border-white/20 dark:border-white/10>
        ...
        <div class="space-y-3">
            <div>
                <span class="text-sm text-gray-600 dark:text-gray-400 dark:text-gray-400">ชื่อบทบาท (ระบบ):</span>
                <span class="ml-2 font-medium text-gray-900 dark:text-gray-100">{{ $role->name }}</span>
            </div>
            <div>
                <span class="text-sm text-gray-600 dark:text-gray-400 dark:text-gray-400">ชื่อที่แสดง:</span>
                <span class="ml-2 font-medium text-gray-900 dark:text-gray-100">{{ $role->display_name }}</span>
            </div>
            @if($role->description)
            <div>
                <span class="text-sm text-gray-600 dark:text-gray-400 dark:text-gray-400">คำอธิบาย:</span>
    ...
    <div class="glass-fusion dark:bg-gray-800 rounded-xl shadow-md p-6 mb-6" hover:scale-105 transition-transform border border-white/20 dark:border-white/10>
                    ...
                    <div class="border border-gray-200 dark:border-gray-700 dark:border-gray-700 rounded-xl p-4">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
                            @foreach($categoryPermissions as $permission)
                                <div class="flex items-start p-2 bg-gray-100/50 dark:bg-gray-800/50/50 dark:bg-gray-800/50 dark:bg-gray-700 rounded">
        ...
        @else
            <p class="text-gray-500 dark:text-gray-400 dark:text-gray-400">บทบาทนี้ยังไม่มีสิทธิ์ใดๆ</p>
        @endif
    </div>
    ...
    <div class="glass-fusion dark:bg-gray-800 rounded-xl shadow-md p-6" hover:scale-105 transition-transform border border-white/20 dark:border-white/10>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    <thead class="bg-gray-100/50 dark:bg-gray-800/50/50 dark:bg-gray-800/50 dark:bg-gray-900">
                        <tr>
                            <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 dark:text-gray-400 uppercase">ชื่อ</th>
                            <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 dark:text-gray-400 uppercase">อีเมล</th>
                            <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 dark:text-gray-400 uppercase">วันที่สร้าง</th>
                            <th class="px-4 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-400 dark:text-gray-400 uppercase">จัดการ</th>
                        </tr>
                    </thead>
                    <tbody class="glass-fusion dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        @foreach($role->users as $user)
                            <tr class="hover:bg-gray-100/50 dark:bg-gray-800/50/50 dark:bg-gray-800/50 dark:hover:bg-gray-700">
                                ...
                                <td class="px-4 py-3 text-sm text-gray-600 dark:text-gray-400 dark:text-gray-400">{{ $user->email }}</td>
                                <td class="px-4 py-3 text-sm text-gray-600 dark:text-gray-400 dark:text-gray-400">{{ $user->created_at->format('d/m/Y') }}</td>
        ...
        @else
            <p class="text-gray-500 dark:text-gray-400 dark:text-gray-400">ยังไม่มีผู้ใช้ที่มีบทบาทนี้</p>
        @endif
    </div>
</div>
@endsection
