...
<head>
    ...
    <!-- Notification Bell Component Script (โหลดก่อน Alpine.js) -->
    <script>
        // กำหนด Alpine component สำหรับ notification bell
        document.addEventListener('alpine:init', () => {
            Alpine.data('notificationBellComponent', () => ({
                isOpen: false,
                loading: false,
                notifications: [],
                unreadCount: 0,
                    ...
                    try {
                        ...
                        if (response.ok) {
                            ...
                            if (data.notifications && data.notifications.length > 0) {
                                // Dispatch event สำหรับ immediate notifications popup
                ...
                async markAsRead(notificationId) {
                    try {
                        const response = await fetch(`/${this.routePrefix}/notifications/${notificationId}/read`, {
                            method: 'POST',
                            headers: {
                ...
                async markAllAsRead() {
                    try {
                        const response = await fetch(`/${this.routePrefix}/notifications/read-all`, {
                            method: 'POST',
                            headers: {
                                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                                'X-Requested-With': 'XMLHttpRequest',
