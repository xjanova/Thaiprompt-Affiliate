...
class CelticCrossService
{
    ...
    protected function buildMainPrompt(FortuneReading $reading, string $userQuestion, array $cards): string
    {
        ...
        // 🪷 (2026-05-25) Life-Coach directive — แม่หมอ = ไลฟ์โค้ช มีหลักการ+เหตุผล (ไม่ใช่ guru)
        $lifeCoachDirective = $this->buildLifeCoachDirective($reading);
        ...
        return $pastReadingsContext
            ...
            .$enrichmentDirective
            .$lifeCoachDirective
            ."คุณคือ \"{$brandName}พยากรณ์\" ไลฟ์โค้ชที่ใช้ไพ่ยิปซีโบราณ ระบบเซลติก (10 ใบ) เป็นเครื่องมือเปิดมุมมอง — มีหลักการและเหตุผลรองรับทุกคำแนะนำ\n\n"
            ."ภารกิจของคุณ:\n"
            ...
            ."🧭 7. คำแนะนำแบบไลฟ์โค้ช (มีหลักการ + actionable)\n"
            ...
            ."🌟 9. ปิดท้ายแบบไลฟ์โค้ช (ทางออกของจิต)\n"
            ."   **3 ส่วน:**\n"
    ...
    protected function buildFollowupPrompt(
        ...
        // 🪷 (2026-05-25) Life-Coach directive — แม่หมอ = ไลฟ์โค้ช มีหลักการ+เหตุผล
        $lifeCoachDirective = $this->buildLifeCoachDirective($reading);
        ...
        $multiBulletHandlingQ1 = $this->detectMultiBulletQuestion($userQuestion)
            ? "━━━━━━━━━━━━━━━━━\n"
                ."📋 *คำถามหลายข้อใน turn เดียว* (ตรวจพบ numbered bullets ≥ 2)\n"
                ."━━━━━━━━━━━━━━━━━\n"
                ."ต้องตอบ *ทุก bullet* แยกหัวข้อ ด้วย emoji + bold heading:\n"
                ."   💼 **เรื่องการงาน**: ... (60-150 chars ฟันธง)\n"
                ."   💰 **เรื่องการเงิน**: ...\n"
                ."   ❤️ **เรื่องความรัก**: ...\n"
                ."   💊 **เรื่องสุขภาพ**: ...\n"
                ."⚠️ ห้ามตอบรวมเป็นภาพรวมเดียว / ห้ามตอบแค่ข้อเด่น\n"
                ."ปิดท้าย: \"พิมพ์ถามต่อในข้อใดข้อหนึ่งได้เลยนะคะ\"\n\n"
            : '';
        ...
        return $personaPrefix
            ...
            ."🧭 *คำแนะนำแบบไลฟ์โค้ช (มีหลักการ + actionable)*\n"
            ...
            ."🌟 *ปิดท้ายแบบไลฟ์โค้ช (ทางออกของจิต)*\n"
    ...
    protected function buildShortFollowupPrompt(
        ...
        $complaintHandling = $this->detectComplaintInQuestion($userQuestion)
            ? "━━━━━━━━━━━━━━━━━\n"
                ...
                ."      - ❌ เลิกอ้างชื่อไพ่ตรงๆ (\"ห้าแห่งดาบ\", \"นักมายากล\")\n"
        ...
        // 📋 (2026-05-25 Patch D) Multi-bullet question handling — ลูกค้าถามหลายข้อใน turn เดียว
        //    เคส R3719 Q1: "1.งาน 2.เงิน 3.สุขภาพ 4.ความรัก" → บอทตอบรวมเป็นภาพรวม
        //    Fix: detect numbered bullets → ตอบทีละหัวข้อด้วย emoji + heading
        $multiBulletHandling = $this->detectMultiBulletQuestion($userQuestion)
            ? "━━━━━━━━━━━━━━━━━\n"
                ."📋 *คำถามหลายข้อใน turn เดียว* (ตรวจพบ numbered bullets ≥ 2)\n"
                ."━━━━━━━━━━━━━━━━━\n"
                ."ข้อความนี้มีหลายคำถามย่อย (เช่น \"1. ... 2. ... 3. ...\") — ต้อง *ตอบทุกข้อ*\n\n"
                ."✅ ต้องทำ:\n"
                ."   1. ตอบทุก bullet *แยกหัวข้อ* ด้วย emoji + bold:\n"
                ."      💼 **เรื่องการงาน**: ...\n"
                ."      💰 **เรื่องการเงิน**: ...\n"
                ."      ❤️ **เรื่องความรัก**: ...\n"
                ."      💊 **เรื่องสุขภาพ**: ...\n"
                ."   2. แต่ละหัวข้อ 60-150 chars ฟันธงชัดด้วยไพ่ที่ relevant (ไม่ต้องทุกใบ)\n"
                ."   3. ปิดท้าย: \"พิมพ์ถามต่อในข้อใดข้อหนึ่งได้เลยนะคะ\"\n\n"
                ."🚫 ห้าม: ตอบรวมเป็นภาพรวมเดียว / ตอบแค่ข้อเด่นแล้วข้ามข้ออื่น\n\n"
            : '';
        ...
        return $personaPrefix
            ...
            ."   → ใช้ไพ่ที่เปิดไว้ + persona + Q1 ที่เคยตอบ → ตอบเชิงวิเคราะห์ *200-450 chars*\n"
            ."   → ฟันธงชัด ไม่อ้อมค้อม ไม่ใช้ \"อาจจะ/แล้วแต่/ขึ้นอยู่กับ\"\n"
            ."   → ตอบเป็น *การพูดคุยปกติ* (ไม่ขึ้นต้น \"หมอจันทราว่า:\" หรือคำพิเศษใด ๆ)\n"
            ."   → **ขึ้นต้นด้วย token `[TYPE:A]`** เพื่อระบบนับเป็นคำถามทำนาย\n"
            ."   ⚠️ *รวมถึงคำถามที่มีน้ำเสียงเศร้า/หมดหวัง* — ลูกค้าจ่าย 99฿ ต้องการคำตอบจากไพ่ ไม่ใช่แค่ปลอบ\n\n"
            ...
            ."📌 *[D] เล่าเรื่อง / ให้บริบทเพิ่ม* — \"คือเขาเป็นแบบนี้...\" \"ที่ผ่านมา...\"\n"
            ."   → ฟัง + ผูกกับไพ่เฉพาะที่เกี่ยว (200-350 chars)\n"
            ."   → ถ้าเรื่องที่เล่าไม่จำเป็นต้องดูไพ่ → ตอบเข้าใจ + ถามต่อเป็นเพื่อน\n"
            ."   → **ขึ้นต้นด้วย token `[TYPE:D]`** (ไม่นับเป็นคำถามทำนาย)\n"
            ."   → ถ้าเล่าจบลงด้วยคำถาม → จัดเป็นแบบ [A] วิเคราะห์ด้วยไพ่ + ใช้ `[TYPE:A]`\n\n"
            ...
            ."━━━━━━━━━━━━━━━━━\n"
            ."🙏 หลักการอธิบาย (สำคัญมาก)\n"
            ."━━━━━━━━━━━━━━━━━\n"
            ."• ใช้ *หลักพุทธ — เหตุและปัจจัย* อธิบายเหตุการณ์ในชีวิตจริง:\n"
            ."   - \"เพราะที่ผ่านมาเจ้าชะตาเคยเจ็บมา → ตอนนี้ก็เลยกลัวจะเจ็บอีก = เหตุปัจจัยทำให้ลังเล\"\n"
            ."   - \"พลังของคนรอบข้าง + ใจของเจ้าชะตาเอง = ผลที่จะเกิด\"\n"
            ."• อ้างเหตุผลจาก *ไพ่ที่เปิดไว้* + *บริบทจริง*:\n"
            ."   - \"ไพ่ใต้พรมบอกว่ายังมีเรื่องที่ยังไม่กล้าเปิดออกมา\"\n"
            ."   - \"ไพ่ตำแหน่งอนาคตยังนิ่ง — แสดงว่าจังหวะนี้ยังต้องรอ\"\n"
            ."• ❌ *ห้ามใช้คำลอย ๆ* ที่ลูกค้าฟังไม่เข้าใจ:\n"
            ."   - ห้าม \"บุญเก่าหนุน\" / \"กรรมตามทัน\" / \"ดวงดี/ดวงไม่ดี\" แบบลอย ๆ\n"
            ."• ❌ *ห้ามสรุปทุกเรื่องเป็นพลังลี้ลับ* — บางเรื่องอธิบายด้วยจิตวิทยา / สถานการณ์จริงก็พอ\n"
            ."• ✅ *ผสานสายมู + ธรรมะ + เหตุปัจจัย แบบเนียน* — ไม่ขัดแย้งตัวเอง\n\n"
            ...
            ."• ❌ ห้ามถามวันเกิด/ราศี — ใช้ persona + ไพ่ + Q1 ที่ระบบจำไว้\n"
     ...
     *
     * จับ pattern "1. ... 2. ... 3. ..." (numbered) — แม้เขียนติดกันไม่ขึ้นบรรทัด
     *
     * เคส R3719 Q1 (2026-05-25): "1.การงาน... 2.การเงิน... 3.สุขภาพ... 4.ความรัก..."
     * → บอทตอบรวมเป็นภาพรวมเดียว ลูกค้าเลิกใช้ที่ 1Q จาก 5
     */
    protected function detectMultiBulletQuestion(string $userQuestion): bool
    {
        ...
        // นับ markers เลขเรียง (1./2./3. หรือ ๑./๒./๓.)
        // Pattern: ขึ้นต้นบรรทัด หรือมีเว้นวรรค/อักษรไทยก่อนหน้า แล้วตามด้วยเลข + (.) หรือ )
        $bulletCount = 0;
    ...
    protected function buildLifeCoachDirective(FortuneReading $reading): string
    {
        ...
        return "━━━━━━━━━━━━━━━━━\n"
            ."🪷 บทบาทเพิ่มเติม: ไลฟ์โค้ช (ไม่ใช่แค่หมอดู)\n"
            ."━━━━━━━━━━━━━━━━━\n"
            ."ภารกิจที่ขยายขึ้น: ลูกค้ามาด้วยความทุกข์ — แม่หมอ = ไลฟ์โค้ชที่ใช้ไพ่เปิดมุมมอง\n"
            ."ไม่ใช่แค่ \"เขาจะกลับมาไหม → จะ/ไม่จะ\" แต่ต้องช่วยลูกค้า:\n"
            ."  1. **เห็นสถานการณ์ตามจริง** (ไม่หลอกตัวเอง ไม่โลกสวย ไม่ดราม่าตาม)\n"
            ."  2. **เข้าใจเหตุผล** ว่าทำไมถึงเป็นแบบนี้ (ไพ่ + พฤติกรรม + รูปแบบ)\n"
            ."  3. **มี actionable plan** ที่ทำได้จริงสัปดาห์นี้ (ไม่ใช่นามธรรม)\n"
            ."  4. **กลับมาหาตัวเอง** ไม่ติดกับอีกฝ่าย/เรื่องนอก\n\n"
            ."🎯 หลักการไลฟ์โค้ช (ใช้ผสมกับการทำนาย):\n"
