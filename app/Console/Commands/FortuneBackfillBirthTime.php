...
class FortuneBackfillBirthTime extends Command
{
    ...
    {
        // 🚫 คำที่ "ดูเหมือน" ชี้คนอื่นแต่ไม่ใช่ — ต้องล้างทิ้งก่อน ไม่งั้นตัดผิดเกือบทุกใบ
        //    "แม่หมอ" = ลูกค้าเรียกหมอดู (โผล่แทบทุกข้อความ) · "ลูกค้า" = ลูกค้าของเขาเอง
        //    ❌ ไม่ใส่ "พี่/น้อง" ในรายการ — คนไทยใช้เป็นสรรพนามเรียกตัวเอง/คู่สนทนา
        $clean = (string) preg_replace('/(แม่หมอ|ลูกค้า)/u', '  ', $text);

        $others = 'แฟน|สามี|ภรรยา|เมีย|ผัว|คู่ครอง|คนรัก|พ่อ|แม่|ลูกชาย|ลูกสาว|เพื่อน|หัวหน้า|เจ้านาย|เขา|เธอ';
        if (! preg_match_all('/'.$others.'/u', $clean, $ms, PREG_OFFSET_CAPTURE)) {
            return false;
        }

        // 🇹🇭 ภาษาไทยวางเจ้าของไว้ **หน้า** กริยา — "แฟนผม.เกิด..." · "พ่อเกิด..." · "สามีเกิด..."
        //    ⇒ ตัดเฉพาะตอนคำชี้คนอื่นมา *ก่อน* คำว่าเกิด และอยู่ติดกัน
        //    ถ้ามาทีหลังแปลว่าคนละประโยค — "เกิดตีห้า อยากถามเรื่องเพื่อนที่ทำงาน" ต้องผ่าน
        // ⚠️ PREG_OFFSET_CAPTURE คืน offset เป็น **ไบต์** → เทียบกับ strpos (ไบต์) เท่านั้น
        //    ไทย ~3 ไบต์/ตัวอักษร ⇒ 45 ไบต์ ≈ 15 ตัวอักษร
        foreach ($ms[0] as $hit) {
            $posOther = (int) $hit[1];
            foreach (['เกิด', 'คลอด'] as $cue) {
                $p = 0;
                while (($p = strpos($clean, $cue, $p)) !== false) {
                    if ($posOther < $p && ($p - $posOther) <= 45) {
                        return true;
                    }
                    $p++;
                }
