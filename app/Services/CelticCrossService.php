...
class CelticCrossService
{
    ...
    protected function buildBaseChartSignalKnowledge(FortuneReading $reading, array $include = ['black_magic', 'health', 'combo']): string
    {
        ...
        // 🪬 ไสยศาสตร์รายไพ่ (โดนของ/มนต์ดำ) — เกือบทุกใบมีบรรทัด ส่วนใหญ่ "ไม่มีของ" ทักเฉพาะที่ไพ่ชี้ชัด
        if (in_array('black_magic', $include, true) && (bool) ($this->settings->enable_celtic_black_magic_mode ?? true)) {
            $bm = trim((string) $svc->blackMagicLinesForCards($cards));
            if ($bm !== '') {
                $hasBlackMagic = true;
                $sections[] = "🪬 สัญญาณไสยศาสตร์/ของ (รายไพ่ — ส่วนใหญ่ \"ไม่มีของ\" ทักเฉพาะใบที่ระบุว่ามีจริง):\n".$bm."\n"
                    ."   🧭 จรรยาบรรณเรื่องของ: ❌ ห้ามขายความกลัว ❌ ห้ามขู่ว่าโดนของหนักจะตาย/พินาศ ❌ ห้ามแนะพิธีแก้ด่วนเป็นเงินก้อนโต\n"
                    .'   • ถ้าทุกใบ "ไม่มีของ" → ห้ามยกเรื่องของมาทักเลย / ลูกค้าเปราะบาง-ระแวง → เน้นดึงสติ ดูสุขภาพกาย-ใจควบคู่';
            }
        }
        ...
        $header = $hasBlackMagic
            ? '⚠️ คลังนี้ "เทียบเคียง" เท่านั้น — ดึงเฉพาะที่หน้าไพ่ชี้ชัด ห้ามยัด ห้ามขายความกลัว (ส่วนใหญ่ "ไม่มีของ/ไม่มีสัญญาณร้าย")'
    ...
    protected function buildFollowupPrompt(
        ...
        if ($previousQA->isNotEmpty()) {
            ...
            $previousContext = "📜 ฐานการทำนายรอบนี้ — คำทำนาย Q1 (พื้นดวง) + บทสนทนาที่ผ่านมา:\n"
                ."⚓ ใช้ \"ทั้งหมด\" นี้ + ไพ่ 10 ใบ + ดาวเจ้าชนะ เป็น *ฐานความจริงเดียวกันทั้งรอบ* — ทุกคำตอบต้องสอดคล้อง ห้ามขัดแย้งกับที่เคยทำนายไว้\n"
                ."🧭 แล้ว *ต่อยอดเป็นทางออกที่ทำได้จริง* ให้เจ้าชะตา (แม่หมอที่เก่ง = ปลอบใจ + บอกความจริงตามหน้าไพ่ + ให้ทางออกที่ใช้ได้จริง ไม่ใช่แค่คำปลอบ):\n\n";
        ...
        if ($reading->getConversationState('celtic_base_chart')) {
            $baseChartDirective =
                ...
                ."📐 โครงสร้าง — *เปิดด้วยพาดหัวเรื่องเด่นก่อน* แล้วตามด้วย 6 ส่วน (ใส่ให้ครบ • ใช้ emoji หัวข้อ • เว้นย่อหน้าให้อ่านง่าย):\n"
                ...
                ."     + ทางแก้ทำได้จริง 1 ข้อ / สำรับสงบ → พาดหัวจุดเด่นเชิงบวก — ❌ ห้ามคลุมๆ ห้ามขายความกลัว\n"
                ...
                ."  4) 📅 ฤกษ์/สีมงคล/เลขมงคล/วันมงคล + ทิศ (จากดาวมิตร) + สิ่งที่ควรรีบทำ/ควรเลี่ยง\n"
                ."  5) ⚠️ สิ่งที่ต้องระวัง (จากดาวศัตรู + ไพ่เตือน) + ทางแก้/วิธีเสริมดวงที่ทำเองได้\n"
                ."  6) 💫 สรุปภาพรวม + ให้กำลังใจ + ชวนเจ้าชะตาถามเจาะลึกต่อทีละเรื่อง\n\n"
    ...
    public function askQuestionWithImage(FortuneReading $reading, string $imageData, string $userText = ''): array
    {
        ...
        $systemPrompt = $personaPrefix
            ...
            ."📸 บริบทรูป + คำตอบ\n"
            ."━━━━━━━━━━━━━━━━━\n"
            ."• 👍 *รูปยกนิ้ว/สติกเกอร์/อิโมจิ/มีม/รูปที่ไม่เกี่ยวกับการดูดวง* → *ห้ามถามกลับ ห้ามวิเคราะห์ลึก*\n"
            ."  ตอบสั้น ๆ รับทราบเป็นกันเอง + ชวนถามเรื่องที่อยากให้ดู เช่น \"รับทราบค่ะ 🌙 อยากให้แม่หมอดูเรื่องไหนต่อคะ\"\n"
            ."• รูปที่เกี่ยวกับการดูดวง (คน/มือ/หน้า/บ้าน/วัตถุมงคล/สถานที่ ฯลฯ) → วิเคราะห์ในบริบทการดูดวง ผูกกับไพ่ที่เปิดไว้\n"
            ."• ❓ *ถ้าไม่ชัดว่าเจ้าชะตาต้องการอะไรจากรูป → ถามก่อนเสมอ อย่าทึกทักเดาเอง*\n"
            ."  เช่น \"แม่หมอเห็นรูปนี้ — อยากให้แม่หมอดูเรื่องอะไรจากรูปนี้คะ?\"\n"
